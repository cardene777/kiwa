...
function freshnessProblem(pkg, pkgDir) {
  ...
  const inputRels = MUTATION_INPUT_DIRS.map((suffix) => `${pkgDir}/${suffix}`);
  ...
  if (byContent.state === 'mismatch') {
    return `${artifactRel} was measured against different content (${byContent.reason}). Re-run \`pnpm -F ${pkg} test:mutation\`.`;
  }
  if (byContent.state === 'unusable') {
    return `cannot tell whether ${artifactRel} is current (${byContent.reason}). Re-run \`pnpm -F ${pkg} test:mutation\`.`;
  }
