 ...
 * `@kiwa-lab/*` package because they are test-adapter infrastructure that
 * emit no runtime DOM.
 *
...
async function main() {
  ...
  let runLayerHarness;
  try {
    ({ runLayerHarness } = await import('@kiwa-lab/a11y'));
  } catch (err) {
    console.error(
      `[a11y] ${pkgName}: could not import @kiwa-lab/a11y (needed for fixture-wired baseline). ${err instanceof Error ? err.message : err}`,
    );
    process.exit(1);
  }
