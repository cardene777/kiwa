...
/**
 * Memcached backend selection.
 * - `stub`: in-process, deterministic Memcached protocol emulation. No docker,
 *   no network. Suitable for unit tests that need to exercise the 8 core
 *   Memcached commands + TTL + multi-server consistent hashing without
 *   spinning up a container.
 * - `testcontainers`: start a real Memcached in a testcontainers-managed
 *   Docker container. Deterministic + prod-shape parity. Requires Docker.
 */
 ...
 * depend on in prod.
 * - `memjs`: the modern Node.js Memcached client (peer `memjs@^1`).
...
/**
 * Return type of {@link setupMemcachedEnv}. Reads much like a tiny Memcached
 * facade — consumers exercise the 8 core commands (get / set / delete / add /
 * replace / increment / decrement / flush) + TTL + multi-server consistent
 * hashing without touching a real client.
 */
