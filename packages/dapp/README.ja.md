...
`@kiwa-test/dapp` は [kiwa](https://github.com/cardene777/kiwa) の Playwright ベース dApp E2E layer が使う runtime fixture。 test page に `window.ethereum` を inject し、 anvil ベースの wallet fixture を起動、 EIP-1193 の主要 flow を直接処理し、 残りの JSON-RPC surface は anvil に転送する。

使い方は 2 通り。 (a) Claude に `/kiwa-play` で Playwright test を生成させる、 (b) fixture を直接 import (`import { dappE2eTest as test } from "@kiwa-test/dapp"`) して手書きする。 contract test (`@kiwa-test/forge`) + dApp e2e (`@kiwa-test/dapp`) + 手書き の 3 経路の全体像は [kiwa README](https://github.com/cardene777/kiwa) を参照。
...
`@kiwa-test/dapp` は以下 method を直接処理する。
