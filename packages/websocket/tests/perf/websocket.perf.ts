...
import { createWSServer, connectClient, sendMessage, broadcastMessage, captureBinaryFrame, encodeBinaryFrame } from '../../src/index.js';
...
describe(MODULE, () => {
  it(
    '3-layer perf: send + broadcast + binary frame parse primary paths',
    async () => {
        ...
        ops: [
          {
            ...
            serialP95CapMs: 5,
            fn: async () => {
              sendMessage(server, server.clients()[0]?.id ?? '', 'ping');
            },
          },
          {
