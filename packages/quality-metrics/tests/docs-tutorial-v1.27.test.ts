 ...
 *
 * v1.27 は @kiwa-lab/quality-metrics v0.3 の 4-tier mutation SSOT + 12-axis
...
describe('tutorial 51 — Section 1: 4-tier SSOT table', () => {
  ...
  it('Framework tier floor is 70 — matches @kiwa-lab/nextjs stryker config', () => {
    expect(DEFAULT_MUTATION_TIER_THRESHOLDS.framework).toBe(70);
  });

  it('SaaS tier floor is 65 — matches @kiwa-lab/ai-llm / payment / queue defaults', () => {
    expect(DEFAULT_MUTATION_TIER_THRESHOLDS.saas).toBe(65);
  });

  it('Test type tier floor is 60 — matches @kiwa-lab/ui / a11y / visual defaults', () => {
