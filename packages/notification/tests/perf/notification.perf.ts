...
import { createNotificationClient, sendPush, sendSMS, parseNotificationEvent } from '../../src/index.js';
...
describe(MODULE, () => {
  it(
    '3-layer perf: push + sms + event parse primary paths',
    async () => {
        ...
        ops: [
          {
            ...
            serialP95CapMs: 5,
            fn: async () => {
              await sendPush(client, { deviceToken: 'tok-1', title: 't', body: 'b' });
            },
          },
          {
            name: 'sendSMS',
            serialP95CapMs: 5,
            fn: async () => {
              await sendSMS(client, { to: '+15551234', from: '+15559999', body: 'hi' });
            },
          },
          {
            name: 'parseNotificationEvent',
            serialP95CapMs: 5,
            fn: async () => {
              parseNotificationEvent({ provider: 'fcm', raw: { event: 'delivered', notification_id: 'fcm-1', timestamp: 1 } });
            },
          },
        ],
      });
