...
describe(MODULE, () => {
  it(
    '3-layer perf: parseGraphQLOperation + executeQuery + client.query primary paths',
    async () => {
      ...
      const result = await runPerf3Layer({
        ...
        ops: [
          {
            name: 'parseGraphQLOperation',
            serialP95CapMs: 5,
            fn: async () => {
              parseGraphQLOperation('query GetHello { hello }');
            },
          },
          {
            name: 'executeQuery',
            serialP95CapMs: 5,
            fn: async () => {
              await server.executeQuery('{ hello }');
            },
          },
          {
            name: 'clientQuery',
