...
export function generateLeanSpec(spec: OrchestratorSpec): LeanSpecOutput {
  ...
  // `blocks` は空にならない。 `renderTheorems()` が state ごとに必ず 1 件返し (§ その JSDoc)、
  // state が 0 件の spec は検査が reject するため `blocks.length >= states.length >= 1` になる。
  //
  // 空の場合を三項で分けていたが、その枝へ到達する入力は組めなかった (#2195)。 到達しない枝は
  // 覆えない未通過 branch として残り続け、 coverage の gap report が毎回「次に埋めるべき候補」
  // として同じ行を出す。 到達不能な枝は実装の欠陥として畳む
  // (`rules/quality.md § 届かない時は理由を書く` の分類 2)。
  const theorems = `\n${blocks.join('\n\n')}\n\n`;
 ...
 */
/**
 * 契約 — **state 1 つにつき定理を必ず 1 件返す**。
 *
 * 終端なら `_absorbing`、 非終端で脱出 cell が無ければ `_no_escape`、 あれば `_can_leave` で、
 * 3 経路のいずれかに必ず落ちる。 返り値が空になるのは `states` が空の時だけで、 その spec は
 * 呼出前の検査が reject する。
 *
 * 呼出側 (`generate()`) はこの契約に依存して、 定理 block が空になる場合を分けていない (#2195)。
 * 経路を 1 つでも push しない形に変えると、 その state の定理が黙って消える = 生成された Lean
 * file は compile を通るが、 その state について何も主張しなくなる。
 */
