...
- 🆕 feat(v1.28-1): `@kiwa-lab/realtime` v0.2.0 — advanced-transport semantics for 3 protocols × 8 axes plus a real-driver env-gate for the 4 base providers.
...
- bd40f20: 🆕 feat(v1.13-2): `@kiwa-lab/realtime` v0.1.0 — 4 provider unified mock harness (Supabase Realtime + Ably + Pusher + Socket.io/SSE)
  ...
  新規 package のため既存 breaking なし。 `@kiwa-lab/quality-metrics` v0.2 の AI-LLM provider 判定 (`isAiLlmProvider`) は変更不要 — realtime 側で AI-LLM 4 軸 shape に変換して同じ 11 軸 gate を再利用する。
