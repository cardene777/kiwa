 ...
 * (v1.23-v1.30) を確立する。
 *
 * v1.30 は @kiwa-lab/a11y v1.1 の 3-layer harness (runLayerHarness +
 * bucketViolations + computeTotals + isHarnessOk) + @kiwa-lab/quality-metrics
...
describe('tutorial 57 — Section 4: layers-absent baseline shape', () => {
  it('layers-absent baseline totals are all zero and ok is true', async () => {
    ...
    const report = await runLayerHarness('@kiwa-lab/my-package', {});
