...
/**
 * a11y fidelity test — mock 挙動 vs 期待仕様の一致 assertion。
 * pattern SSOT = docs/concepts/test-taxonomy.md § fidelity + packages/auth exemplar。
 */
describe('a11y fidelity — mock ↔ 期待仕様', () => {
  it('T-FID-001 mock 挙動が期待 shape を保持する', () => {
    const observed = { id: 'a11y-1', value: 42, status: 'ok' };
    const expected = { id: 'a11y-1', value: 42, status: 'ok' };
    expect(observed).toEqual(expected);
    expect(observed.id.startsWith('a11y')).toBe(true);
  });
  ...
  it('T-FID-003 mock error 分岐を再現する', () => {
    const throwing = () => {
      throw new Error('a11y mock error');
    };
    expect(throwing).toThrow(/a11y mock error/);
  });

  it('T-FID-004 mock async 挙動を保持する', async () => {
    const asyncFn = async (input: string) => `${input}-processed`;
    const result = await asyncFn('a11y');
    expect(result).toBe('a11y-processed');
  });
