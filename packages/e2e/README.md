...
`@kiwa/e2e` exposes a single `setupE2eEnv` helper that:
...
Unlike `@kiwa/dapp` (which is anchored on dApp + anvil), `@kiwa/e2e` targets generic web apps.

## Install

```bash
pnpm add -D @kiwa/e2e @kiwa/core @playwright/test vitest
