 ...
 * client SDK. All server-side verification funnels through two APIs — JWT session
 * verification (HS256-signed access_token / refresh_token pair) and the REST admin API
 * (`auth.admin.createUser` / `auth.admin.getUserById` / `auth.admin.listUsers` etc).
 *
 * The mock mirrors those two surfaces so a suite can swap the real client with the
 * mock and drive the same code paths without hitting the network. The v0.3 scope
 * covers Supabase Auth core semantics — email/password + OAuth + magic link + JWT
 * session mock. Advanced features (RLS policy mock / MFA / SSO SAML / Web3 wallet)
 * are covered by the advanced adapter in v1.10-2.
 */

/**
 * Supabase identity — one per external provider (email or OAuth) linked to a user.
 * Real Supabase surfaces `identities: Identity[]` on every user record.
 */
...
/**
 * Supabase user record. The mock covers the fields that consumers assert against
 * in tests — id + email + phone + identities + app_metadata + user_metadata +
 * timestamps. Real Supabase carries additional fields (banned_until / is_sso_user /
 * confirmation_sent_at etc) that are added lazily as needed.
 */
...
/**
 * Supabase session — a pair of tokens (access_token + refresh_token) plus the user
 * they authenticate. Real Supabase returns this shape from `signInWithPassword` /
 * `signUp` / `signInWithOtp` / `exchangeCodeForSession`.
 */
...
/**
 * JWT claims Supabase Auth embeds in the access_token. The mock encodes these in
 * an HS256-signed JWT the `verifyToken` helper decodes back.
 */
...
/**
 * OTP delivery record — captured whenever the mock issues a magic link or an SMS
 * OTP. Tests can inspect this to assert the delivery channel + one-time code.
 */
...
/**
 * OAuth authorization URL response — mimicking `signInWithOAuth`. The mock
 * captures the URL for the test to assert against, then a follow-up
 * `exchangeCodeForSession` completes the PKCE flow.
 */
...
/**
 * Options accepted by {@link setupSupabaseAuthEnv}. Every field is optional — the
 * defaults exercise a single anonymous project shape.
 */
...
/**
 * The test env returned by {@link setupSupabaseAuthEnv}. Consumers hold this handle
 * for the lifetime of a test and call `stop()` in `afterEach` to reset all
 * in-memory state.
 *
 * The `auth` handle mirrors `@supabase/supabase-js`'s `client.auth.*` client surface
 * so call sites are drop-in-compatible after swap. The `admin` handle mirrors
 * `client.auth.admin.*` — the service-role-only API.
 */
