...
describe(MODULE, () => {
  it(
    '3-layer perf: register + execute + retry primary paths',
    async () => {
      const client = createWorkflowClient({ provider: 'temporal' });
      const wf = defineWorkflow('noop', [
        { name: 'step1', run: () => ({ ok: true }) },
      ]);
      client.register(wf);
      ...
      const result = await runPerf3Layer({
        ...
        ops: [
          ...
          },
          {
            ...
            serialP95CapMs: 5,
            fn: async () => {
              defineWorkflow('inline', [{ name: 's', run: () => ({}) }]);
            },
          },
          {
            name: 'retryStepSucceed',
            serialP95CapMs: 5,
            fn: async () => {
              await retryStep(async () => ({ ok: true }), { maxAttempts: 1, baseDelayMs: 1 });
            },
          },
        ],
      });
