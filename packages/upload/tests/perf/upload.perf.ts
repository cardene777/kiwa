...
import { createUploadClient, createPresignedUrl, verifyUpload } from '../../src/index.js';
...
describe(MODULE, () => {
  it(
    '3-layer perf: upload + presigned URL + verify primary paths',
    async () => {
        ...
        ops: [
          {
            ...
            serialP95CapMs: 5,
            fn: async () => {
              await client.upload({ bucket: 'b', key: `k-${Math.random()}`, body });
            },
          },
          {
            ...
            serialP95CapMs: 5,
            fn: async () => {
              createPresignedUrl({ provider: 's3', bucket: 'b', key: 'k', operation: 'put' });
            },
          },
          {
            name: 'verifyUpload',
            serialP95CapMs: 5,
            fn: async () => {
              verifyUpload({ body, expectedSize: body.byteLength });
            },
          },
        ],
      });
