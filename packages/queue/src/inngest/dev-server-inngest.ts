...
async function pingDevServer(url: string, timeoutMs: number): Promise<void> {
  ...
  throw new Error(
    `@kiwa-lab/queue: Inngest dev-server did not respond at ${probeUrl} within ${timeoutMs}ms`,
  );
}
...
export async function createDevServerInngestEnv(
  ...
  async function postEventToDevServer(event: InngestEvent): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `@kiwa-lab/queue: dev-server rejected event "${event.name}" (HTTP ${response.status}): ${body}`,
      );
    }
    try {
