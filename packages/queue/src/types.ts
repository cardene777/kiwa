 ...
 *   `testcontainers` + `bullmq` + `ioredis` peer dependencies.
 * - `sandbox`: run against an in-process Redis-compatible stub tied to the
 *   test process only. Fast (no container startup), fully offline, and
 *   sufficient for a large slice of BullMQ semantics (add / process / retry /
 *   fail / drain) but does not exercise Redis-side pipelining semantics.
 */
...
/**
 * Options accepted by every {@link BullMQTestEnv.addJob} call. Mirrors the
 * subset of `bullmq.JobsOptions` we honour in both testcontainers and sandbox
 * modes.
 */
...
/**
 * Common options for the `setupBullMQEnv` factory. `mode` chooses the backend;
 * `redis` and `sandbox` are backend-specific overrides.
 */
...
/**
 * Return type of {@link setupBullMQEnv}. Reads much like a mini BullMQ facade —
 * consumers register a processor, add jobs, then use the assertion helpers to
 * observe outcomes without touching BullMQ directly.
 */
