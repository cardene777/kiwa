...
/**
 * AWS SQS backend selection.
 * - `stub`: in-process, deterministic FIFO / standard queue emulation. No
 *   docker, no network. Suitable for unit tests that need to exercise the
 *   send / receive / delete / batch / visibility timeout / DLQ semantics
 *   without spinning up localstack.
 * - `localstack`: run against a real LocalStack container. Exercises the
 *   actual `@aws-sdk/client-sqs` wire with a real (offline) SQS API.
 */
 ...
 * live in the queue waiting for the next receive. `inflight` messages have
 * been received and are within their visibility timeout window. `deleted`
 * covers messages the consumer explicitly deleted. `dead` covers messages
 * that exhausted `maxReceiveCount` and were routed to the DLQ.
 */
...
/**
 * Received message wrapper — consumers call `.delete()` to remove the
 * message after successful processing, `.changeVisibility()` to extend the
 * inflight window, or let the visibility timeout expire so the message
...
/**
 * Queue declaration — passed to `setupSQSEnv({ queues: [...] })` to create
 * the queue up front (both stub + localstack modes honour this).
 */
...
/**
 * Return type of {@link setupSQSEnv}. Reads much like a mini SQS facade —
 * consumers create queues, send / receive / delete messages, and use the
