...
// Lifecycle tests (v1.6-3 Stop() activation, v1.7-4 Fiber adapter).
//
// Post-Stop Send() is a lifecycle contract violation and the Fiber adapter
// reports it through the captured testing.TB Fatalf handle so tests see a
// diagnostic failure instead of silently hitting the retired *fiber.App.
// Directly asserting t.Fatalf inside a real *testing.T would abort the
// enclosing subtest, so these tests use a spy testing.TB that captures the
// Fatalf call and hands control back to the outer test for assertion —
// same trick the standard library uses in testing/internal/testdeps and
// the sibling gin / echo lifecycle_test.go files.
...
func (s *spyTB) Cleanup(fn func()) {
	...
	// *testing.T. The Fiber harness always runs its own Stop on the spy
	// at teardown when the outer test cleans up its resources.
	_ = fn
}
...
func TestSendAfterStopReportsThroughFatalf_Fiber(t *testing.T) {
	app := newApp()
	...
	// Warm exercise — the pre-Stop call must go through so we know Send()
	// is only rejecting the post-Stop path.
	...
	// Post-Stop Send must Fatalf. Run on a helper goroutine so the
	// spy's runtime.Goexit() does not tear down this test.
	runInGoroutine(func() {
		srv.Request(kiwa.MethodGET, "/ok").Send()
	})
	...
	if !strings.Contains(msg, "kiwa-fiber") {
		t.Fatalf("Fatalf message = %q, want kiwa-fiber prefix", msg)
	}
}

//  2. RequestCount does not advance across a post-Stop Send() attempt —
//     the recorder must not observe traffic that Send() rejected.
func TestRequestCountFrozenAfterStop_Fiber(t *testing.T) {
	app := newApp()
	app.Get("/ok", func(c *fiber.Ctx) error { return c.SendString("ok") })
...
func TestSendReportsNewRequestFailureThroughFatalf_Fiber(t *testing.T) {
	app := newApp()
	...
	// returns an error before we ever call into Fiber.
	badPath := "/ok\x7f"
	...
	msg := spy.LastFatal()
	if !strings.Contains(msg, "kiwa-fiber: build request") {
...
func TestRecorderFrozenAfterNewRequestFailure_Fiber(t *testing.T) {
	app := newApp()
...
func TestIsStoppedReflectsLifecycle_Fiber(t *testing.T) {
	app := newApp()
...
//  6. NewTestServer with a nil *fiber.App reports the misuse through
//     Fatalf so a test that forgets to construct the app does not panic
//     deep inside the harness.
func TestNewTestServerNilAppFatals_Fiber(t *testing.T) {
	spy := &spyTB{TB: t}

	runInGoroutine(func() {
		_ = kiwa_fiber.NewTestServer(spy, nil)
	})

	if !spy.DidFatal() {
		t.Fatalf("NewTestServer(nil) did not call Fatalf on the captured TB")
	}
	msg := spy.LastFatal()
	if !strings.Contains(msg, "kiwa-fiber: NewTestServer requires a non-nil *fiber.App") {
		t.Fatalf("Fatalf message = %q, want it to mention non-nil *fiber.App", msg)
	}
}
