...
// test failure instead of silently hitting the retired engine — matching
// the v1.4 mock_server contract where post-Stop traffic hits a closed
// port and fails at the transport layer.
...
// recorder captured. v1.6-2 (Issue #608) hazard 3 — the pre-v1.6-5
// implementation stored the caller slice by reference, so
// buf[i] = 'X' after .Body(buf) but before .Send() would corrupt the
// dispatched request.
...
// retroactively mutate the wire payload (v1.6-2 hazard 3). Returns a
// fresh []byte even for nil input so downstream code can treat the
// result as a valid slice without a nil check.
...
func (r *Request) Send() *Response {
	if r.server.IsStopped() {
		r.server.t.Helper()
		r.server.t.Fatalf(
			"kiwa-gin: Send() called after Stop() — lifecycle contract requires build → exercise → Stop, post-Stop traffic is a bug (request: %s %s)",
	...
	//
	// Request-to-Snapshot conversion is delegated to internal/recorder.FromClient
	// — the single SSOT shared with kiwa root and kiwa/echo (v1.6-5,
	// Issue #611). We pass the builder's body slice (not req.Body) so the
	// recorder captures the exact bytes dispatched even when the request
	// body reader is single-shot.
