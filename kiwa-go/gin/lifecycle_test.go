...
//
// Post-Stop Send() is a lifecycle contract violation and the gin adapter
// reports it through the captured testing.TB Fatalf handle so tests see a
// diagnostic failure instead of silently hitting the retired engine.
...
// same trick the standard library uses in testing/internal/testdeps.
...
// tests can assert on it. It also stops the calling goroutine via
// runtime.Goexit() to mirror *testing.T.Fatalf's control-flow (the real
// Fatalf calls runtime.Goexit on the test goroutine).
type spyTB struct {
	testing.TB
	fatal   atomic.Bool
	fatalMu sync.Mutex
...
func (s *spyTB) Cleanup(fn func()) {
	...
	// *testing.T. The gin harness always runs its own Stop on the spy at
...
func TestSendAfterStopReportsThroughFatalf_Gin(t *testing.T) {
	engine := newEngine()
	engine.GET("/ok", func(c *gin.Context) { c.String(http.StatusOK, "ok") })

	spy := &spyTB{TB: t}
	srv := kiwa_gin.NewTestServer(spy, engine)
...
func TestRequestCountFrozenAfterStop_Gin(t *testing.T) {
	engine := newEngine()
	engine.GET("/ok", func(c *gin.Context) { c.String(http.StatusOK, "ok") })
...
func TestSendReportsNewRequestFailureThroughFatalf_Gin(t *testing.T) {
	engine := newEngine()
	...
	// returns an error before we ever call into gin.
	badPath := "/ok\x7f"
	...
	msg := spy.LastFatal()
	if !strings.Contains(msg, "kiwa-gin: build request") {
...
func TestRecorderFrozenAfterNewRequestFailure_Gin(t *testing.T) {
	engine := newEngine()
...
func TestIsStoppedReflectsLifecycle_Gin(t *testing.T) {
	engine := newEngine()
