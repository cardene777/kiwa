...
```bash
pnpm exec kiwa layers --json --layer cli --lang "$DOC_LANG" --module "$MODULE" \
...
実測 (cwd = repo root、 cli layer / module `kiwa-cli` / `examples/cli-poc`)。 応答は下の検証表を全行 pass するが、 そのまま `spec_path` を開くと `No such file or directory` になる。
...
`--no-review` 未指定なら `/kiwa-review --mode test-review --module {module} --layer cli --test-path <解決した出力先> --lang $DOC_LANG` を内部呼出し、 5 軸判定。 `--test-path` には生成した path をそのまま渡す (既定は `tests/{module}.cli.test.ts`)。
