...
`createChartClient({ provider })` で client、 `renderChart({ type: 'bar', data })` で svg-like tree を取得、 series 数 / bar 数 / color 割当を assert。 4 provider を it.each で回す。

### Step 2: computeAxis test 生成

`computeAxis(data, { min: 0, max: 100 })` で tick / domain / scale 計算、 negative value / log scale / date axis の 3 case で分岐 cover。
