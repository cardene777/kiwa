...
description: |
  ...
  2 mode — `spec-review` (生成 spec の 11 観点網羅 / 優先度妥当性 / 不足観点を判定) / `test-review` (spec vs 実装 test の整合 / 観点別 cover 率 / 追加すべき test を提案)。
  単体起動 + 他 kiwa skill (kiwa-design / kiwa-forge / kiwa-hardhat / kiwa-play) の完了 step から自動呼出。 report は `tests/reports/review/` に Write。
user_invocable: true
...
- `--mode {spec-review|test-review}` — review mode (必須)
...
`--mode` 引数で 2 分岐。 spec-review / test-review いずれかを必ず実行 (mode 未指定時はエラー停止 + AskUserQuestion で確認)。
...
| `/kiwa-play` Step 5e 完了後 | `test-review` | Playwright spec 生成 + 4 round PASS 後 | 同上、 UI 起点 e2e 整合 |
...
- `references/doc-language-selection.md` — 文書生成言語選択 共通 SSOT (4 kiwa skill 共用、 symlink で参照)
