...
- 対応 test file を Glob で特定:
  ...
  - Auth session / provider / DB adapter mock (`--layer auth`、 Issue #642 v1.8): `{example}/tests/*.auth.test.ts` or `packages/auth/tests/*.test.ts` (`@kiwa-test/auth` の NextAuth v5 `setupNextAuthEnv` / Lucia v3 `setupLuciaEnv` / Better Auth `setupBetterAuthEnv` factory を statement-level に mapping、 session mock + OAuth provider mock + email/password + magic link + 2FA + passkey + organizations の 6 sub-feature を 1 file 内で cover)
  - Job queue / event queue (`--layer job-queue`、 Issue #642 v1.8): `{example}/tests/*.queue.test.ts` or `packages/queue/tests/*.test.ts` (`@kiwa-test/queue` の BullMQ `setupBullMQEnv` (sandbox / testcontainers) + Inngest `setupInngestEnv` (stub / dev-server) 2 backend を statement-level に mapping、 job add / process / retry / fail / drain / delay + event send / step function / concurrency の 8 sub-feature を 1 file 内で cover)
  - Redis cache (`--layer cache`、 Issue #642 v1.8): `{example}/tests/*.cache.test.ts` or `packages/cache/tests/*.test.ts` (`@kiwa-test/cache` の `setupCacheEnv` (in-memory / testcontainers) + ioredis / node-redis 2 client を statement-level に mapping、 get / set / delete / TTL / expiry / Pub/Sub / publish-subscribe / assertPublished の 8 sub-feature を 1 file 内で cover)
