...
- `--layer {contract|e2e|e2e-generic|a11y|visual|api|ui|data|cli|integration|unit|all}` — spec layer (default `all`、 spec path 解決に使用)。 `e2e` は dApp e2e (`tests/spec/e2e/test-spec-{module}.md`、 `/kiwa-play` 出力)、 `e2e-generic` は汎用 browser e2e (`tests/spec/integration/test-spec-{module}.e2e.md`、 `/kiwa-e2e` 出力)、 `a11y` は accessibility (`tests/spec/integration/test-spec-{module}.a11y.md`、 `/kiwa-a11y` 出力)、 `visual` は visual regression (`tests/spec/integration/test-spec-{module}.visual.md`、 `/kiwa-visual` 出力)。 spec path 規約は `/kiwa-design § 出力 path の決定` SSOT と一致、 spec-review / test-review / result-review 3 mode は全 layer に対応
