...
- observe dashboard を統合 report の Section 2 に載っている分だけ Read。 **file 名を組み立てない** = 名前は `/kiwa-test` Step 5a が `--out` で決めており、 写しを持つと writer が軸を足した時にずれる (#2077 で `{module}` が足された時に実際にずれた)。 `/kiwa-test` Step 5a が layer ごとに書く。 **Section 2 が「observe 失敗」 や「observe skip」 と書いている行は読まない** = file が無い。 dashboard が 1 枚も無い場合も軸 3 (flaky 兆候) は従来どおり実行結果数値から判定する
...
**具体値であることと、識別力を持つことは別**。 具体値でも壊れた実装を検知できない形がある。

両軸の score を付ける時、次を追加で問う。

> この期待結果 / assertion は、実装の該当箇所を 1 行壊した時に成立しなくなるか。

「いいえ」 なら、具体値で書かれていても減点する。 本 repo で実測した 3 形。

| 形 | 例 | なぜ効かないか |
|---|---|---|
| 常に成立する値 | `status===200` | route へ到達すれば検証失敗も状態不整合も 200 |
| 実測より緩い件数 | `>= 1` と書くが実測は常に 1 | 2 件に変わっても別種別に変わっても通る |
| 両経路が同じ結果 | 「throw する」 だけを見る | 正しい場合も壊れた場合も throw する |

**「〜されないこと」 を見る TC が並ぶ spec では、対照の TC があるかも見る**。
処理そのものが動いていなくても全件通る形になるため。

判定基準と実測値の全文は
`.claude/skills/kiwa-design/references/assertion-discrimination.md` が SSOT。
ここには要約を書かない = 要約が詳細を弱める。 review 実行時は同 file を Read する。
