...
name: kiwa-queue
description: |
  /kiwa-design (Layer 1) が出力した `tests/spec/integration/test-spec-{module}.queue.md` を入力に、 `@kiwa-lab/queue` を使う `test/*.queue.test.ts` を Write して `vitest` で動作確認する Layer 2 queue test skill。
...
`@kiwa-lab/queue` v0.1 (v1.8-4〜v1.8-5、 Issue #640 / #641) の 2 factory (`setupBullMQEnv` / `setupInngestEnv`) を Layer 1 spec の観点別 TC 表から自動的に選択し、 sandbox / testcontainers / stub / dev-server の 4 backend mode を TC ごとに割当てる。

## 前提

- `@kiwa-lab/queue` v0.1.0+ が devDependency に入っている (`pnpm add -D @kiwa-lab/queue`)
...
`--spec-path` を省略した時、 **自前で組み立てず `kiwa layers` に訊く**。 本 skill が扱う layer は `job-queue` の 1 つ。

```bash
pnpm exec kiwa layers --json --layer job-queue --lang "$DOC_LANG" --module "$MODULE" \
...
`--provider` と `--mode` は spec の中身の選択で、 path には影響しない。 4 provider は 1 つの `job-queue` layer が持つ選択肢で、 spec は provider ごとに分かれない。
...
`--no-review` 未指定なら `/kiwa-review --mode test-review --layer job-queue --module {module} --lang $DOC_LANG --producer kiwa-queue --project-root .` を chain 呼出。 `--producer` と `--project-root` は review 側が test file を `kiwa layers` に訊くために要る (#1902)。
...
- 遅い test の上位を確認済 = `/kiwa-observe` の dashboard `Execution time` section (または runner の実行時間出力) を読み、遅い test に対処したか、対処しない理由 (testcontainers / dev-server 等の本質的な遅さ) を report に記録 (#2186)
...
- `@kiwa-lab/queue` v0.1 (v1.8-4〜v1.8-5、 Issue #640 / #641) SSOT
