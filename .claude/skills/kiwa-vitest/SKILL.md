...
contract / real-chain と連携した動作を verify したい case (例 storage slot 直書き / time-warp / EIP-1271 / 実 RPC 経路) は `@kiwa-test/dapp` の `setupTestEnv` helper で **実 anvil 経路** に opt-in する。
