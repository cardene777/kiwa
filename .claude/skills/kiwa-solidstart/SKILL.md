...
description: |
  Layer 1 spec (`tests/spec/integration/test-spec-{module}.solidstart.md` / `.solidstart-api.md`) を SolidStart の Server Functions + API Routes test (Vitest + @kiwa-test/solidstart) に変換する Layer 2 skill。
  ...
  `/kiwa-design --layer solidstart-server-function` / `--layer solidstart-api-route` が出力する 9 column 表を `@kiwa-test/solidstart` の API に機械的に変換する。
user_invocable: true
...
`/kiwa-design --layer solidstart-server-function` / `--layer solidstart-api-route` が出力した 9 column 表を、 `@kiwa-test/solidstart` v1.0+ の `invokeServerFunction` / `invokeApiRoute` を使った Vitest test に機械変換する。
...
```ts
// server-function
import { invokeServerFunction, redirect, SOLIDSTART_REDIRECT_SYMBOL } from '@kiwa-test/solidstart';
...
// api-route
import { invokeApiRoute, json, redirectResponse } from '@kiwa-test/solidstart';
...
- runtime fixture ... `@kiwa-test/solidstart` v1.0+ (`packages/solidstart/`)
