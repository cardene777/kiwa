...
name: kiwa-e2e
description: |
  Layer 1 spec (`tests/spec/integration/test-spec-{module}.e2e.md`) を汎用 browser E2E test (Playwright + @kiwa-test/e2e) に変換する Layer 2 E2E test skill。
  dApp E2E (kiwa-play) と区別される非 web3 文脈の汎用 browser e2e (static html / fetch app / SSR app) を担当する。
  `/kiwa-design --layer e2e` が出力する 9 column 表を `@kiwa-test/e2e` の `setupE2eEnv` の引数に機械的に変換する。
user_invocable: true
...
- 対象 example に `package.json` があり、 `@playwright/test` + `@kiwa-test/e2e` + `@kiwa-test/core` が devDependencies で利用可能 (未インストールなら install を強制)
...
| spec column | Playwright + @kiwa-test/e2e への変換 |
|---|---|
...
- `@kiwa-test/e2e` 公式 API ... `packages/e2e/README.md`
