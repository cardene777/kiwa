...
name: kiwa-astro
description: |
  Layer 1 spec (`tests/spec/integration/test-spec-{module}.{astro|astro-ssr|astro-vt}.md`) を Astro の 3 mode (Server Endpoints `pages/api/*.ts` の `GET` / `POST` 等 function exports + `.astro` page SSR + View Transitions API lifecycle event) test (Vitest + @kiwa-lab/astro) に変換する Layer 2 skill。
  ...
  `/kiwa-design --layer astro-endpoint` / `--layer astro-ssr` / `--layer astro-view-transitions` が出力する 9 column 表を `@kiwa-lab/astro` v1.1+ の `invokeEndpoint` / `renderAstroPage` / `setupAstroViewTransitionEnv` の引数に機械的に変換する。
user_invocable: true
...
`/kiwa-design --layer astro-endpoint` が出力した 9 column 表を、 `@kiwa-lab/astro` v1.0+ の `invokeEndpoint` を使った Vitest test に機械変換する。
...
```ts
import { invokeEndpoint, type APIRoute } from '@kiwa-lab/astro';
...
```ts
import { renderAstroPage, kiwaAstroNotFound, ASTRO_REDIRECT_SYMBOL, ASTRO_NOT_FOUND_SYMBOL } from '@kiwa-lab/astro';
...
- runtime fixture ... `@kiwa-lab/astro` v1.1+ (`packages/astro/`)
