...
name: kiwa-qwikcity
description: |
  Layer 1 spec (`tests/spec/integration/test-spec-{module}.qwik.md` / `.qwik-action.md` / `.qwik-endpoint.md`) を Qwik City の routeAction$ + routeLoader$ + Endpoints test (Vitest + @kiwa/qwikcity) に変換する Layer 2 skill。
  ...
  `/kiwa-design --layer qwikcity-action` / `--layer qwikcity-loader` / `--layer qwikcity-endpoint` が出力する 9 column 表を `@kiwa/qwikcity` の API に機械的に変換する。
user_invocable: true
...
`/kiwa-design --layer qwikcity-action` / `--layer qwikcity-loader` / `--layer qwikcity-endpoint` が出力した 9 column 表を、 `@kiwa/qwikcity` v1.0+ の `invokeRouteAction` / `invokeRouteLoader` / `invokeEndpoint` を使った Vitest test に機械変換する。
...
```ts
// routeAction
import { invokeRouteAction, QWIK_FAIL_SYMBOL, QWIK_REDIRECT_SYMBOL } from '@kiwa/qwikcity';
...
- runtime fixture ... `@kiwa/qwikcity` v1.0+ (`packages/qwikcity/`)
