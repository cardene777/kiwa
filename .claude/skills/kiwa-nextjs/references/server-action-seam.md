...
`@kiwa-test/nextjs` works around this by requiring the Server Action under test to accept its environment via an **injectable seam** — a parameter or a module-level setter that defaults to the real Next.js bindings in production but can be replaced in tests.
...
import { REDIRECT_SYMBOL } from '@kiwa-test/nextjs';
