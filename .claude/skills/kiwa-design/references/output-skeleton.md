...
- 候補 column は repo root 相対の `file:行番号` + test 名を **そのまま** 書く (要約 / 意訳しない、 `packages/{name}/` / `examples/{name}/` prefix を落とさない、 人が 1 手で開いて確かめる)
...
- `TC` column の ID は **§ テストケース一覧 と同じ形式をそのまま使う**。 layer 別 9 column 表は ID 形式が違う (`TC-001` / `T-UI-001` / `T-API-001` / `T-DATA-001` / `T-CLI-001`)。 下の例は `TC-001` 形式だが、 `--layer ui` なら `T-UI-001` と書く
