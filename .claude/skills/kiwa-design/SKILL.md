...
| `auth` | `tests/spec/integration/test-spec-{module}.auth.md` | `/kiwa-auth` (auth session / provider / DB adapter mock 専用、 `@kiwa-test/auth` の NextAuth v5 / Lucia v3 / Better Auth 3 backend を statement-level に mapping、 session mock + OAuth provider mock + email/password + magic link + 2FA + passkey + organizations の 6 sub-feature を 1 spec 内で cover、 v1.8-1〜v1.8-3 で `@kiwa-test/auth` 追加、 v1.8-6 で本 layer 追加) |
| `job-queue` | `tests/spec/integration/test-spec-{module}.queue.md` | `/kiwa-queue` (job queue / event queue 専用、 `@kiwa-test/queue` の BullMQ (Redis-backed、 sandbox / testcontainers) + Inngest (event-driven、 stub / dev-server) 2 backend を statement-level に mapping、 job add / process / retry / fail / drain / delay + event send / step function / concurrency の 8 sub-feature を 1 spec 内で cover、 v1.8-4〜v1.8-5 で `@kiwa-test/queue` 追加、 v1.8-6 で本 layer 追加) |
| `cache` | `tests/spec/integration/test-spec-{module}.cache.md` | `/kiwa-cache` (Redis cache 専用、 `@kiwa-test/cache` の in-memory / testcontainers 2 backend + ioredis / node-redis 2 client を statement-level に mapping、 get / set / delete / TTL / expiry / Pub/Sub / publish-subscribe / assertPublished の 8 sub-feature を 1 spec 内で cover、 v1.8-6 で本 layer + `@kiwa-test/cache` v0.1 同時追加) |
