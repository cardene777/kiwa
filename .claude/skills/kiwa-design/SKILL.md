...
`期待結果` は **実装を壊した時に値が変わるものを名指しする**。 書いた後に自問する。

> この実装の該当箇所を 1 行壊したら、この期待結果は成立しなくなるか。

「はい」 でなければ観測する対象を変える。 `status===200` のように常に成立する値、
実測が定まるのに `>= 1` と緩く書く形、両経路が同じ例外を投げるのに「throw する」 だけを
見る形は、いずれも実行はされるが何も守らない。

実測では、行カバレッジを 94.74% → 98.09% に上げても生存変異が 36 件のまま
1 件も減らなかった (`packages/core/src/temp.ts`)。

**規範と 3 anti-pattern の全文は `references/assertion-discrimination.md` が SSOT**。
ここには要約を書かない = 要約が詳細を弱める。 Step 4 を実行する時は同 file を Read する。
...
report 出力先: `/kiwa-review` Step 0 の言語別出力 path (`spec-review` mode)
