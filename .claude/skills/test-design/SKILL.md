...
`--layer` に応じて出力 path を分岐する。 layer 別に dir を分けることで Layer 2 skill (`/contract-test-foundry` / `/contract-test-hardhat` / `/dapp-e2e-test`) が対象 layer の spec だけを Read できる。

| `--layer` | 出力 path | 主要消費 Layer 2 skill |
|---|---|---|
| `contract` | `.context/spec/contract/test-spec-{module}.md` | `/contract-test-foundry` / `/contract-test-hardhat` |
| `e2e` | `.context/spec/e2e/test-spec-{module}.md` | `/dapp-e2e-test` |
...
|---|---|---|---|
| `/contract-test-foundry` | `.context/spec/contract/test-spec-{module}.md` | `test/*.t.sol`、 `forge test` 実行 | 境界値 = `forge fuzz` / 状態遷移 = `forge invariant` |
| `/contract-test-hardhat` | `.context/spec/contract/test-spec-{module}.md` | `test/*.test.ts`、 `npx hardhat test` 実行 | 境界値 = `fast-check` / 並行処理 = `Promise.all` race |
| `/dapp-e2e-test` (refactored) | `.context/spec/e2e/test-spec-{module}.md` | `tests/*.spec.ts` + `tests/prepare-env.ts` | 正常系 = happy path / セキュリティ = signature 検証 |
