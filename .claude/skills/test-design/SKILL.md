...
description: |
  ...
  contract (Foundry / Hardhat) と dApp e2e (Playwright + dapp-e2e fixture) で共通の入口になり、 出力は `.context/spec/test-spec-{module}.md` に保存。
  Layer 2 skill (`/contract-test-foundry` / `/contract-test-hardhat` / `/dapp-e2e-test`) が本 skill の出力を実 `.t.sol` / `.test.ts` / `.spec.ts` に変換する。
user_invocable: true
...
# /test-design — Layer 1 テスト設計 skill

`docs/SKILL-DESIGN.md` の SSOT に従い、 1 回の起動で 5 段階フロー (入力整理 → 品質リスク → テスト観点 → ケース生成 → 優先度 + 自動化) を完走し、 9 section 統一テンプレで仕様書を Write する。
...
- `--module {name}` — 出力 file 名を `.context/spec/test-spec-{name}.md` に固定
- `--layer {contract|integration|e2e|all}` — 想定 test layer を指定 (default `all`、 case のテストレベル割当が変わる)
- `--input {path}` — 機能仕様 file の path (省略時は対話形式で要約を求める)
- `--no-examples` — examples/ サンプル参照をスキップ (skill 内部の参照のみで仕様書を生成)

## 実行フロー

5 段階を順に通る。 各 step は対応する section を `.context/spec/test-spec-{module}.md` に append する。 飛ばし / 順序入れ替えは禁止 (`docs/SKILL-DESIGN.md` SSOT に従う)。
...
各ケースは統一 9 column 表の 1 行。 ID は `TC-001` から連番、 観点ごとにグループ化し、 グループ内は優先度 (高 → 中 → 低) 順に並べる。
...
優先度は Step 2 のリスク要約から導出 (skill が勝手に判定しない):
...
`.context/spec/test-spec-{module}.md` を以下 9 section で Write する (順序固定、 省略禁止)。 完全な雛形は `references/output-skeleton.md` を Read する。
...
Layer 1 出力 (`.context/spec/test-spec-{module}.md`) を Layer 2 skill が消費する経路と引き渡し方は `references/layer2-bridge.md` を Read する。

| Layer 2 skill | 変換先 | 推奨観点 |
|---|---|---|
| `/contract-test-foundry` | `test/*.t.sol`、 `forge test` 実行 | 境界値 = `forge fuzz` / 状態遷移 = `forge invariant` |
| `/contract-test-hardhat` | `test/*.test.ts`、 `npx hardhat test` 実行 | 境界値 = `fast-check` / 並行処理 = `Promise.all` race |
| `/dapp-e2e-test` (refactored) | `tests/*.spec.ts` + `tests/prepare-env.ts` | 正常系 = happy path / セキュリティ = signature 検証 |

Layer 2 skill は仕様書の「テストケース一覧」表を行単位で読み取り、 観点 → ランナー特化 helper に変換する。

## 完了条件

- `.context/spec/test-spec-{module}.md` が 9 section 全て揃って Write 済 (空 section は `(なし)`)
- 「テストケース一覧」が 1 ケース 1 行で観点別グループ化されている
- 優先度判定が Step 5 のロジック (リスク 5 基準) と整合している
- 「不足している仕様」が空でなければ追加ヒアリングが必要な旨を末尾で報告
- Layer 2 連携先 skill を末尾で 1 件以上推奨 (contract / e2e / 両方のいずれか)
...
- 既存 e2e skill (Layer 2 候補): `.claude/skills/dapp-e2e-test/SKILL.md`
- 偽陽性 self-check: `.claude/skills/dapp-e2e-test/references/adversarial-pitfalls.md`
