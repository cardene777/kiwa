...
|---|---|---|
| `/dapp-e2e-test` (refactored) | `tests/*.spec.ts` + `tests/prepare-env.ts` (Playwright) | Phase E-3 で refactor 予定 |
| `/contract-test-foundry` | `test/*.t.sol` (`forge test` 実行) | Phase E-4 で新規追加予定 |
| `/contract-test-hardhat` | `test/*.test.ts` (`npx hardhat test` 実行) | Phase E-5 で新規追加予定 |

現状 (Phase E-2 時点) では `/dapp-e2e-test` が既存の `Step 1.5` 経路で Layer 1 出力を任意で消費できる。 厳密な Layer 2 統合は Phase E-3 以降で対応。

## 引き渡し flow

```mermaid
graph LR
    A[/test-design 起動] --> B[5 段階フロー]
    B --> C[.context/spec/test-spec-X.md]
    C --> D{Layer 2 選択}
    D -->|contract| E[/contract-test-foundry]
    D -->|contract| F[/contract-test-hardhat]
    D -->|e2e| G[/dapp-e2e-test]
    E --> H[*.t.sol Write]
    F --> I[*.test.ts Write]
    G --> J[*.spec.ts Write]
```
...
- 9 column 表 (`テスト ID | テストレベル | 前提条件 | 入力値 | 操作手順 | 期待結果 | 優先度 | 自動化`)
...
| 異常系 | `await expect(...).to.be.revertedWithCustomError(c, 'NotOwner')` |
| 境界値 | `fc.assert(fc.property(fc.bigUintN(256), async (x) => { ... }))` |
| 状態遷移 | `before/after` で state seed + `it.each(states)` |
| 権限 | `await c.connect(admin).pause()` |
| 入力バリデーション | `fc.string()` + revert assertion |
| 冪等性 | 2 回 call して 2 回目 revert |
| 並行処理 | `Promise.all([tx1, tx2])` race |
| 性能 | `hardhat-gas-reporter` |
| セキュリティ | `ethers.getContractAt + verify` |
...
```mermaid
graph TD
    A[Phase E-2 = 本 PR] -->|Layer 1 確立| B[/test-design 単独で利用可]
    B --> C[Phase E-3 = /dapp-e2e-test refactor]
    C --> D[Step 1.5 を /test-design に置換]
    D --> E[Phase E-4 = /contract-test-foundry 新規]
    E --> F[Phase E-5 = /contract-test-hardhat 新規]
