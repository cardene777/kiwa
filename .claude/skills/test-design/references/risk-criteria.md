...
skill は以下ロジックで優先度を機械的に判定する。 主観で上書きしない。 SSOT (`docs/SKILL-DESIGN.ja.md` § Step 5) の判定条件と完全一致させる (前提条件を追加しない、 上位優先の fall-through 規約に従う)。

| 優先度 | 条件 |
|---|---|
| 高 | 売上 / セキュリティ / データ破壊 のいずれかが「高」 |
| 中 | 利用頻度 / 過去障害 のいずれかが「高」 (「高」が同時に満たされる場合は 高 を優先) |
| 低 | 全基準「低」 |

判定は **上から順** に評価し、 該当した時点で確定する (fall-through で「高」を取りこぼさないため)。 例: 「セキュリティ = 高 + 利用頻度 = 高」の場合、 高 を採用 (中・低 を評価しない)。
