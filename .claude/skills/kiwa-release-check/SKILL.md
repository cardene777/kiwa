...
description: |
  kiwa の release レベル判定を 4 gate + release-smoke で一括 check する project-local skill。
  Gate 1 (coverage) / Gate 3 (taxonomy CLI) / release-smoke / build-check を統合実行して kiwa が
  release 可能な状態か判定、 全 gate pass で「✅ RELEASE READY」 と判定する。
  内部で `scripts/release-readiness-check.mjs` を呼出、 test check tool (test:cov / taxonomy CLI /
  release-smoke) をすべて含めて read する。
...
# /kiwa-release-check — kiwa release レベル 4 gate 一括判定 skill

user 要求 「リリースできるレベルをチェックするもの、 テストチェックツールもその中で読んで欲しい」
に対応する project-local skill。 4 gate (coverage / taxonomy / release-smoke / build) を統合実行して
kiwa が release 可能な状態か判定する。
...
# quick verify (Gate 1 のみ、 30ms 相当)
/kiwa-release-check --skip=gate3-taxonomy,release-smoke,build-check

# release-smoke skip (Gate 1 + Gate 3 のみ)
/kiwa-release-check --skip=release-smoke
```
