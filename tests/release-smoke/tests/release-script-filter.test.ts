...
// after the fact: every publishable `@kiwa-test/*` package must appear in
...
//
// This axis discovers `@kiwa-test/*` packages dynamically from `packages/*/package.json`
...
//   1. Add the missing `-F @kiwa-test/{name}` + `--filter @kiwa-test/{name}` pair to
...
// Keep empty unless there is a hard reason a `@kiwa-test/*` name cannot appear
...
const NON_PUBLISHED_ALLOWLIST: readonly string[] = [
  // (empty — all @kiwa-test/* packages currently publish through the standard filter)
];
...
function discoverPublishablePackages(): string[] {
  ...
  for (const dir of dirs) {
    ...
    if (!pkg.name || !pkg.name.startsWith('@kiwa-test/')) continue;
...
const buildFilters = extractFilters(release, /-F\s+(@kiwa-test\/[a-z0-9-]+)/g);
const publishFilters = extractFilters(release, /--filter\s+(@kiwa-test\/[a-z0-9-]+)/g);
...
describe('release script filter — systematic root cause pattern SSOT (Issue #986)', () => {
  ...
  it('discovers publishable @kiwa-test/* packages from packages/*/package.json', () => {
  ...
  it('every allowlisted name is a real @kiwa-test/* package (guard against stale entries)', () => {
