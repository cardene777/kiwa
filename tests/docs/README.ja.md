...
- ⚒️ [run-contract-tests.ja.md](./run-contract-tests.ja.md) — 自分の dApp dir で `claude` を起動し、 `/kiwa-design` で test 仕様書を生成 → `/kiwa-forge` / `/kiwa-hardhat` で contract test を生成 → 実走する 3 skill chain 手順 (mint-nft 題材、 完成形 fixtures との diff 比較込み)
- 🎭 [run-dapp-e2e-tests.ja.md](./run-dapp-e2e-tests.ja.md) — 自分の dApp dir で `claude` を起動し、 `/kiwa-design` で test 仕様書を生成 → `/kiwa-play` で dApp e2e test を生成 → 実走する 2 skill chain 手順 (mint-nft 題材、 同 diff 比較込み)
