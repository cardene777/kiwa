...
- ⚒️ [run-contract-unit-tests.ja.md](./run-contract-unit-tests.ja.md) — `contracts/` 配下の **複数 contract を一括** で `/kiwa-design` → `/kiwa-forge` / `/kiwa-hardhat` で unit test を生成 → 実走する手順 (nft-marketplace 2 contract を題材、 単一 contract も同 flow)
- 🔗 [run-contract-integration-tests.ja.md](./run-contract-integration-tests.ja.md) — 複数 contract の **連携 scenario** (state 遷移 / 失敗 rollback / 順序依存 等) を `/kiwa-design --layer integration` で test 仕様書化 → `/kiwa-forge` / `/kiwa-hardhat` で integration test を生成 → 実走する手順
