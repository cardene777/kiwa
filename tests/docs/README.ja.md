...
- ⚒️ [run-contract-tests.ja.md](./run-contract-tests.ja.md) — kiwa の skill chain (`/kiwa-design` → `/kiwa-forge` / `/kiwa-hardhat`) で contract test を **0 から作って実走** する手順 (mint-nft 題材、 完成形 fixtures との diff 比較込み)
- 🎭 [run-dapp-e2e-tests.ja.md](./run-dapp-e2e-tests.ja.md) — kiwa の skill chain (`/kiwa-design` → `/kiwa-play`) で dApp e2e test を **0 から作って実走** する手順 (mint-nft 題材、 同 diff 比較込み)
