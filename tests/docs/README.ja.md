...
- ⚒️ [run-contract-tests.ja.md](./run-contract-tests.ja.md) — 自分の dApp dir で `claude` を起動し、 `/kiwa-forge` / `/kiwa-hardhat` に **既存の contract + 仕様書を読み込ませて** contract test を 0 から作って実走する手順 (mint-nft 題材、 完成形 fixtures との diff 比較込み)
- 🎭 [run-dapp-e2e-tests.ja.md](./run-dapp-e2e-tests.ja.md) — 自分の dApp dir で `claude` を起動し、 `/kiwa-play` に **既存の contract + UI + 仕様書を読み込ませて** dApp e2e test を 0 から作って実走する手順 (mint-nft 題材、 同 diff 比較込み)
