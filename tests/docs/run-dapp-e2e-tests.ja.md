...
```bash
pnpm install && pnpm -F @kiwa-test/dapp build && anvil --version && node --version && pnpm --filter examples-nextjs-token-gating exec playwright install chromium
```
...
| `Cannot find module '@kiwa-test/dapp'` | repo root で `pnpm -F @kiwa-test/dapp build` |
...
- `@kiwa-test/dapp` fixture 仕様: `packages/dapp/src/fixture.ts`
