...
- ✍️ [write-tests-manually.md](./write-tests-manually.md) — **Hand-write tests by importing `@kiwa-lab/dapp` as a library instead of using skills** (retrofitting tests into an existing dApp, reusing only fixtures, swapping only some helpers over to kiwa, etc.). Includes four single-file samples (mint / marketplace / snapshot / custom error)
...
| `/kiwa-play` | Layer 3 e2e | Design / implement / run Playwright `tests/*.spec.ts` on top of the `@kiwa-lab/dapp` fixture | `.claude/skills/kiwa-play/SKILL.md` |
...
- [docs/en/quickstart.md](../../docs/en/quickstart.md) — OSS user introduction (`@kiwa-lab/cli init` for a brand-new dApp)
