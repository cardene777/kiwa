...
```bash
pnpm install && pnpm -F @kiwa-lab/dapp build && anvil --version && node --version && pnpm --filter examples-nextjs-token-gating exec playwright install chromium
```
...
| `Cannot find module '@kiwa-lab/dapp'` | Run `pnpm -F @kiwa-lab/dapp build` at the repo root |
...
- `@kiwa-lab/dapp` fixture spec: `packages/dapp/src/fixture.ts`
