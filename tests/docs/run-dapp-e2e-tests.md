...
```bash
pnpm install && pnpm -F @kiwa/dapp build && anvil --version && node --version && pnpm --filter examples-nextjs-token-gating exec playwright install chromium
```
...
| `Cannot find module '@kiwa/dapp'` | Run `pnpm -F @kiwa/dapp build` at the repo root |
...
- `@kiwa/dapp` fixture spec: `packages/dapp/src/fixture.ts`
