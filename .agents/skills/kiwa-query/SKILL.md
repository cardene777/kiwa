...
`mutate(client, (body) => api.post('/user', body), body, { invalidateKeys: [['users']] })` で mutation → 指定 query の invalidate → 次 fetch で refetch の verify。mutation が reject した場合は invalidate されないことも verify する。
...
```text
/kiwa:kiwa-query --module users --output tests/integration/users.query.test.ts
/kiwa:kiwa-query --module posts --provider tanstack
```
