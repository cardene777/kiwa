...
name: kiwa-sveltekit
description: |
  Layer 1 spec (`tests/spec/integration/test-spec-{module}.svk.md` / `.svk-action.md` / `.svk-hooks.md`) を SvelteKit の load function + form actions + hooks.server.ts (handle / handleFetch / handleError) test (Vitest + @kiwa-lab/sveltekit) に変換する Layer 2 skill。
  ...
  `/kiwa-design --layer sveltekit-load` / `--layer sveltekit-action` / `--layer sveltekit-handle` / `--layer sveltekit-handle-fetch` / `--layer sveltekit-handle-error` が出力する 9 column 表を `@kiwa-lab/sveltekit` の API に機械的に変換する。
user_invocable: true
...
`/kiwa-design --layer sveltekit-load` / `--layer sveltekit-action` が出力した 9 column 表を、 `@kiwa-lab/sveltekit` v1.0+ の `invokeLoad` / `invokeAction` を使った Vitest test に機械変換する。
...
- Layer 1 spec が存在
- `@kiwa-lab/sveltekit` v1.0+ install 済
...
```ts
// load
import { invokeLoad, SK_REDIRECT_SYMBOL } from '@kiwa-lab/sveltekit';
...
// action
import { invokeAction, SK_FAIL_SYMBOL } from '@kiwa-lab/sveltekit';
...
```ts
import { invokeHandle, invokeHandleFetch, invokeHandleError } from '@kiwa-lab/sveltekit';
...
import { setupSvelteKitHooksEnv, sequence } from '@kiwa-lab/sveltekit';
...
- runtime fixture ... `@kiwa-lab/sveltekit` v1.1+ (`packages/sveltekit/`、 v1.0.1 は単発 invoke、 v1.1+ は `setupSvelteKitHooksEnv` + `sequence` chain 対応)
