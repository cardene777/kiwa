...
Step 5d で test-passed marker 作成成功後、 生成 test の品質を独立 review する。 `/kiwa-review --mode test-review --module {module} --test-path test/*.t.sol` を内部呼出し、 spec vs test 整合 / 観点別 cover 率 / 追加 test 提案 を 5 軸で判定。
...
- Layer 1 skill: `.Codex/skills/kiwa-design/SKILL.md` (`--layer contract` で本 skill 用 spec を生成)
- 並立 Layer 2: `.Codex/skills/kiwa-hardhat/SKILL.md` (Hardhat 用)、 `.Codex/skills/kiwa-play/SKILL.md` (Playwright e2e 用)
