...
description: |
  ...
  `/kiwa-design --layer {nuxt-server-route|nuxt-route-middleware|nuxt-nitro-plugin}` が出力する 9 column 表を `@kiwa-lab/nuxt` v1.0.3+ の対応 helper の引数に機械的に変換する。
user_invocable: true
...
`/kiwa-design --layer {nuxt-server-route|nuxt-route-middleware|nuxt-nitro-plugin}` が出力した spec の 9 column 表を、 `@kiwa-lab/nuxt` v1.0.3+ の `invokeEventHandler` / `invokeRouteMiddleware` / `invokeNitroPlugin` を使った Vitest test に機械変換する。
...
- `@kiwa-lab/nuxt` v1.0.3+ が install 済 (`pnpm add -D @kiwa-lab/nuxt`)
...
```ts
import { invokeRouteMiddleware, NUXT_MIDDLEWARE_REDIRECT_SYMBOL, NUXT_MIDDLEWARE_ABORT_SYMBOL } from '@kiwa-lab/nuxt';
...
- runtime fixture ... `@kiwa-lab/nuxt` v1.0.3+ (`packages/nuxt/`)
