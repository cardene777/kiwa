...
> Layer 1 (`/kiwa-design`) 出力サンプル — `examples/basic-connect/` の wallet connect dApp を題材にした最小サイズの仕様書 (SSOT `docs/SKILL-DESIGN.ja.md` 準拠)

## 対象機能

`basic-connect` — kiwa fixture が inject する `window.ethereum` を経由して、 mini HTML dApp (inline で `page.setContent` 注入) の Connect / Sign / SignTypedData / SendTransaction / RegisterEvent ボタンを動作させる最小 example。
...
- Sign → `personal_sign("hello kiwa")` で signature を取得し `#result` に表示
...
- anvil RPC (`http://127.0.0.1:{port}`、 kiwa fixture が自動起動)
- `window.ethereum` (kiwa fixture が inject、 EIP-1193 互換 + EIP-6963 announce)
...
| テスト ID | テストレベル | テスト観点 | 前提条件 | 入力値 | 操作手順 | 期待結果 | 優先度 | 自動化 |
|---|---|---|---|---|---|---|---|---|
| TC-004 | E2E | セキュリティ | TC-002 完了 + fixed PRIVATE_KEY | message = "hello kiwa" | `#sign` click → `viem.verifyMessage({address, message, signature})` | signature が `0x{130 hex}` 形式 + `verifyMessage === true` (identity 立証 OK) | 高 | 推奨 |
