...
describe('{Component} a11y', () => {
  it('passes WCAG 2.1 AA', async () => {
    ...
    const results = await runAxe({ context: container, runOptions: { runOnly: ['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'] } });
...
test('{Module} a11y check', async ({ page }) => {
  ...
  // この page へ axe-core を注入する処理は project 側で用意する。
  // @kiwa-lab/a11y は Playwright dependency や browser injection を持たない。
  const results = await page.evaluate(async () => {
    const axe = (window as typeof window & { axe: { run: (context: Document, options: unknown) => Promise<unknown> } }).axe;
    if (!axe) throw new Error('axe-core is not injected into this Playwright page.');
    return axe.run(document, { runOnly: ['wcag2aa'] });
  });
  const report = await runLayerHarness('{Module}', { playwright: { results } });
...
`expectNoViolations` の既定は `minor` 以上を fail させる。release gate を serious 以上にする場合は `expectNoViolations(results, expect, { maxImpact: 'serious' })` を明示する。
