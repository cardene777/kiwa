...
Real-vs-mock behavioural fidelity for the trace flame graph explorer dogfood, produced by `examples/dogfood-trace-flame-graph/tests/emit-fidelity-report.test.ts`. Feeds `@kiwa-test/quality-metrics` 7-axis release gate.
...
```
provider   : @kiwa-test/observability/trace-flame
version    : 2.0.0
verdict    : PASS
