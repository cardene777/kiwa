...
Package under exercise: [`@kiwa-lab/streaming`](../../../packages/streaming/) (v0.3.0).
...
5. KIP-98 raw protocol — `initProducerId` + `fenceProducer` (epoch bump) + txn coordinator state machine walk (`Empty` → `Ongoing` → `PrepareCommit` → `CompleteCommit` → `Empty`) + incremental fetch session (KIP-227). Exercised through the `driveRawProtocol` adapter op wired to `createKafkaRawProtocol` from `@kiwa-lab/streaming` v0.3.
