...
Real-vs-mock behavioural fidelity for the Anthropic Claude MCP tool-use agent dogfood, produced by `examples/dogfood-mcp-tool-agent/tests/emit-fidelity-report.test.ts`. Feeds `@kiwa-test/quality-metrics` 11-axis release gate.
...
```
provider   : @kiwa-test/ai-llm/mcp-tool-agent
version    : 0.1.0
