...
landscape that `@kiwa-test/payment` v0.3 mocks: hosted checkout (redirect
...
- `src/adapters/mock.ts` — `@kiwa-test/payment` `createLemonSqueezyMock` +
...
| 3 | `build` | `pnpm --filter @kiwa-test/payment -F @kiwa-test/core build` runs as a precondition of `pnpm test`. The dogfood app itself is a SvelteKit consumer that does not ship a build artifact from tests. | pass |
