...
that `@kiwa-test/payment` v0.3 mocks: checkout session creation, webhook
...
- `src/adapters/mock.ts` — `@kiwa-test/payment` `createStripeMock` +
...
| 3 | `build` | `pnpm --filter @kiwa-test/payment -F @kiwa-test/core build` runs as a precondition of `pnpm test`. dogfood app itself is a Next.js consumer that does not ship a build artifact. | pass |
