...
Package under exercise: [`@kiwa-test/orm`](../../../packages/orm/) (v0.9).
...
1. **Transactional outbox** — Debezium-style outbox row appended in the
   ...
   `@kiwa-test/orm`'s `createCdcSession` / `decodeEvent` / `appendOutbox`
   / `markEventOrdered`).
...
3. **Streaming replication** — primary write advances LSN, per-replica
   ...
   `promoteReplica`) via `@kiwa-test/orm`'s replication semantics.
