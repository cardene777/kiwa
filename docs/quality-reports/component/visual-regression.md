...
Real-vs-mock behavioural fidelity for the 10-scene Chromatic-style visual regression dogfood, produced by `examples/dogfood-visual-regression/tests/emit-fidelity-report.test.ts`. Feeds `@kiwa-lab/quality-metrics` 7-axis release gate.
...
- `seedBaselines` — walks the 10-scene registry, renders each with `seedArgs()`, hashes the pseudo-HTML through `@kiwa-lab/component` `hashMarkup`, and seeds a `VisualBaseline` for every declared viewport (10 scenes × 2 viewports = 20 seed pairs per invocation)
