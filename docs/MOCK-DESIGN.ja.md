...
kiwa がサードパーティ wallet / AA SDK (WalletConnect / Safe / thirdweb / Privy / Biconomy ...) に対して **何をどこまで mock するか** を決める仕様書。 Phase D-3 の実装 PR が参照する SSOT。
...
- kiwa fixture が SDK の標準 entry point を wrap
...
2. kiwa が in-memory relay stub を提供 (実 WalletConnect cloud project 不要)
3. 利用者は test 内で `walletKit.pair(...)` を実 wallet 接続と同じ書き方で呼ぶ
4. kiwa が session proposal / response を stub 経由でルーティング
...
- 内部実装は kiwa contributors が書く (mock contract deploy + deterministic data 返却)
...
1. kiwa が `examples/nextjs-safe-multisig/` で Safe contract を自前 deploy (threshold + module + guard 振る舞いを再現)
...
kiwa は「embedded wallet」の汎用 pattern (private key を server-side 保管、 signature 要求は stub auth endpoint 経由で round-trip) を Privy / Dynamic と名指しせず documentation する。 利用者は自分の SDK に pattern を読み替える。
...
各 level で実環境との既知差分を documentation する。 利用者が「kiwa で十分か、 実 infra での staging test も必要か」を判断できるようにする。
...
これらの境界を検証したい利用者は kiwa CI に加えて **実 testnet での staging test** を実施する。
