...
@kiwa-lab/streaming v0.2 → v0.3 minor bump。 3 provider (Kafka / Redpanda / NATS) 上に advanced streaming semantics 8 axis を追加 (Kafka raw + consumer group + Redpanda schema + transactions + NATS JetStream durable + KV/Object + exactly-once + consumer lag telemetry)。
...
`pnpm add -D @kiwa-lab/streaming` で v0.3.0 が入る。 breaking change なし。 migration guide は https://cardene777.github.io/kiwa/migrations/v1.30-to-v1.31
