# kiwa v1.31 released — Streaming 深化 II (@kiwa-lab/streaming v0.3.0 + 8 axis advanced + real driver + 縦深化 pair 第 3 pair 完成)
...
- **`@kiwa-lab/streaming` v0.2.0 → v0.3.0 minor bump** — 8 axis advanced streaming production semantics + real driver env-gate + 3 provider (Kafka / Redpanda / NATS) neutral state machine 追加
...
v1.20 で `@kiwa-lab/streaming` v0.1 を land した時点で、 kiwa は 3 provider (Kafka / Redpanda / NATS) 上に **5 base semantics** (producer / consumer / exactly-once / DLQ / schema-registry) を統一 mock として提供していた。 broker binary + Docker + Zookeeper 不要で mock only mode で走る、 実 test 環境の生産性を確保する目的の layer。
...
```typescript
import { createKafkaRawEnv } from '@kiwa-lab/streaming/semantics/kafka-raw-protocol';
