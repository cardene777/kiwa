...
v1.14-4 で land した `@kiwa-lab/observability` v1.1 telemetry mock 3 provider (OpenTelemetry / Datadog / Sentry) を基盤に、 実運用の dashboard 描画 + alert routing + trace flame graph + log ↔ span correlation の 4 追加軸を major bump として重ねる縦軸思想です。

## 主な追加

### `@kiwa-lab/observability` v2.0 (major bump)
...
- Migration guide `v1.16 → v1.17` (additive-only、 既存 test は無変更で pass、 `@kiwa-lab/observability` v1.x → v2.0 major bump は import 追加のみで移行完了)
