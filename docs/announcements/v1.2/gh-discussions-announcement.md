...
|---|---|---|
| Core fixture | `@kiwa-lab/dapp` + `@kiwa-lab/core` | Playwright + viem + anvil + EIP-6963 + ERC-4337 |
| API integration | `@kiwa-lab/api` | msw / supertest |
| Component | `@kiwa-lab/ui` | React / Vue / Svelte / Solid / Lit / Qwik / Angular / Chromium (8 adapters) |
| E2E + A11y + Visual | `@kiwa-lab/e2e` + `@kiwa-lab/a11y` + `@kiwa-lab/visual` | Playwright + axe-core + pixelmatch |
| Data + CLI + Observability | `@kiwa-lab/data` + `@kiwa-lab/cli-test` + `@kiwa-lab/observability` | queue / cron / shell IO / flaky detection |
| **Server framework (v1.2 main)** | `@kiwa-lab/{nextjs,nuxt,sveltekit,remix,astro,solidstart,qwikcity}` | 7 framework helpers, all `setupXxxEnv` + `mode` + `stop()` |
| **Edge runtime (v1.2 new)** | `@kiwa-lab/edge` | Cloudflare Workers / Vercel Edge, KV mock, no Miniflare |
| **ORM query (v1.2 new)** | `@kiwa-lab/orm` | Drizzle / Prisma / Kysely × SQLite / Postgres / MySQL — 9 combos |
...
## 2. ORM matrix — 9 combos via `@kiwa-lab/orm` v0.6.0
...
- `examples/nuxt-server-routes-full/` — Nuxt 3 + `@kiwa-lab/nuxt` v1.0.4
- `examples/sveltekit-full/` — SvelteKit 2 + `@kiwa-lab/sveltekit` (load + actions + hooks)
- `examples/remix-full/` — Remix v2 + `@kiwa-lab/remix` (loader + action + Resource Routes)
- `examples/astro-server-endpoints-full/` — Astro v5 SSR + `@kiwa-lab/astro` (`invokeEndpoint`)
- `examples/nextjs-app-router-full/` — Next.js v15 + `@kiwa-lab/nextjs` (all 4 layers)
...
| Cloudflare Workers / Vercel Edge | `@kiwa-lab/edge` (KV mock + `invokeEdgeHandler`) | ✅ no Miniflare required |
...
pnpm add -D @kiwa-lab/dapp @kiwa-lab/orm @kiwa-lab/sveltekit
