...
| Component | `@kiwa-lab/ui` ships **8 adapters** (see § 2) |
...
## 2. `@kiwa-lab/ui` covers 8 component surfaces from one package
...
| Python | 1 PyPI package (`kiwa-test-py`) | `@kiwa-lab/spec` Python port + requests / httpx adapter |
...
- Adapter contributions welcome — `@kiwa-lab/ui` is purpose-built for new framework adapters (the SolidJS adapter took 1 PR and 100 LOC).
- Polyglot bridges are the most strategic contribution surface. Rust / Go ports of `@kiwa-lab/spec` are on the roadmap — let's talk in this thread if you want to drive one.
