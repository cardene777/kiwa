...
| Surface | 例 | adapter |
|---|---|---|
| Contract | Foundry / Hardhat / forge / Solidity | `@kiwa-lab/spec` + skill chain |
| API integration | msw / supertest / Vitest | `@kiwa-lab/api` |
| Component | React / Vue / Svelte / Solid / Lit / Qwik / Angular / Chromium | `@kiwa-lab/ui` |
| E2E | Playwright + anvil + viem + EIP-6963 + ERC-4337 | `@kiwa-lab/core` + `@kiwa-lab/e2e` |
| A11y + Visual | axe-core / pixelmatch | `@kiwa-lab/a11y` + `@kiwa-lab/visual` |
| Data + CLI + Observability | queue / cron / shell IO / flaky 検出 | `@kiwa-lab/data` + `@kiwa-lab/cli-test` + `@kiwa-lab/observability` |

`@kiwa-lab/ui` だけで 8 framework + 実 Chromium に対応します。 全 adapter が `mode (render / interaction / snapshot) + stop()` 契約を共有し、 `env.kind` で discriminate できます。
...
- **TypeScript** ... 11 npm package (`@kiwa-lab/{core, spec, api, ui, data, cli-test, e2e, a11y, visual, observability, cli}`)
- **Python** ... 1 PyPI package (`kiwa-test-py`、 `@kiwa-lab/spec` の Python 移植 + requests / httpx adapter)
...
|---|---|---|
| `@kiwa-lab/api` | **96.06 %** | 90 |
| `@kiwa-lab/a11y` | **93.62 %** | 90 |
| `@kiwa-lab/ui` | **91.76 %** | 80 |
| `@kiwa-lab/cli-test` | 89.69 % | 80 |
| `@kiwa-lab/data` | 86.93 % | 80 |
| `@kiwa-lab/spec` | 85.51 % | 80 |
| `@kiwa-lab/core` | 85.09 % | 80 |
| `@kiwa-lab/cli` | 84.44 % | 80 |
| `@kiwa-lab/e2e` | 84.21 % | 80 |
| `@kiwa-lab/observability` | 84.12 % | 80 |
| `@kiwa-lab/visual` | 83.02 % | 80 |
...
- **enterprise web (Angular / Vue)** ... `@kiwa-lab/ui` の framework adapter で component test
- **CLI tool / queue worker** ... `@kiwa-lab/cli-test` + `@kiwa-lab/data` で shell IO + queue test
...
- 📦 npm ... https://www.npmjs.com/package/@kiwa-lab/core
...
- `@kiwa-lab/ui` 1 package で React / Vue / Svelte / Solid / Lit / Qwik / Angular + Chromium の 8 adapter
