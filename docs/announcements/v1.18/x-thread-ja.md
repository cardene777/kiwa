...
6/ dogfood app 3 種 — `dogfood-reth-node-test` (Reth NodeBuilder dev chain + alloy Provider ERC-20 drive + 3-block reorg × fidelity harness、 32 cargo test)、 `dogfood-foundry-invariant-fuzz` (ERC-20 + Vault + Router × invariant 10_000 run + fuzz seed 決定的化 + shrink parser 検証 + coverage feed、 21 cargo test)、 `dogfood-dapp-e2e-reorg` (Next.js 15 + viem + wagmi + `@kiwa-test/dapp` reorg fixture + anvil fork mainnet + 4 reorg scenario、 26 Playwright test)。 全 3 app が 7 軸 release gate PASS。

7/ `@kiwa-test/dapp` reorg helper — 2 new named export `snapshotChain(client)` + `revertChain(client, snapshotId)` (`anvil_snapshot` + `anvil_revert` の JSON-RPC 経由 wrap)。 reorg dogfood app + tutorial 27 が 1 `import` で共有。 package version は変更なし。
