...
v1.18 は kiwa の 8 milestone 目です。 v1.17 (Observability v2 縦軸、 `@kiwa/observability` v2.0 で Grafana / AlertManager / trace flame / log correlation を統一 mock harness に land) の後、 v1.18 は 2026 dApp production team が実運用で必要な **Rust blockchain testing stack (Reth (Rust EL client) integration + Foundry-rs invariant / fuzz runner 深化 + Alloy encoder helpers (EIP-712 / Multicall3 / Permit2) + dApp e2e reorg 拡張) を 1 統一 harness に land** しました。
...
### `@kiwa/dapp` reorg helpers (v0.x additive)

v1.18 で `@kiwa/dapp` package に 2 named export を追加 (package version は変更なし、 既存 fixture API も無変更、 backward compatible)。
