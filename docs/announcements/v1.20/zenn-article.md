...
### `@kiwa-lab/streaming` v0.1.0 (new package)
...
### `@kiwa-lab/streaming` v0.1.0 の Redpanda + schema registry

```ts
import { createRedpandaMock, createSchemaRegistry } from '@kiwa-lab/streaming';
...
### `@kiwa-lab/streaming` v0.1.0 の NATS + JetStream + KV + Object store

```ts
import { createNatsMock, compileSubject, matchSubject } from '@kiwa-lab/streaming';
...
- **`examples/dogfood-nats-jetstream`** — NATS core pub/sub + `*` / `>` wildcard routing + JetStream persistent stream + KV store + Object store。 `makeMockAdapter` は `@kiwa-lab/streaming` の `createNatsMock` + `js.kv` + `js.objectStore`、 `makeRealAdapter` は minimal NATS-shaped runtime。 `NatsFidelityReport` は 5 scenario (core pub/sub / wildcard routing (`*` single-token + `>` trailing multi-token) / JetStream persistent stream + consumer ack / KV round-trip + revision tracking / Object store round-trip + chunk-based store) を比較。 7 軸 release gate PASS。
...
- **1 new npm package** (`@kiwa-lab/streaming` v0.1.0)
