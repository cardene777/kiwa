...
```ts
import { startThreeDs, threeDsRequestChallenge, threeDsSubmitChallenge, threeDsFrictionless } from '@kiwa-test/payment';
...
```ts
import { createSubscription, changePlan, pauseSubscription, resumeSubscription, cancelSubscription, reactivateSubscription } from '@kiwa-test/payment';
...
```ts
import { calculateTax } from '@kiwa-test/payment';
...
tutorial 39-41 の全 code snippet を実 `@kiwa-test/payment` API import + execute + assertion で走査、 18 test で drift を検知 (`docs-tutorial-v1.21.test.ts` / `docs-tutorial-v1.22.test.ts` と同じ pattern)。

## Numbers

- **6 sub-Issue 解決** (#900-#905)
- **6 PR merge** (v1.23-1 + v1.23-2 + v1.23-3 + v1.23-4 + v1.23-5 + 本 publish PR)
- **1 npm minor bump** (`@kiwa-test/payment` v0.2.0 → v0.3.0) — kiwa runtime fixture 34 packages 維持
...
9 axis は 3 target provider の billing envelope を再現する最小 set。 各 axis は `@kiwa-test/payment/semantics/*` 下の独立 module、 shared `PaymentAdapter` interface + 強型 `AxisStep` sequence emit で「event 名」 ではなく「transition」 に対する assertion を可能にする。
