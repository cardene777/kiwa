# kiwa v1.32 x-thread (English)

## Tweet 1 — hook

kiwa v1.32 is out — Database 深化 II land.

@kiwa-test/orm v0.9 → v0.10 minor bump. 8 axis advanced production db semantics (Postgres logical replication + Postgres MVCC + MySQL cluster + MySQL binlog + SQLite WAL + SQLite FTS5 + txn isolation + advanced connection pool) across 3 ORM (Drizzle / Prisma / Kysely) × 3 backend (Postgres / MySQL / SQLite).

Real driver env-gate + testcontainers for opt-in production fidelity walkthrough. 3 dogfood app v2 / 新規 (postgres-cdc-outbox-app v2 + mysql-rls-tenant-app v2 + sqlite-wal-fts-app 新規) 全 7 軸 release gate PASS.

Vertical deepening pair pattern 第 4 pair 連続化 (Auth v1.21→v1.22 + Realtime v1.13→v1.28 + Streaming v1.20→v1.31 + Database v1.14→v1.32) — kiwa の縦深化戦略 SSOT を database production layer に拡張。

## Tweet 2 — 8 axis semantics

v1.32 shipped 8 axis advanced production db semantics:
...
3 ORM × 3 backend × (v1.26 8 base + v1.32 8 advanced) axis fidelity harness 144-row grid で release-gate 露出。 real driver env-gate + testcontainers で opt-in production walkthrough。

## Tweet 3 — vertical deepening pair pattern 4 pair grid

v1.32 で kiwa の縦深化 pair pattern が 4 pair 連続完成:
...
`pnpm add -D @kiwa-test/orm` で v0.10.0 が入る。 zero breaking changes。 migration guide は https://cardene777.github.io/kiwa/migrations/v1.31-to-v1.32

Next up: v2.0. Multi-version Vitest matrix + desktop/mobile adapters + coverage 100 % milestone + database depth III (SurrealDB + EdgeDB + Turso + CockroachDB + TimescaleDB + QuestDB) が有力候補。 feedback welcome。
