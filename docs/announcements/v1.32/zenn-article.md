# kiwa v1.32 released — Database 深化 II (@kiwa-test/orm v0.10.0 + 8 axis advanced + real driver + 縦深化 pair 第 4 pair 連続化)
...
- **`@kiwa-test/orm` v0.9.0 → v0.10.0 minor bump** — 8 axis advanced production db semantics + real driver env-gate + 3 ORM × 3 backend × 8 axis neutral state machine 追加
...
```bash
pnpm add -D @kiwa-test/orm @kiwa-test/core
```

Postgres logical replication advanced の pure state machine helper:

```typescript
import { createLogicalReplicationAdvancedSession } from '@kiwa-test/orm/semantics/postgres-logical-replication';
