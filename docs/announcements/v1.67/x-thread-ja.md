...
kiwa v1.67 リリース — Desktop 深化 IX。 **@kiwa-test/desktop v1.0** (major bump) で invoke-cache layer 統合、 InvokeCache class = LRU + TTL 両立 in-memory cache + withCache helper = probeAndInvoke 統合、 4 status 経路 (cache-hit / cache-miss / cache-invalidated / cache-disabled)。 v1.55-v1.66 4 PR rhythm 継承 (**14 milestone 連続 = 56 PR 連続同 rhythm**)、 **systematic pattern 42 度目適用**、 **depth-6 pattern 2 例目確定 candidate**。
