...
kiwa v1.64 リリース — Desktop 深化 VIII。 **@kiwa-lab/desktop v0.9** で 実 native binding 呼出 (probeAndInvoke) 追加、 probe availability 判定で 実 CLI 存在時のみ 実 spawn 呼出。 v1.55-v1.63 4 PR rhythm 継承 (**11 milestone 連続 = 44 PR 連続同 rhythm**)、 **systematic pattern 39 度目適用**、 **depth-9 pattern 新設 candidate 到達**。
