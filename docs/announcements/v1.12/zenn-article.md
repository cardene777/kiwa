---
title: "kiwa v1.12 リリース — AI-LLM 縦軸 (11 軸 release gate + @kiwa-test/ai-llm 4 SDK 統一 mock + dogfood 3 app + non-determinism SSOT)"
emoji: "🌱"
type: "tech"
...
cost / latency / token / accuracy の 4 axis を release gate に追加、 `@kiwa-test/ai-llm` v0.1 で Anthropic + OpenAI + Vercel AI SDK + LangChain の 4 SDK を 1 mock engine で統一、 dogfood 3 app で streaming / tool-use / RAG の 3 主要 use case を real vs mock 実測、 `docs/concepts/ai-llm-testing.md` で non-determinism を第一級 concept として言語化。
...
## 1. `@kiwa-test/quality-metrics` v0.2 — 11 軸 release gate SSOT (v1.12-1)

v1.11 の 5 軸 (coverage / test count / fidelity / perf p95 / mutation kill) は unchanged。 name が `@kiwa-test/ai-` で始まる provider だけ AI-LLM 分岐に入り、 追加 4 軸を強制。
...
```ts
import { isAiLlmProvider, DEFAULT_AI_LLM_RELEASE_GATE_THRESHOLDS } from '@kiwa-test/quality-metrics';

// name が "@kiwa-test/ai-" で始まる時のみ AI-LLM 分岐に入る
...
## 2. `@kiwa-test/ai-llm` v0.1 — 4 SDK 統一 mock (v1.12-1)
...
- v1.11 → v1.12 (additive-only、 diff 形式、 verification コマンド付、 `@kiwa-test/ai-llm` add 1 行 + AI-LLM provider は自動で 11 軸 gate に乗る)
...
v1.11 user は zero-migration。 既存 test file はそのまま動く。 v1.12 追加は全て opt-in、 `@kiwa-test/ai-*` provider を使わない限り 11 軸 branch には入らない。
