...
### `@kiwa-lab/auth` v0.5.0 (real driver adapter + caBLE + Nuxt 3 RP full flow + Federation JWKS rotation real e2e)
...
- **`mock only`** (default、 `KIWA_MODE` 未 set) = pure `@kiwa-lab/auth` mock、 network 0、 ~1 ms per test。 全 PR が本 mode で走行。
...
- 40 new test で `@kiwa-lab/auth` 新 caBLE mock vs real Chrome + Android phone pair の 5 軸 fidelity 検証
...
- **1 npm minor bump** (`@kiwa-lab/auth` v0.4.0 → v0.5.0) — kiwa runtime fixture 34 packages 維持
