# kiwa v1.36 released — Search 深化 II (@kiwa-test/search v0.3.0 + 8 axis advanced search + 縦深化 pair 第 8 pair 連続化 + 3-stage 拡張 pattern 3 例目)

## TL;DR

- **kiwa v1.36 released** — Search 深化 II milestone
- **`@kiwa-test/search` v0.2.0 → v0.3.0 minor bump** — 8 axis advanced search semantics + real driver env-gate + 4 provider × 8 axis neutral state machine 追加
...
v1.14 で `@kiwa-test/search` v0.1 (Meilisearch / Algolia / Typesense の 3 provider を統一 mock として提供する in-memory search envelope) を land、 v1.15 で search v0.2 に拡張した時点で、 kiwa は query / index / search / facet / typo tolerance の base semantics までは cover していた.
