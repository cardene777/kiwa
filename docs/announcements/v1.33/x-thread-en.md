...
@kiwa-lab/payment v0.3 → v0.4 minor bump. 8 axis advanced billing II semantics (orchestration + revenue recovery + refund advanced + dispute + webhook idempotency advanced + tax localization + subscription state machine + payment method vault) across 3 provider (Stripe / Paddle / Lemon Squeezy).
...
`pnpm add -D @kiwa-lab/payment` で v0.4.0 が入る。 zero breaking changes。 migration guide は https://cardene777.github.io/kiwa/migrations/v1.32-to-v1.33
