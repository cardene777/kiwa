# kiwa v1.33 released — Payment 深化 II (@kiwa-lab/payment v0.4.0 + 8 axis advanced billing II + real driver + 縦深化 pair 第 5 pair 連続化 + 11 milestone snippet streak)
...
- **`@kiwa-lab/payment` v0.3.0 → v0.4.0 minor bump**。 8 axis advanced billing II semantics + real driver env-gate + 3 provider × 8 axis neutral state machine を追加。 v0.3 API は完全維持 (additive-only 契約)。
...
- **v1.33-6 publish** (Issue #1041, this PR)。 `.claude-plugin/plugin.json` 1.32.0 → 1.33.0 + description v1.33 section + payment keywords + Roadmap ✅ v1.33 row + announcement 4 file + release-smoke `v1-33-publish.test.ts` (7 axis publish artefact invariant) + docs-e2e `V1_33_PAGES` (5 page render check) + `pnpm run release` 経由 npm publish (`@kiwa-lab/payment` v0.4.0) + `/docs-publish-kiwa` 経由 gh-pages 反映。

## Numbers

- **6 sub-Issues resolved** (#1035 / #1037 / #1038 / #1039 / #1040 / #1041)
- **6 PRs merged** (v1.33-1 through v1.33-6)
- **1 npm minor bump** (`@kiwa-lab/payment` v0.3.0 → v0.4.0) — kiwa runtime fixture **35 packages** 維持
...
- **Payment pair (v1.23 → v1.33、 this)** ... `@kiwa-lab/payment` v0.2-v0.3 3 provider webhook + 9 base billing semantics → v0.4 8 advanced billing II semantics + real driver env-gate + Stripe Connect + Paddle Billing v2 + Lemon Squeezy license
...
Thanks to everyone who reviewed the v1.33 sub-Issues, tested `@kiwa-lab/payment` v0.4 pre-release, and helped shape the 縦深化 pair pattern SSOT into a 5-pair grid. On to v2.0.
