# kiwa v1.33 released — Payment 深化 II (@kiwa-lab/payment v0.4.0 + 8 axis advanced billing II + real driver + 縦深化 pair 第 5 pair 連続化)

## TL;DR

- **kiwa v1.33 released** — Payment 深化 II milestone (advanced billing II 8 axis + real driver + 縦深化 pair 第 5 pair 連続化)
- **`@kiwa-lab/payment` v0.3.0 → v0.4.0 minor bump** — 8 axis advanced billing II semantics + real driver env-gate + 3 provider × 8 axis neutral state machine 追加
...
```bash
pnpm add -D @kiwa-lab/payment @kiwa-lab/core
```

Payment orchestration の pure state machine helper:

```typescript
import { startOrchestration, routeCharge, probeCircuit } from '@kiwa-lab/payment/semantics/orchestration';
