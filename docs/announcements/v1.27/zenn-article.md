...
`@kiwa-lab/*` 34 package (含 quality-metrics itself) に以下 4 経路を land。
...
`@kiwa-lab/quality-metrics` v0.3 で `evaluateReleaseGate(report, thresholdOverrides, ctx)` の第 3 引数に `ReleaseGateContext` type を追加、 `ctx.mutationTier` が set された時のみ 12th axis (`mutation.tier`) を発動、 tier floor 未達で `mutation.tier` axis を fail。
...
const PACKAGE_TIER = {
  '@kiwa-lab/core': 'Core',
  '@kiwa-lab/nextjs': 'Framework',
  '@kiwa-lab/payment': 'SaaS',
  '@kiwa-lab/a11y': 'TestType',
} as const;

const tier = resolveMutationTier('@kiwa-lab/payment', PACKAGE_TIER); // 'SaaS'
