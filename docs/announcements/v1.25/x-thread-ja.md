...
3/ v1.25-1 core layer perf sweep — 9 package (`@kiwa-test/core` / `dapp` / `api` / `ui` / `data` / `cli-test` / `observability` / `e2e` / `cli`) に `tests/perf/{package}.perf.ts` + `test:perf` script + `.perf-baseline/{package}.json` 追加。 各 suite は `runPerf3Layer` (serial concurrency=1 200 iter + concurrent concurrency=10 500 iter + memory heap sampling) 経由。
...
8/ v1.25-6 publish — `@kiwa-test/perf-harness` v0.1.1 → v0.2.0 minor bump を npm publish。 additive-only ... v0.1 signature 完全維持。 VitePress sidebar に `Perf-harness sweep (v1.25)` tutorial section + `perf-testing-ssot.md` (Concepts) + `v1.24 → v1.25` (Migrations) 追加。 gh-pages publish。 release script filter に `@kiwa-test/perf-harness` 追加 (v1.14 payment 漏れ再発防止)。 Roadmap ... https://github.com/cardene777/kiwa#roadmap — v1.11 → v1.12 → v1.13 → v1.14 → v1.15 → v1.16 → v1.17 → v1.18 → v1.19 → v1.20 → v1.21 → v1.22 → v1.23 → v1.24 → v1.25 の 15 milestone 連続完遂。
