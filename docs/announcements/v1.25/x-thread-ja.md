...
3/ v1.25-1 core layer perf sweep — 9 package (`@kiwa-lab/core` / `dapp` / `api` / `ui` / `data` / `cli-test` / `observability` / `e2e` / `cli`) に `tests/perf/{package}.perf.ts` + `test:perf` script + `.perf-baseline/{package}.json` 追加。 各 suite は `runPerf3Layer` (serial concurrency=1 200 iter + concurrent concurrency=10 500 iter + memory heap sampling) 経由。
