...
- **`@kiwa-test/mcp` v0.1** — Model Context Protocol server + client mock with `InMemoryTransport`. Covers the 4 ops MCP-aware tool-use clients need (`initialize` / `notifications/initialized` / `tools/list` / `tools/call`), 8 JSON-RPC error codes (including MCP extensions -32000 / -32001 / -32002 / -32003), and 5 fixture tools (echo / calc / weather / search / db-query).
- **`@kiwa-test/agent` v0.1** — LangGraph-style `StateGraph` + OpenAI Assistants v2 client under one API. 6-item compile validation (fail-fast), deterministic run ids, run-status transitions (`queued` → `requires_action` → `submitToolOutputs` → `completed`) match real Assistants v2 shape.
- **dogfood-multimodal-chat + dogfood-mcp-tool-agent** — 2 new dogfood apps that measure the mocks against real Anthropic + real MCP endpoints. Both roll into the 11-axis release gate via the `@kiwa-test/ai-*` provider prefix.
...
- **3 new / expanded packages** (`@kiwa-test/ai-llm` v0.2 + `@kiwa-test/mcp` v0.1 + `@kiwa-test/agent` v0.1)
