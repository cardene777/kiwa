...
| allowList.length
        ...
        : 'credentialAssertion: no credentials are registered — call credentialCreation first' | [packages/auth/src/webauthn/assertion.ts](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/webauthn/assertion.ts#L80) |
...
この section は [公開 entry point](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/index.ts) から同期しています。各項目は公開名、実際の TypeScript 宣言、宣言元のソース位置を示します。実装に JSDoc がある場合は、その説明も表示します。
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/passkey/credential-sync.ts#L21) `packages/auth/src/passkey/credential-sync.ts`
...
```ts
export function backupPasskeyCredential(
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/webauthn/encoding.ts#L27) `packages/auth/src/webauthn/encoding.ts`

```ts
export function base64UrlDecodeWebAuthn(input: string): Uint8Array;
```

#### `base64UrlEncodeWebAuthn`

[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/webauthn/encoding.ts#L14) `packages/auth/src/webauthn/encoding.ts`

```ts
export function base64UrlEncodeWebAuthn(input: string | Uint8Array): string;
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/supabase-advanced/mfa.ts#L56) `packages/auth/src/supabase-advanced/mfa.ts`

Build the standard `otpauth://` URI clients scan into an authenticator app.

```ts
export function buildSupabaseOtpAuthUri(input: {
  secret: string;
  accountName: string;
  issuer: string;
}): string;
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/oauth21/dpop.ts#L55) `packages/auth/src/oauth21/dpop.ts`
...
```ts
export function computeDpopJkt(jwk: DpopJwk): string;
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/better-auth/session.ts#L36) `packages/auth/src/better-auth/session.ts`

```ts
export async function createBetterAuthSessionFor(
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/oidc/federation.ts#L143) `packages/auth/src/oidc/federation.ts`
...
```ts
export function createOidcEntityStatement(input: {
  iss: string;
  sub: string;
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/oidc/federation.ts#L127) `packages/auth/src/oidc/federation.ts`

Build a plain trust-anchor fixture for tests. Wraps the manual object construction so tests import a single helper.

```ts
export function createOidcTrustAnchor(input: {
  entity_id: string;
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/supabase-advanced/siwe.ts#L74) `packages/auth/src/supabase-advanced/siwe.ts`
...
```ts
export function deriveSupabaseMockAddress(privateKey: string): string;
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/passkey/credential-sync.ts#L92) `packages/auth/src/passkey/credential-sync.ts`
...
```ts
export function findPasskeyFabricHolding(
  credentialId: string,
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/better-auth/session.ts#L28) `packages/auth/src/better-auth/session.ts`

```ts
export function generateBetterAuthSessionId(): string;
```

#### `generateBetterAuthSessionToken`

[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/better-auth/session.ts#L32) `packages/auth/src/better-auth/session.ts`

```ts
export function generateBetterAuthSessionToken(): string;
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/clerk/jwt.ts#L81) `packages/auth/src/clerk/jwt.ts`

Generate a random secret for signing. Called once per {@link setupClerkEnv } invocation so each env has its own signing key.

```ts
export function generateClerkSigningSecret(): string;
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/supabase-advanced/mfa.ts#L76) `packages/auth/src/supabase-advanced/mfa.ts`

Generate a set of one-time backup codes. Each code is 10 hex characters, matching a common Supabase-adjacent pattern.

```ts
export function generateSupabaseBackupCodes(count = 10): string[];
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/supabase-advanced/siwe.ts#L12) `packages/auth/src/supabase-advanced/siwe.ts`
...
```ts
export function generateSupabaseSiweNonce(): string;
```

#### `generateSupabaseTotpCode`

[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/supabase-advanced/mfa.ts#L23) `packages/auth/src/supabase-advanced/mfa.ts`

Generate the TOTP code for the given moment. `nowSeconds` is exposed so tests can advance time deterministically.

```ts
export function generateSupabaseTotpCode(secretBase32: string, nowSeconds: number = Math.floor(Date.now() / 1000)): string;
```

#### `generateSupabaseTotpSecret`

[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/supabase-advanced/mfa.ts#L14) `packages/auth/src/supabase-advanced/mfa.ts`

```ts
export function generateSupabaseTotpSecret(byteLength = 20): string;
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/better-auth/password.ts#L27) `packages/auth/src/better-auth/password.ts`
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/better-auth/session.ts#L68) `packages/auth/src/better-auth/session.ts`

```ts
export async function invalidateBetterAuthSessionsForUser(
  database: BetterAuthDatabaseAdapter,
  userId: string,
): Promise<void>;
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/passkey/credential-sync.ts#L107) `packages/auth/src/passkey/credential-sync.ts`
...
```ts
export function requirePasskeyFabric(
  fabrics: readonly SyncFabric[],
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/oidc/federation.ts#L25) `packages/auth/src/oidc/federation.ts`
...
```ts
export function resolveOidcTrustChain(input: ResolveTrustChainInput): TrustChainResult;
```

#### `restorePasskeyCredential`

[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/passkey/credential-sync.ts#L50) `packages/auth/src/passkey/credential-sync.ts`
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/semantics/index.ts#L1) `packages/auth/src/semantics/index.ts`
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/supabase-advanced/siwe.ts#L21) `packages/auth/src/supabase-advanced/siwe.ts`
...
```ts
export function serializeSupabaseSiweMessage(msg: SiweMessage): string;
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/passkey/credential-sync.ts#L69) `packages/auth/src/passkey/credential-sync.ts`
...
```ts
export function syncPasskeyCredentials(
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/better-auth/session.ts#L50) `packages/auth/src/better-auth/session.ts`

```ts
export async function validateBetterAuthSessionByToken(
  database: BetterAuthDatabaseAdapter,
  token: string,
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/better-auth/password.ts#L40) `packages/auth/src/better-auth/password.ts`
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/supabase-advanced/mfa.ts#L40) `packages/auth/src/supabase-advanced/mfa.ts`

```ts
export function verifySupabaseTotpCode(
  secretBase32: string,
  code: string,
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/webauthn/encoding.ts#L83) `packages/auth/src/webauthn/encoding.ts`
...
```ts
export function webAuthnClientDataHash(clientDataJSON: string): string;
```

#### `webAuthnCredentialAssertion`

[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/webauthn/assertion.ts#L61) `packages/auth/src/webauthn/assertion.ts`
...
```ts
export function webAuthnCredentialAssertion(
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/webauthn/creation.ts#L41) `packages/auth/src/webauthn/creation.ts`
...
```ts
export function webAuthnCredentialCreation(
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/webauthn/encoding.ts#L59) `packages/auth/src/webauthn/encoding.ts`
...
```ts
export function webAuthnMockSignature(
  publicKey: string,
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/webauthn/encoding.ts#L46) `packages/auth/src/webauthn/encoding.ts`

Normalize a challenge or credential.id that a caller may hand in as either `string` (base64url or plain UTF-8) or `Uint8Array`.

```ts
export function webAuthnNormalizeChallenge(input: string | Uint8Array): string;
```
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/oidc/types.ts#L280) `packages/auth/src/oidc/types.ts`
...
```ts
export interface OidcEntityStatement {
  /** Issuing entity. */
  iss: string;
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/semantics/types.ts#L194) `packages/auth/src/semantics/types.ts`

```ts
export interface SemanticsAxisStep<TState> {
...
[ソース宣言](https://github.com/cardene777/kiwa/blob/main/packages/auth/src/semantics/types.ts#L40) `packages/auth/src/semantics/types.ts`
...
```ts
export type SemanticsNeutralEventName =
  // device-bound-passkey
...
export interface SetupSupabaseAuthEnvOptions {
   ...
   * the suite.
   */
  tokens?: Array<{ userEmail: string }> | undefined;
}
```
...
export interface SupabaseAuthTestEnv extends TestEnvBase<'mock'> {
   ...
   */
  seededTokens: Record<string, { accessToken: string; refreshToken: string; sessionId: string }>;
  ...
  auth: {
    ...
    }) => Promise<{ otp: SupabaseOtpDelivery }>;
...
```ts
export type SupabaseIdentityProvider =
  | 'email'
  | 'google'
  | 'github'
  | 'apple'
