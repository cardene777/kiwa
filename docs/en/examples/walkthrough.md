...
Expect — 11 cases from `connect.spec.ts` + 4 from `eip6963.spec.ts` = 15 passing, end-to-end in ~4s (the chain is `pnpm -F @kiwa-test/dapp build` → `playwright test`).
...
1. `pnpm -F @kiwa-test/dapp build` refreshes the fixture dist
