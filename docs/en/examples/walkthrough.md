...
Expect — 11 cases from `connect.spec.ts` + 4 from `eip6963.spec.ts` = 15 passing, end-to-end in ~4s (the chain is `pnpm -F @kiwa/dapp build` → `playwright test`).
...
1. `pnpm -F @kiwa/dapp build` refreshes the fixture dist
