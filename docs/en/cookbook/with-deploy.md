...
pnpm dlx @kiwa-lab/cli init --with-deploy ../contract
```
...
Instead of using `dappE2eTest` from `@kiwa-lab/dapp` directly, route through `tests/fixture.ts` so the spec shares the globalSetup anvil.
...
- [Quickstart](../quickstart.md) — the first five minutes (the bare `pnpm dlx @kiwa-lab/cli init` path)
