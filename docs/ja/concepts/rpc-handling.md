...
dapp-e2e core は 9 つの EIP-1193 RPC method を直接処理し、それ以外は anvil JSON-RPC へ forward します。
...
dapp-e2e は wallet の挙動を **コード側で完結** させることで CI フレンドリーにし、`setApprovalMode('reject')` のような UX 経路も切り替えで test できるようにしています。
