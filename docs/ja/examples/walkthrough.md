...
期待 — `connect.spec.ts` 11 件 + `eip6963.spec.ts` 4 件 = 計 15 件 PASS、 実行は約 4 秒 (内訳 — `pnpm -F @kiwa-lab/dapp build` で fixture dist 更新 → playwright test)。
...
1. `pnpm -F @kiwa-lab/dapp build` で fixture dist 更新
