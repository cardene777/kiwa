...
結論だけ先に言うと、dapp-e2e は anvil を使う headless E2E の安定運用に寄せた立ち位置です。
...
dapp-e2e は「ローカル chain 上で dApp 自体を headless に検証する」用途に最も合います。
...
3. ローカル chain 上の dApp 挙動と CI 安定性を同時に欲しいなら dapp-e2e
