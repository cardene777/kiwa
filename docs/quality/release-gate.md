...
package が tier 既定より緩い bar を持つ場合は `context.mutationTierThreshold` で渡す。 **現在その状態の package は 1 つも無い** = `scripts/check-mutation-gates.mjs` の `PACKAGE_TIER` が SSOT で、 `auth` の 65 と `realtime` の 60 が #1973 で、 `cache` の 60 が #1967 で、 `orm` の 60 が #1941 で撤去された。 本 field に数値を直接渡すと tier 表を迂回するため、 `PACKAGE_TIER` に override を記録した package にだけ使う。
...
`@kiwa-lab/ai-llm` 使用時は `buildAiLlmReport` / `buildAiLlmReportFromMock` で `runFidelityCheck` の結果から直接 `QualityReport` を組み立てられる (詳細 = `packages/ai-llm/README.md`)。

## AI-LLM 分岐の判定

`isAiLlmProvider(provider: string): boolean` が SSOT。 `@kiwa-lab/quality-metrics` から export、 判定は `provider.startsWith('@kiwa-lab/ai-')` の 1 行。 downstream consumer (dogfood app 等) は本 helper を import して同一判定を使う。
...
- `@kiwa-lab/quality-metrics` v0.2 (`packages/quality-metrics`)
- `@kiwa-lab/ai-llm` v0.1 (`packages/ai-llm`)
