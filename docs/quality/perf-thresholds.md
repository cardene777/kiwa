...
Matching the kind is the whole mechanism. A CPU reference against an fs op makes the spread **worse than the raw value** (135 → 171 %, 270 → 313 %), and so does an fs reference against a CPU op (16 → 175 %). Read and write are worth separating too: the read-shaped op lands at 17 % against `fs-read` and 51 % against `fs-write`.
