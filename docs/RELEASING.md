...
2. **scope の確保 (任意)** — `@kiwa/core` `@kiwa/cli` を使うには `@kiwa` organization を作成 (`https://www.npmjs.com/org/create`)。衝突する場合は `@cardene-kiwa/core` 等の別 scope を検討してください。
...
1. **npmjs.com で表示確認** — `https://www.npmjs.com/package/@kiwa/core` / `@kiwa/cli` で README / provenance badge / version `0.1.0` を確認。
