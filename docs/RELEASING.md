...
2. **Reserve a scope (optional)** — `@kiwa-test/dapp` and `@kiwa-test/cli` use the `@kiwa-test` npm organization (already created at https://www.npmjs.com/org/kiwa-test). If you fork kiwa and want a different scope, create your own org via `https://www.npmjs.com/org/create`.
...
1. **Verify the npmjs.com pages** — Check `https://www.npmjs.com/package/@kiwa-test/dapp` and `@kiwa-test/cli` for the README, provenance badge, and version `0.1.0`.
