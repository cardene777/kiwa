...
2. **scope の確保 (任意)** — `@dapp-e2e/core` `@dapp-e2e/cli` を使うには `@dapp-e2e` organization を作成 (`https://www.npmjs.com/org/create`)。衝突する場合は `@cardene-dapp-e2e/core` 等の別 scope を検討してください。
3. **Granular Access Token を発行** — Settings > Access Tokens > Generate New Token > Granular Access Token。
   - name: `dapp-e2e-publish`
...
1. **NPM_TOKEN secret 登録** — https://github.com/cardene777/dapp-e2e/settings/secrets/actions で `NPM_TOKEN` という名前で Phase 1.3 のトークンを登録。
...
1. **npmjs.com で表示確認** — `https://www.npmjs.com/package/@dapp-e2e/core` / `@dapp-e2e/cli` で README / provenance badge / version `0.1.0` を確認。
2. **smoke test** — 別の dApp project で `pnpm dlx @dapp-e2e/cli init` を実行し、生成された `e2e/connect.spec.ts` が PASS することを確認:

   ~~~bash
   mkdir /tmp/dapp-e2e-smoke && cd /tmp/dapp-e2e-smoke
   pnpm init
