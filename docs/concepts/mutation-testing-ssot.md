...
- **Slow container-backed tests** — the exclusions above are implementation files kept out of `mutate`. A test file kept out of the *runner* carries the same risk in a different place: the mutants it alone covers become no-coverage, and the covered score stops accounting for them. `@kiwa-lab/orm` excludes its two live-container test files from mutation runs (#1981), and the same rule licensed it — measured, those tests killed 1 of 15 mutants on the slice they alone cover, in 3m19s, with 6 of the remaining 14 recorded as timeouts. Since covered MSI counts a timeout as caught, keeping them raised the number without detecting anything. `docs/quality/mutation-thresholds.md § A slow test can also be a weak one` holds that measurement. The test of such an exclusion is whether the tests were *detecting*, not whether they were slow.
- **`include` allowlists in the runner config** — the two exclusions above are files kept out of `mutate` and test files kept out of the runner. A third form hides better: an `include` that names test files individually rather than globbing them. Nothing is marked excluded, so nothing reads as an exclusion, and the mutants those tests would have covered simply report as no-coverage. `@kiwa-lab/dapp` named three of its thirty-seven test files this way and measured 85 % no-coverage; with the glob restored it measures 15 % (#1982). **A high no-coverage rate is a claim about what ran, and the first thing to check is how many test files the runner was given** — compare Stryker's `Ran N tests` against `pnpm test`. `tests/release-smoke/tests/mutation-gate-coverage.test.ts` derives which packages still name files outright, so the set is checked rather than written; the recorded exceptions are `orm`, whose live-container exclusion is measured rather than assumed (#1981), and `ui`, which excludes only its real-Chromium test file and reaches zero no-coverage without it (#1986). The check reads `exclude` as well as `include`, since narrowing from either side has the same effect.
