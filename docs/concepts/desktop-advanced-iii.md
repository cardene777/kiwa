...
`@kiwa-lab/desktop` v0.3 の advanced III 4 axis (Screen recording + Global shortcut + Clipboard + Dark-mode) target-neutral state machine SSOT。 v1.58 で v0.2 8 axis → v0.3 12 axis に minor bump、 kiwa 縦深化 pair 第 14 の第 3 段、 v0.2 baseline (`docs/concepts/desktop-advanced-axis.md`) を extend。
...
v0.3 4 axis の追加は additive、 v0.1 + v0.2 の 8 axis / 32 method / 32 event / 96 mapping は完全保持。 依存関係も `@kiwa-lab/core` のみで v0.1 + v0.2 と同じ、 他 42 package への影響 0。
