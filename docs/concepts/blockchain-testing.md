...
- `@kiwa-test/dapp` — `snapshotChain(client)` / `revertChain(client, snapshotId)` wrap the raw JSON-RPC calls through the viem `PublicClient` request path, returning the typed `Hex` snapshot id and anvil's boolean revert response.
