...
```ts
import { measureSemanticsGrid, SEMANTICS_GRID } from '@kiwa-lab/realtime';
...
The v1.28 milestone applied the 8-axis grid to one package (`@kiwa-lab/realtime`) and three dogfood apps.

| Layer | Packages / apps |
|---|---|
| SaaS (adapter, 1) | `@kiwa-lab/realtime` v0.2 |
...
| mutation — tier | floor | v1.27-4 (12-axis) | `DEFAULT_MUTATION_TIER_THRESHOLDS[tier]` — SaaS default 65 % for the 3 dogfood apps + `@kiwa-lab/realtime` v0.2 |
