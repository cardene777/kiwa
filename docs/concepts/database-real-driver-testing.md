...
```ts
import { collectFidelityCoverage } from '@kiwa-test/orm';
...
v1.32 does not add a 14th release-gate axis. The 8 advanced axes gate the orm package's own tests (via `pnpm --filter @kiwa-test/orm test`) but do not surface as a per-package `@kiwa-test/quality-metrics` axis. The reasoning — the fidelity harness is backend-shape-specific, and a package that does not use a database has nothing to assert on. When a future milestone adds a `backend.fidelity` axis that describes "which backends this package's tests hit," it will slot into the 13-axis release gate as the 14th; v1.32 keeps the axis count at 13.
