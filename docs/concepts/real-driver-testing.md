...
Every v1.22 test surface (`@kiwa-lab/auth` v0.5, dogfood apps, tutorials 37-38) exposes three execution modes gated on the `KIWA_MODE` environment variable.

| Mode | Trigger | Behaviour | Latency budget | When to use |
|---|---|---|---|---|
| `mock only` | `KIWA_MODE` unset (default) | Pure `@kiwa-lab/auth` mock; every spec-critical branch runs deterministically | < 5 ms per test | Every PR, every push, every local run |
