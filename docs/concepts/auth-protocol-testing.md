...
- `createOidcTrustAnchor({ entity_id })` + `createOidcEntityStatement({ iss, sub, exp?, now? })` + `resolveOidcTrustChain({ leaf, intermediates, anchor, now? })` — Federation walker. Returns `{ valid, chain?, anchor?, reason? }` where `reason` surfaces one of `/no intermediate describes/`, `/expired/`, `/exhausted intermediates/`, `/cycle detected/`.
