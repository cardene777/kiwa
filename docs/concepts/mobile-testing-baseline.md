...
`@kiwa-lab/mobile` v0.1 の 3 axis (React Native + Expo + Metro) target-neutral state machine SSOT。 v1.50 で kiwa 縦深化 pair 第 13 新規 base pair として導入、 **41 package 到達**。
...
新 package `@kiwa-lab/mobile` の追加は additive、 既存 40 package 全部 API 変更 0。 依存関係も `@kiwa-lab/core` のみ、 他 package への影響なし。
