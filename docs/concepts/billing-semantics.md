...
The 9 axes below are the smallest set that reproduces the full billing envelope across the 3 target providers. Each axis is an independent module under `@kiwa-lab/payment/semantics/*`; each provides pure functions that operate on a `PaymentAdapter` (the same interface `createStripeMock` / `createPaddleMock` / `createLemonSqueezyMock` return) and emit a strongly typed `AxisStep` sequence.
...
|---|---|---|
| `mock only` | `KIWA_MODE` unset | Pure `@kiwa-lab/payment` mock adapter, 0 network, sub-100 ms per test |
