...
3. Emits a `FidelityReport` with per-op traces, latency samples, and per-axis coverage summary that feeds `@kiwa-lab/quality-metrics` release gate.
...
The report is hand-promoted to `docs/quality-reports/observability/<name>.md` when a release cuts. `evaluateReleaseGate` consumes the JSON directly and gates the release on the 7-axis pass (`@kiwa-lab/observability/*` prefix stays on the common 7-axis branch — no token pricing to measure).

## What v2 does not model — SSOT

- **UI pixels** — dashboard panels have `value` + `badge` metadata; there is no `<canvas>` rendering. Real Grafana pixel differences (font, chart plugin, plot style) belong to a component-test harness (`@kiwa-lab/component`), not observability.
