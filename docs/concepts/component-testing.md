...
import { buildButton, createStoryRegistry, fireEvent } from '@kiwa-test/component';
...
The report is then hand-promoted to `docs/quality-reports/component/<name>.md` when a release cuts. `@kiwa-test/quality-metrics`'s `evaluateReleaseGate` consumes the JSON directly and gates the release on the 7-axis pass.
