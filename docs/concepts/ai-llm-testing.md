...
Only providers whose name starts with `@kiwa-lab/ai-` enter the 11-axis branch. Everyone else still runs on 7 axes with the v1.11 semantics unchanged (see `isAiLlmProvider` in `packages/quality-metrics/src/types.ts`).
...
Accuracy is the hardest of the four axes because it depends on what "close" means for your domain. `@kiwa-lab/quality-metrics` supports three methods with the same 0.80 default threshold but different semantics:
...
- **Skip AI-LLM 11 axes if** your test suite drives a pure logic mock (Supabase Auth, RabbitMQ, Foundry, etc). Even if your app internally calls an LLM, the provider prefix (`@kiwa-lab/auth`, `@kiwa-lab/queue`, etc.) keeps you on the 7-axis branch. Adding AI-LLM axes to non-AI providers is possible via override but generally not useful — the metrics measure LLM behaviour, not auth / queue / contract behaviour.
...
- [`@kiwa-lab/ai-llm` README](https://github.com/cardene777/kiwa/blob/main/packages/ai-llm/README.md) — full API reference for the 4 mocks + fidelity harness
...
- 4 new axes (cost / latency / token / accuracy) turn the 7-axis gate into an 11-axis gate — but only for providers whose name starts with `@kiwa-lab/ai-`. Non-AI providers are unaffected.
