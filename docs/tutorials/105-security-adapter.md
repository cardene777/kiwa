...
A vitest suite wired to `@kiwa-test/security-devsecops` v0.2 adapter interface。 mock adapter (deterministic replay、 常時実行可能) と real adapter (env-gate 通過時のみ実 CLI 呼出隠蔽経路) を pair で使い、 dev-flow の security 4 skill を library 経由で再現性 test 可能にする。
...
pnpm init
pnpm add -D @kiwa-test/security-devsecops@^0.2 vitest typescript @types/node
```
