# Testing Next.js Server Actions with @kiwa-test/nextjs
...
pnpm init -y
pnpm add -D vitest typescript @types/node @kiwa-test/nextjs
```
...
import { invokeServerAction } from '@kiwa-test/nextjs';
...
- **`redirect() is not a function`** — Make sure `next/navigation` resolves to the Next.js 14+ runtime. `@kiwa-test/nextjs` peer-depends on `next@^14 || ^15`.
...
- The App Router variants of the harness cover middleware, RSC, and RSC streaming — see [`@kiwa-test/nextjs`](../../packages/nextjs/README.md).
