...
- **Federation** — `createOidcTrustAnchor({ entity_id })` mints an anchor. `createOidcEntityStatement({ iss, sub, exp?, now? })` mints an entity statement. `resolveOidcTrustChain({ leaf, intermediates, anchor, now? })` walks the chain and returns `{ valid, chain?, anchor?, reason?, reason_code? }` — `reason` surfaces `/no intermediate describes/`, `/expired/`, `/exhausted intermediates/`, or `/cycle detected/`, and `reason_code` pins the failure axis (`broken_link` / `cycle` / `expired_intermediate` / `expired_leaf`) so downstream wrappers can discriminate without substring matching.
