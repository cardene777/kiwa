...
```js
/**
 * A11y (axe-core) config for @kiwa-lab/my-package.
...
{
  "scripts": {
    "test:a11y": "pnpm -r --filter '@kiwa-lab/*' test:a11y"
  }
}
```

The `-r --filter '@kiwa-lab/*'` glob picks up every workspace package named `@kiwa-lab/…` automatically, so as long as your package name follows the convention no additional wiring is required. Add the new package to the `A11Y_PACKAGE_TIER` map in `scripts/check-a11y-gates.mjs` so the release gate reads the right ceiling.

### 4. Seed the baseline on the first run

```bash
pnpm --filter @kiwa-lab/my-package test:a11y
```
...
A package with no runtime DOM produces a `layers-absent` baseline — every layer records `applicable: false` with an explicit reason. This is the expected state for every current `@kiwa-lab/*` package because they are test-adapter infrastructure that emit no runtime DOM.
...
describe('@kiwa-lab/my-package — 13-axis release gate', () => {
