...
- `simulcastLayers` is the video-only field. `getUserMedia` in `@kiwa-lab/realtime` v0.2 produces a video track with 3 default layers (`low: 100 kbps` / `med: 300 kbps` / `high: 900 kbps`) — the exact defaults mediasoup + Janus SFUs negotiate.
