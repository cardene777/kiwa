...
`@kiwa-test/mobile` v0.3 で追加された advanced III 4 axis (fabric + turbo-modules + codegen + new-architecture) を、 React Native 0.76+ New Architecture の semantics を target-neutral に扱う vitest suite。 v1.50 base 3 + v1.51 advanced II 4 と合わせて **11 axis を production layer で使う pattern**、 pair 深度 3 段拡張達成 5 例目。
...
pnpm init
pnpm add -D @kiwa-test/mobile@^0.3 vitest typescript @types/node
```
