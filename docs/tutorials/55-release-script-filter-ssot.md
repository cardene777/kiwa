...
A provider-neutral `ReleaseInvariantsAdapter` with two implementations — a **mock adapter** backed by `@kiwa-test/release-invariants` v0.1's `checkReleaseScriptFilter` + `checkProvenanceFlagAbsence` + `checkGateScriptPackageCoverage` + `buildReleaseInvariantsSummary`, and a **file adapter** stub that reads the real `package.json` under `REPO_ROOT`. Both satisfy the same 4-op contract (`loadPublishable` / `checkFilter` / `checkProvenance` / `checkGateCoverage`), so a fidelity harness can diff them against a repository under audit. This is the exact pattern the release-smoke `release-script-filter.test.ts` (v1.29-1, PR #989) uses to run 40 per-package assertions against the root `package.json` release script.
...
pnpm init
pnpm add -D @kiwa-test/release-invariants@^0.1 vitest typescript @types/node
```
...
### 3. Write the mock adapter using `@kiwa-test/release-invariants`
...
import { buildReleaseInvariantsSummary } from '@kiwa-test/release-invariants';
import type { ReleaseInvariantsAdapter } from './interface.js';
