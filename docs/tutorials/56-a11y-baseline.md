...
pnpm init
pnpm add -D @kiwa-test/a11y@^1.1 @kiwa-test/quality-metrics@^0.4 \
...
`axe-core` is a peer dependency of `@kiwa-test/a11y` — it must be installed alongside the adapter or `runAxe` throws at first call.
...
import { runLayerHarness } from '@kiwa-test/a11y';
import { renderCounter } from '../../src/counter.js';
...
`tests/a11y/gate.a11y.test.ts` — feed the baseline totals into `@kiwa-test/quality-metrics` v0.4 and fail the vitest suite when any impact ceiling is breached.
