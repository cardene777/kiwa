...
pnpm init -y
pnpm add -D vitest typescript @types/node @kiwa-test/mcp
```
...
`@kiwa-test/mcp` also ships `registerAllFixtureTools(server)` which registers all 5 fixture tools (echo / calc / weather / search / db-query) in one call — use the per-tool form when the test suite should only expose a subset.
...
import { connectClientToServer, InMemoryTransport, McpClient } from '@kiwa-test/mcp';
...
`@kiwa-test/mcp` v0.1 covers the tool-use half of the Model Context Protocol spec. The remaining ops (resources / prompts / sampling / logging) land in a future release.
...
import { connectClientToServer } from '@kiwa-test/mcp';
...
- [`@kiwa-test/mcp` on npm](https://www.npmjs.com/package/@kiwa-test/mcp)
