...
- **Bi-stream window semantics**. `writeStream` snapshots `windowRemaining` **before** the write, decides `backpressure` from that snapshot, then awaits the write. The `@kiwa-lab/realtime` bi mock refills the window after backpressure via an artificial delay — so `remainingWindow` after a backpressure event is the fresh window, not the drained value. Behavior tests that assert `remainingWindow > 0` post-backpressure land the same way in the mock and in real Chrome + aioquic.
