...
A vitest suite wired to `@kiwa-test/realtime` v0.3 that models the 3 pieces of a real adaptive media consumer stack — SVC layer selection with scalability mode (L1T1 → L3T3), WebCodecs decoder with key/delta frame handling + reorder buffer, and MoQ datagram delivery with FEC recovery.
...
```ts
import { describe, expect, it } from 'vitest';
import { createWebCodecsDecoderMock } from '@kiwa-test/realtime';
...
```ts
import { describe, expect, it } from 'vitest';
import { createMoqDatagramMediaMock } from '@kiwa-test/realtime';
