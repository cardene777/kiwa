...
A vitest suite wired to `@kiwa-lab/desktop` v1.0 (invoke-cache layer、 v1.67 で **depth-6 pattern 2 例目確定 candidate** = Mobile v1.55 depth-5 + Desktop v1.61 depth-5 + Desktop v1.67 depth-6 の 2 例目化、 **systematic pattern 42 度目適用**、 **45 milestone streak**、 4 PR rhythm 14 milestone 連続 = 56 PR 連続)、 v0.9 で確立した probeAndInvoke 統合経路 の 上位 layer として、 同一 axis + target + args-hash の invoke を 2 回目以降 skip、 LRU + TTL 両立 in-memory cache で release cycle 全体 の 実 spawn cost を削減。

## Prerequisites

- Node.js ≥ 20 + pnpm
- `@kiwa-lab/desktop` v1.0 (`pnpm add -D @kiwa-lab/desktop@^1.0`)
...
```ts
import { InvokeCache } from '@kiwa-lab/desktop';
...
```ts
import { withCache } from '@kiwa-lab/desktop';
...
```ts
import { buildCacheKey } from '@kiwa-lab/desktop';
...
- v1.67 で v1.0 の 実運用 経路 が 確立 = 次 v1.68 は depth-6 実運用 継続 or v2.0 rename milestone (@kiwa-lab/* → @kiwa-lab/*)
