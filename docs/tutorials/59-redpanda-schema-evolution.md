...
import { createRedpandaSchemaEvolution } from '@kiwa-test/streaming';
...
The 5 evolution pieces the tutorial covered — BACKWARD, FORWARD, FULL, subject naming, references — are the ones every Redpanda / Confluent Schema Registry deployment hits within the first 3 months. `@kiwa-test/streaming` v0.3 models them with structural markers so tests iterate in milliseconds; the fidelity harness (see the concept doc) runs the same assertions against real Redpanda 23+ testcontainers under `KIWA_MODE=real REDPANDA_KEY=...`, which is what the v1.31-3 dogfood app does.
