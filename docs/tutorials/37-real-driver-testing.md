...
pnpm init
pnpm add -D @kiwa-lab/auth@^0.5 vitest typescript @types/node testcontainers
```
...
|---|---|---|
| `mock only` | `KIWA_MODE` unset | Pure `@kiwa-lab/auth` mock, 0 container, sub-100 ms per test |
...
- **Fidelity report shows drift.** The mock is out of date — bump `@kiwa-lab/auth` to the latest patch. If drift persists, file an Issue with the failing axis name; the concept doc lists which axes are load-bearing for each protocol.
