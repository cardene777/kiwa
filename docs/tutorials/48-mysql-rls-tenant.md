...
A vitest suite wired to `@kiwa-test/orm` v0.9 that walks the RLS (row-level security) axis end-to-end for a multi-tenant SaaS on MySQL 8. You will install a per-table policy, filter a tenant's read / write path, exercise a `bypass_rls` superuser role under audit, and record every access in an audit log. The exact pattern that `examples/dogfood-mysql-rls-tenant-app` (Nuxt 3 + Prisma + MySQL 8) uses — same `createRlsSession` + `installPolicy` + `filterTenant` + `bypassRls` + `logAudit` primitives, same state-machine guards, same tenant isolation invariants. You leave this tutorial with a runnable multi-tenant test and a working audit trail for any RLS flow you point it at.
...
import { createRlsSession } from '@kiwa-test/orm';
...
```ts
import { describe, expect, it } from 'vitest';
import { createRlsSession, installPolicy } from '@kiwa-test/orm';
