...
For a package with 5-10 exported functions, pick the 2-3 that downstream users call most. In `@kiwa-test/core`, that is `parseSpec` (spec markdown → structured spec) + `createPool` (async resource pool). In `@kiwa-test/dapp`, it is `contractCall` + `readStorageSlot`. In `@kiwa-test/edge`, it is `invokeEdgeHandler` + `kvRead` + `kvWrite`.
...
```bash
pnpm --filter @kiwa-test/my-package test:perf
```
...
{
  "scripts": {
    "test:perf": "pnpm -r --filter '@kiwa-test/*' test:perf"
  }
}
```

The `-r --filter '@kiwa-test/*'` glob picks up every workspace package named `@kiwa-test/…` automatically, so as long as your `package.json` name follows the convention, no additional wiring is required.
...
The v1.13 milestone seeded `@kiwa-test/ai-llm`, `@kiwa-test/quality-metrics`, and `@kiwa-test/realtime` with the initial `measure` + `saveBaseline` pattern. v1.25 generalises the pattern to every other package. If you want to see the exact shape of a production-grade perf suite, read.
