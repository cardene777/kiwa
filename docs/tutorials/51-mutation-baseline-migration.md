...
- **List every implementation file, including the provider-specific ones.** It is tempting to leave out a file that needs a live container or socket, on the grounds that its mutants will all be `no-coverage`. Three packages carried exclusions of that shape, covering eight files between them, and every one was wrong when measured: `cache` (#1967 — three `testcontainers-*` files, 330 of 345 mutants covered), `queue` (#1980 — `testcontainers-queue.js` 234 covered and `rabbitmq/testcontainers-rabbitmq.js` 40), and `realtime` (#1980 — `pusher.js` 119, `socketio.js` 137, `report.js` 67). A file needing a live server for *some* of its assertions still runs plenty without one. `covered MSI` — the number the gate reads — excludes no-coverage mutants from its denominator, so listing such a file cannot lower it; only `total MSI` moves. If you believe a file genuinely has no covered mutants, run it and record the count.
