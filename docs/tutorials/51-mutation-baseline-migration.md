...
- **List every implementation file, including the provider-specific ones.** It is tempting to leave out a file that needs a live container or socket, on the grounds that its mutants will all be `no-coverage`. Four packages carried an exclusion of that shape and all four were wrong when measured: `cache` (#1967, 330 of 345 covered), `queue` and `realtime` (#1980, 234 / 119 / 137 / 67 covered). A file needing a live server for *some* of its assertions still runs plenty without one. `covered MSI` — the number the gate reads — excludes no-coverage mutants from its denominator, so listing such a file cannot lower it; only `total MSI` moves. If you believe a file genuinely has no covered mutants, run it and record the count.
