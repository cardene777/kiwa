...
- **List every implementation file, including the provider-specific ones.** It is tempting to leave out a file that needs a live container or socket, on the grounds that its mutants will all be `no-coverage`. Three packages carried exclusions of that shape, covering eight files between them, and every one was wrong when measured: `cache` (#1967 — three `testcontainers-*` files, 328 covered mutants), `queue` (#1980 — `testcontainers-queue.js` 234 and `rabbitmq/testcontainers-rabbitmq.js` 40), and `realtime` (#1980 — `pusher.js` 115, `socketio.js` 137, `report.js` 67). Those are covered mutants as the gate counts them — killed + survived + timeout — with no-coverage and error mutants both outside the denominator. A file needing a live server for *some* of its assertions still runs plenty without one.

  **Expect the aggregate to move, in either direction.** Five of the eight scored below their package and three above: `realtime`'s three came in at 64.35 / 64.96 / 56.72 against a 69.41 aggregate and `queue`'s `rabbitmq/testcontainers-rabbitmq.js` at 30.00 against 78.37, while `cache`'s KeyDB and Memcached files scored 92.31 and 89.66 against 78.77. So the exclusions were sometimes holding a number up and sometimes holding it down — which is the point: a score computed over the part of a package someone chose to look at is not a score about the package, whichever way it leans. (A file whose mutants are *all* no-coverage changes neither `totalMsi` nor `coveredMsi`, since `summariseReport` leaves no-coverage out of both denominators. None of the eight was that.) If you believe a file genuinely has no covered mutants, run it and record the count.
...
- [`packages/cache/stryker.config.mjs`](https://github.com/cardene777/kiwa/blob/main/packages/cache/stryker.config.mjs) — an exclusion that did not survive measurement. `testcontainers-cache.js` was left out of `mutate` on the grounds that its assertions need a live container (a v1.27-3 sweep saw 0 covered mutants). #1967 measured 328 covered mutants across the three `testcontainers-*` files and reversed it. Exclude a file when a run shows it has no covered mutants, and record which run — not on the shape of its name.
