...
A single vitest test file that drives a small collaboration board through **three Ably surfaces** — a cursor position broadcast on a board channel, a 60 fps client-side throttle on mass mousemove events, and a `channel.history({ limit })` rewind for a late-joining user — using `@kiwa-lab/realtime`'s `createAblyMock`. The same test file also works against real Ably when `ABLY_API_KEY` is set, so the fidelity harness can diff mock vs real behaviour.
