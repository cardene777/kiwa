...
pnpm add -D @playwright/test @kiwa-test/dapp typescript @types/node
```
...
Create the Playwright fixture at `tests/fixture.ts` that layers on top of `@kiwa-test/dapp`.
...
```ts
import { snapshotChain, revertChain } from '@kiwa-test/dapp';
...
## What v1.18 brings to `@kiwa-test/dapp`

The v1.10 `@kiwa-test/dapp` package shipped the wallet-injection + `dappE2eTest` fixture. v1.18 adds two primitives on top.
