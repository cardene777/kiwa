...
`@kiwa-lab/mobile` v0.6 で v0.5 spawn stub 契約層が **実 child_process.spawn 実行** に置換された経路を、 dry-run + DI + env sanitize の 3 pattern で決定的に扱う vitest suite。 **depth-5 pattern 実装完成**、 v0.5 shape 契約 preserving で backward compat 絶対維持。
...
pnpm init
pnpm add -D @kiwa-lab/mobile@^0.6 vitest typescript @types/node
```
...
import { describe, expect, it } from 'vitest';
import { invokeMobileCliWith, type SpawnFn } from '@kiwa-lab/mobile';
...
```ts
import { describe, expect, it } from 'vitest';
import { sanitizeEnv } from '@kiwa-lab/mobile';
