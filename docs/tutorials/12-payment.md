...
pnpm init -y
pnpm add -D vitest typescript @types/node @kiwa-lab/payment
```
...
The 3 providers ship slightly different webhook payload shapes. `@kiwa-lab/payment` normalises them via the shared engine so your handler code reads identical `PaymentWebhookEvent` fields — but the raw JSON body differs so signature-verify tests exercise the actual bytes each provider sends.
...
`@kiwa-lab/payment` lets you test **all three at once** so migrating between providers (or supporting multiple simultaneously) doesn't force a test rewrite.
