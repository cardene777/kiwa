/**
 * Tiny HTTP server that mounts the mtls / zero-trust / broker handlers
 * directly. The Playwright e2e specs spawn this server so Chromium can
 * drive the full mTLS + Zero-trust ceremony from a real browser origin,
...
import {
  handleMtlsRequest,
  validateMtlsRequest,
} from '../app/mtls/route.js';
import {
  handleZtRequest,
  validateZtRequest,
} from '../app/zero-trust/route.js';
import {
  handleBrokerRequest,
  validateBrokerRequest,
} from '../app/broker/route.js';
...
async function dispatch(
  ...
  if (route === 'mtls') {
    const parsed = validateMtlsRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handleMtlsRequest(adapter, parsed.value);
  }
  if (route === 'zero-trust') {
    const parsed = validateZtRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handleZtRequest(adapter, parsed.value);
  }
  const parsed = validateBrokerRequest(body);
  if (!parsed.ok) {
    return { ok: false, errorKind: parsed.errorKind };
  }
  return handleBrokerRequest(adapter, parsed.value);
}
