 ...
 * `@kiwa-test/security` v0.2 mtls + zero-trust helpers track the real
 * Istio + OPA broker behaviour closely enough to be trusted as a mock in
 * unit tests. The report tracks the same 15 ops the adapter contract
 ...
 * accuracy) do not apply. mTLS / zero-trust / broker latency samples feed
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the mTLS + Zero-trust +
   * broker surface — 15 ops (startMtls / completeHandshake / verifyPin /
   * verifyOcsp / checkCtLog / closeMtls / startZeroTrust / evaluatePosture
   * / scoreRisk / requestJit / enforceMicroSegment / closeZeroTrust /
   * startBroker / decideBroker / closeBroker) are the AC scope of Issue
   * CAR-864.
   */
