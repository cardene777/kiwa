...
- `KIWA_MODE=mock` — `@kiwa-lab/auth` `setupOAuth21Env` + `createAuthorizationServer` deterministic mock. Always runs.
...
src/
  adapters/
    ...
    mock.ts            # makeMockAdapter — @kiwa-lab/auth setupOAuth21Env + createAuthorizationServer
  ...
  lib/
    pkce.ts            # PKCE helpers (createPkceChallenge / deriveChallengeS256 / verifyChallenge / assertVerifierFormat / assertMethodAllowed) — thin wrapper around @kiwa-lab/auth's PKCE primitives
  lib/
    dpop.ts            # DPoP helpers (parseDpopHeader / assertDpopHeaderShape / verifyDpopProofBinding / computeJkt) — thin wrapper around @kiwa-lab/auth's DPoP primitives
    refresh-rotation.ts # Refresh-rotation helpers (rotateAndMint / classifyRefreshTokenError / RefreshRotationError) — thin wrapper around @kiwa-lab/auth's rotateRefreshToken
...
| axis | mock (`@kiwa-lab/auth`) | real (oauth2-mock-server, gated by `OAUTH21_BOOTSTRAP=1`) | assertion |
|---|---|---|---|
...
| axis | mock (`@kiwa-lab/auth`) | real (oauth2-mock-server) | assertion |
|---|---|---|---|
