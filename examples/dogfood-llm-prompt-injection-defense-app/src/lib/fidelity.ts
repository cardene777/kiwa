 ...
 * `@kiwa-lab/ai-llm` v0.4 prompt-injection + guardrails helpers track
 ...
 *  - latency = same distribution as perf (in-process classifier)
 *  - token = input length divided by 4 (Claude tokenizer proxy)
 *  - accuracy = 1.0 (deterministic classifier, mock=real by design)
 * Once the real Anthropic driver lands (follow-up milestone), each
 * proxy is replaced by the corresponding real measurement.
 */
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the injection + guardrails +
   * pipeline surface — 14 ops (startInjection / analyzeInjection /
   * classifyDirect / blockJailbreak / blockRoleHijack / closeInjection /
   * startGuardrail / validateSchema / blockToxicity / redactPii /
   * checkConstitutional / closeGuardrail / startPipeline / runPipeline)
   * are the AC scope of Sub-Issue CAR-845.
   */
...
export function runFidelityHarness(input: FidelityRunInput): FidelityRunOutput {
  ...
  // Deterministic defense — cost is $0 per request because the classifier
  // is in-process. Populate 1 sample per op so cost.perRequestUsd is defined.
  ...
  // module it wraps, so we score 1.0. Once the real Anthropic driver
  // lands, actual mock-vs-real cosine similarity replaces this constant.
  ...
  const report = assembleReport({
    ...
    accuracy: accuracyFromSamples({
      ...
      method: 'deterministic-classifier-parity',
    }),
...
export async function runAdapterMatrix(input: {
  mock: LlmSafetyAdapter;
  real: LlmSafetyAdapter;
  run: (adapter: LlmSafetyAdapter) => Promise<number[]>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
