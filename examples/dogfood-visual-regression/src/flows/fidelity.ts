 ...
 * `@kiwa-test/component` `createChromaticVisualMock` tracks real Chromatic
 ...
 * accuracy) do not apply — Chromatic is a snapshot + diff surface, not a
 * token-priced generative surface. Latency samples still feed `perf.p95Ms`
 * (the seed + capture + review round-trip axis) so visual-regression
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock ops the app exercised vs the real Chromatic adapter
   * surface — 4 ops (seedBaselines / captureOne / captureAll / review) are
   * the AC scope of Issue #766.
   */
...
/**
 * Drive the visual-regression flow against both adapters, capturing traces
 * + rolled up latency samples. The real adapter may be skipped — divergences
 * are counted from the trace either way. Mock failures propagate: the release
 * gate must never spuriously pass on a partial trace.
 */
export async function runAdapterMatrix(input: {
  mock: VisualRegressionAdapter;
  real: VisualRegressionAdapter;
  run: (adapter: VisualRegressionAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Real path is allowed to throw when CHROMATIC_PROJECT_TOKEN is unset
  // (SkippedError) or when the live driver is not implemented — the
  // divergence is recorded in the trace by the adapter, and the harness
  // proceeds with the mock's measurements. Only real-mode exceptions are
  // swallowed.
  try {
    await input.run(input.real);
  } catch {
    // Trace already reflects the failure (CHROMATIC_REAL_ENV_MISSING /
    // CHROMATIC_LIVE_NOT_IMPLEMENTED). Downstream divergences count is
