 ...
 * `@kiwa-test/ai-llm` v0.4 hallucination + llm-eval helpers track the
 * real OpenAI Chat Completions behaviour closely enough to be trusted as
 * a mock in unit tests. The report tracks the same 15 ops the adapter
 ...
 * deterministic scoring (not a real LLM call), so the 4 sub axes are
 * populated with quality-detection-specific proxies —
 *  - cost = $0 per request (no upstream OpenAI call)
 *  - latency = same distribution as perf (in-process scorer)
 *  - token = input length divided by 4 (GPT tokenizer proxy)
 *  - accuracy = 1.0 (deterministic scorer, mock=real by design)
 * Once the real OpenAI driver lands (follow-up milestone), each proxy is
 * replaced by the corresponding real measurement.
 */
...
import type { LlmQualityAdapter, TraceEvent } from '../adapters/interface.js';
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the hallucination +
   * eval + pipeline surface — 15 ops (startHallucination /
   * scoreSelfConsistency / checkFactuality / verifyCitation /
   * scoreConfidence / closeHallucination / startEval / judgeCandidates
   * / applyRubric / rankPreference / updateElo / closeEval /
   * startPipeline / runPipeline) are the AC scope of Issue CAR-848.
   */
...
export function runFidelityHarness(input: FidelityRunInput): FidelityRunOutput {
  ...
  // Deterministic scorer — cost is $0 per request because the scorer is
  // in-process. Populate 1 sample per op so cost.perRequestUsd is
  // defined.
  const costSamples = input.mockLatencySamplesMs.map(() => 0);
  // token proxy — chars / 4 ≈ GPT tokenizer, keeps totalTokens sane.
  ...
  // module it wraps, so we score 1.0. Once the real OpenAI driver lands,
  // actual mock-vs-real cosine similarity replaces this constant.
  ...
  const report = assembleReport({
    ...
    accuracy: accuracyFromSamples({
      ...
      method: 'deterministic-scorer-parity',
    }),
 ...
 * samples. The real adapter may be skipped — divergences are counted
 * from the trace either way.
 */
export async function runAdapterMatrix(input: {
  mock: LlmQualityAdapter;
  real: LlmQualityAdapter;
  run: (adapter: LlmQualityAdapter) => Promise<number[]>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
