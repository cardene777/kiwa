 ...
 * `@kiwa-test/ai-llm` v0.4 agent-orchestration helpers track the real
 ...
 *  - token = intent + plan node length divided by 4 (Claude tokenizer proxy)
 *  - accuracy = 1.0 (deterministic ranker, mock=real by design)
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the agent + pipeline
   * surface — 8 ops (startAgent / reactStep / expandToT / reflect /
   * selectTool / closeAgent / startPipeline / runPipeline) are the AC
   * scope of Sub-Issue CAR-858.
   */
...
export function runFidelityHarness(input: FidelityRunInput): FidelityRunOutput {
  ...
  // Deterministic planner — cost is $0 per request because the ranker
  // + state machine are in-process. Populate 1 sample per op so
  // cost.perRequestUsd is defined.
  ...
  const report = assembleReport({
    ...
    accuracy: accuracyFromSamples({
      ...
      method: 'deterministic-planner-parity',
    }),
...
export async function runAdapterMatrix(input: {
  mock: LlmAgentAdapter;
  real: LlmAgentAdapter;
  run: (adapter: LlmAgentAdapter) => Promise<number[]>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
