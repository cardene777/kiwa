...
Dogfood app 2 (v1.11-3) — order-processing worker driven by `@kiwa-test/queue` RabbitMQ basic + advanced adapters (`KIWA_MODE=mock`) vs a live rabbitmq:3-management broker (`KIWA_MODE=real`), so behavioural fidelity can feed `@kiwa-test/quality-metrics`.
