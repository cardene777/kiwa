 ...
 * `@kiwa-test/component` `createStoryRegistry` mock + MDX registry +
 ...
 *
 * v1.34-4 (Issue #1051) — 8-op contract (registerAll / listStories /
 * resolveArgs / mount / renderMdx / runInteraction / runA11y /
 * computeCoverage).
 */
 ...
 * from the trace either way. Mock failures propagate.
 */
export async function runAdapterMatrix(input: {
  mock: StorybookMdxAdapter;
  real: StorybookMdxAdapter;
  run: (adapter: StorybookMdxAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  try {
    ...
    // Trace reflects the failure. Downstream divergences count is computed
