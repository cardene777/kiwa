/**
 * Tiny HTTP server that mounts the treasury / card / kyc handlers
 * directly. The Playwright e2e specs spawn this server so Chromium can
 * drive the full embedded finance ceremony from a real browser origin,
...
import {
  ...
  validateTreasuryRequest,
} from '../app/treasury/route.js';
import {
  handleCardRequest,
  validateCardRequest,
} from '../app/card/route.js';
import {
  handleKycRequest,
  validateKycRequest,
} from '../app/kyc/route.js';
...
async function dispatch(
  ...
  if (route === 'treasury') {
    const parsed = validateTreasuryRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handleTreasuryRequest(adapter, parsed.value);
  }
  if (route === 'card') {
    const parsed = validateCardRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handleCardRequest(adapter, parsed.value);
  }
  const parsed = validateKycRequest(body);
  if (!parsed.ok) {
    return { ok: false, errorKind: parsed.errorKind };
  }
  return handleKycRequest(adapter, parsed.value);
}
