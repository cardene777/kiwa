...
Dogfood app 1 (v1.12-2) — an Anthropic Messages API chatbot that exercises **streaming + system prompt + tool_use + cost tracking** across a provider-neutral interface so `@kiwa-test/ai-llm`'s Anthropic mock can be measured against a real Anthropic call. The resulting fidelity report feeds `@kiwa-test/quality-metrics` 11-axis release gate.

## Modes

- `KIWA_MODE=mock` (default) — driven by `makeMockAdapter()` (`@kiwa-test/ai-llm` `createAnthropicMock`, deterministic response bank)
...
Because the provider string is `@kiwa-test/ai-llm/anthropic-chatbot`, `evaluateReleaseGate` runs the AI-LLM branch (11 axes = common 7 + AI-LLM 4).
