...
/**
 * Mock adapter — drives the `@kiwa-lab/ai-llm` Anthropic mock so the same
...
export function makeMockAdapter(): ChatbotAdapter {
  const client = createAnthropicMock({
    ...
    defaultResponse:
      'This is a deterministic mock reply produced by @kiwa-lab/ai-llm createAnthropicMock. The dogfood harness diffs this against a real Anthropic response.',
