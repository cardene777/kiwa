...
describe('fidelity harness — 3 platform × 3 stage grid', () => {
  ...
  it('drift 9 for every scenario', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    expect(report.driftCount).toBe(9);
  });

  it('step-up stage op sequence', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    const row = report.rows.find((r) => r.scenario.stage === 'step-up');
  ...
  it('continuity stage op sequence', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    const row = report.rows.find((r) => r.scenario.stage === 'continuity');
    expect(row?.mockTrace.map((s) => s.op)).toEqual([
      'startContinuityFlow',
      'reauthSeamlessly',
      'rotateRefreshToken',
  ...
  it('hijack stage op sequence', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    const row = report.rows.find((r) => r.scenario.stage === 'hijack');
    expect(row?.mockTrace.map((s) => s.op)).toEqual([
      'startHijackWatchFlow',
      'reportDrift',
      'cascadeLogout',
      'closeHijackWatch',
    ]);
  ...
  it('mock trace 4 op per row', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    for (const row of report.rows) {
