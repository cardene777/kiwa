...
describe('fidelity harness — 3 platform × 3 stage grid', () => {
  ...
  it('drift is 9 (all rows differ)', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    expect(report.driftCount).toBe(9);
  });

  it('risk stage op sequence', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    const row = report.rows.find((r) => r.scenario.stage === 'risk');
  ...
  it('telemetry stage op sequence', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    const row = report.rows.find((r) => r.scenario.stage === 'telemetry');
    expect(row?.mockTrace.map((s) => s.op)).toEqual([
      'startTelemetryFlow',
      'recordAttemptOp',
      'detectAbuseOp',
