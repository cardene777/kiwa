...
import type { HonoAdapter, TraceEvent } from '../adapters/interface.js';

/**
 * Fidelity harness — compares a Hono / Workers run under {@link makeMockAdapter}
 ...
 * `@kiwa-test/hono` mock tracks real Cloudflare Workers + Hono closely
 * enough to be trusted in unit tests — this harness is where that trust
 * is measured.
 *
 ...
 * / token / accuracy) do not apply — Hono route dispatch and CF bindings
 * are not token-priced generative surfaces. Latency samples still feed
 * `perf.p95Ms` so route invoke + binding round-trip performance stays
...
export async function runAdapterMatrix(input: {
  mock: HonoAdapter;
  real: HonoAdapter;
  run: (adapter: HonoAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Real path is allowed to throw when CF_ACCOUNT_ID is unset
  ...
  // proceeds with the mock's measurements.
  try {
