...
A Next.js 15.4 + React 19.1 App Router app that drives Server Action + form action + useFormStatus + useOptimistic + revalidatePath + revalidateTag + redirect + progressive enhancement across a provider-neutral `ServerActionAdapter`. Both mock (`@kiwa-lab/component` v0.3 form-action-advanced semantics + `@kiwa-lab/nextjs` v1.2 server-action-advanced semantics) and real (Playwright + Chromium headless when `SERVER_ACTION_BROWSER_READY=1`) implementations satisfy the same 15-op contract so the fidelity harness can diff them side by side.
...
The vitest suite writes `quality-report/fidelity-latest.md` + `quality-report/fidelity-latest.json` that `@kiwa-lab/quality-metrics` picks up for the 13-axis release gate. The doc counterpart lives at `docs/quality-reports/component/nextjs-server-action-app.md`.
