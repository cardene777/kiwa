/**
 * Tiny HTTP server that mounts the subscribe / like / login handlers
 * directly. The Playwright e2e specs spawn this server so Chromium can
 * drive the full Server Action ceremony from a real browser origin,
...
} from '../app/subscribe/route.js';
import {
  handleLikeRequest,
  validateLikeRequest,
} from '../app/like/route.js';
import {
  handleLoginRequest,
  validateLoginRequest,
} from '../app/login/route.js';
...
async function dispatch(
  ...
  if (route === 'subscribe') {
    ...
    return await handleSubscribeRequest(adapter, parsed.value);
  }
  if (route === 'like') {
    const parsed = validateLikeRequest(body);
    if (!parsed.ok) return { ok: false, errorKind: parsed.errorKind };
    return await handleLikeRequest(adapter, parsed.value);
  }
  const parsed = validateLoginRequest(body);
  if (!parsed.ok) return { ok: false, errorKind: parsed.errorKind };
  return await handleLoginRequest(adapter, parsed.value);
}
