 ...
 * real Next.js 15.4 + React 19.1 App Router Server Action behaviour
 * closely enough to be trusted as a mock in unit tests. The report tracks
 * the same 15 ops the adapter contract exposes so any divergence surfaces
 * the op that broke.
 *
 ...
 * latency / token / accuracy) do not apply. Subscribe / like / login
 * latency samples feed `perf.p95Ms` so end-user timings stay visible.
 */
...
import type { ServerActionAdapter, TraceEvent } from '../adapters/interface.js';
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the Server Action + form
   * action + revalidation + redirect + progressive enhancement surface —
   * 15 ops (startSubscribe / submitSubscribe / revalidateSubscribePath /
   * startLike / markLikePending / applyOptimisticLike / submitLike /
   * revalidateLikeTag / resolveLike / startLogin / enhanceLogin /
   * markLoginPending / submitLogin / redirectLogin / resolveLogin) are the
   * AC scope of Sub-Issue CAR-786.
   */
   ...
   * regress before the app declares an axe-config baseline. The Server
   * Action dogfood maps to the SaaS tier (strict 0/0/0) by default
...
export async function runAdapterMatrix(input: {
  mock: ServerActionAdapter;
  real: ServerActionAdapter;
  run: (adapter: ServerActionAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  const samples = [
    ...m.subscribeLatencySamplesMs,
    ...m.likeLatencySamplesMs,
    ...m.loginLatencySamplesMs,
  ];
  return {
