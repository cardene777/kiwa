 ...
 * `MYSQL_ENV_MISSING` / `REAL_ADAPTER_NOT_IMPLEMENTED` in real is a
...
export async function runAdapterMatrix(input: {
  mock: MysqlRlsTenantAdapter;
  real: MysqlRlsTenantAdapter;
  run: (adapter: MysqlRlsTenantAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  perfSamplesMs: number[];
