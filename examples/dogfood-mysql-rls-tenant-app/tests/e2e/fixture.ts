/**
 * Shared fixture for the Playwright e2e specs (v1.32-3) — spins up a small
 ...
 * on the JSON responses; no UI is rendered because the MySQL dogfood is a
 * headless surface.
 */
...
import type { MysqlRlsTenantAdapter, OrganizationRow } from '../../src/adapters/interface.js';
...
type PartialOrg = Partial<OrganizationRow>;

const ROUTES: Readonly<Record<string, RouteHandler>> = {
  '/tenant-injection': async (adapter, body) => {
    const orgs = ((body as { orgs?: unknown }).orgs ?? []) as readonly PartialOrg[];
    const normalized = orgs.map((o) => sampleOrgRow(o));
    return adapter.driveTenantInjection({ orgs: normalized });
  },
  '/cross-tenant-refuse': async (adapter, body) => {
    const parsed = body as {
      context?: { tenantId?: string; actorId?: string };
      orgs?: readonly PartialOrg[];
      intruderTenantId?: string;
    };
    const orgs = (parsed.orgs ?? []).map((o) => sampleOrgRow(o));
    return adapter.driveCrossTenantRefuse({
      context: {
        tenantId: parsed.context?.tenantId ?? 'tenant-a',
        actorId: parsed.context?.actorId ?? 'actor-e2e',
      },
      orgs,
      intruderTenantId: parsed.intruderTenantId ?? 'tenant-b',
    });
  },
  '/bypass-audit': async (adapter, body) => {
    const parsed = body as {
    ...
    const ops = (parsed.ops ?? []).map((op) => ({
      tenantId: op.tenantId ?? 'tenant-a',
      operation: op.operation ?? ('read' as const),
    }));
    return adapter.driveBypassAudit({
      supportRoleId: parsed.supportRoleId ?? 'support-e2e',
      reason: parsed.reason ?? 'e2e',
      ops,
    });
  },
  '/audit-integrity': async (adapter, body) => {
    const parsed = body as { tamperAtIndex?: number };
    return adapter.driveAuditIntegrity(
      parsed.tamperAtIndex === undefined
        ? {}
        : { tamperAtIndex: parsed.tamperAtIndex },
    );
  },
...
export interface AdapterServerHandle {
  ...
  readonly adapter: MysqlRlsTenantAdapter;
}
...
export async function bootAdapterServer(): Promise<AdapterServerHandle> {
  ...
  const server: Server = createServer(async (nodeReq, nodeRes) => {
    ...
    if (!handler || nodeReq.method !== 'POST') {
      ...
      nodeRes.end(
        '<!doctype html><html><head><title>mysql-rls-tenant-app</title></head><body></body></html>',
      );
      return;
    }
    try {
