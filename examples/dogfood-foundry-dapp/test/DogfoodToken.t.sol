...
contract DogfoodTokenTest is Test {
    ...
    // 未覆と判定した 11 件 (TC-001 / 002 / 004 / 005 / 006 / 007 / 009 / 011 / 012 / 013 / 014)。
    // spec = tests/spec/contract/test-spec-dogfood-token.ja.md
    //
    // 既存 3 件 (TC-003 / 008 / 010 に対応) は中身を読んで重複と判断したため書いていない。
    ...
    function test_TC004_transferEmitsTransferEvent() public {
    ...
    function test_TC005_transferReturnsTrue() public {
        vm.prank(alice);
        assertTrue(token.transfer(bob, 100e18));
    }

    function test_TC006_approveSetsAllowance() public {
        vm.prank(alice);
        token.approve(bob, 500e18);
        assertEq(token.allowance(alice, bob), 500e18);
    }

    function test_TC007_approveEmitsApprovalEvent() public {
        vm.expectEmit(true, true, false, true);
    ...
    function test_TC009_transferFromReducesAllowance() public {
        vm.prank(alice);
        token.approve(bob, 500e18);
        vm.prank(bob);
        token.transferFrom(alice, bob, 400e18);
    ...
    function test_TC011_transferFromRevertsOnInsufficientBalance() public {
        vm.prank(alice);
    ...
    function test_TC012_transferFromRevertsOnInsufficientAllowance() public {
        vm.prank(alice);
    ...
    function test_TC013_transferAcceptsExactBalance() public {
        vm.prank(alice);
        token.transfer(bob, 1_000e18);
        assertEq(token.balanceOf(alice), 0);
    }

    function test_TC014_transferFromAcceptsExactAllowance() public {
        vm.prank(alice);
        token.approve(bob, 500e18);
        vm.prank(bob);
