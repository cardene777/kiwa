 ...
 * `@kiwa-lab/observability` v2.2 LLM-obs helpers track the real OpenAI
 ...
 * AI-LLM 4 axes (cost / latency / token / accuracy) do not apply. Token
 * / prompt / budget latency samples feed `perf.p95Ms` so operator timings
 * stay visible.
 */
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the token + prompt +
   * budget surface — 10 ops (startToken / countTokens / closeToken /
   * startPrompt / logPrompt / flagHallucination / closePrompt /
   * startBudget / checkBudget / closeBudget) are the AC scope of Issue
   * CAR-1048.
   */
   ...
   * regress before the app declares an axe-config baseline. The LLM-obs
