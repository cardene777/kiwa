/**
 * Tiny HTTP server that mounts the tokens / prompts / budget handlers
 * directly. The Playwright e2e specs spawn this server so Chromium can
 * drive the full LLM observability ceremony from a real browser origin,
...
} from '../app/tokens/route.js';
import {
  handlePromptRequest,
  validatePromptRequest,
} from '../app/prompts/route.js';
import {
  handleBudgetRequest,
  validateBudgetRequest,
} from '../app/budget/route.js';
import type { LlmOpsAdapter } from '../adapters/interface.js';
...
async function dispatch(
  ...
  if (route === 'tokens') {
    ...
    return handleTokenRequest(adapter, parsed.value);
  }
  if (route === 'prompts') {
    const parsed = validatePromptRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handlePromptRequest(adapter, parsed.value);
  }
  const parsed = validateBudgetRequest(body);
  if (!parsed.ok) {
    return { ok: false, errorKind: parsed.errorKind };
  }
  return handleBudgetRequest(adapter, parsed.value);
}
