...
import type { CursorBoardAdapter, TraceEvent } from '../adapters/interface.js';
 ...
 *
 * The dogfood app is the source of truth for whether the kiwa Ably mock
 * tracks real Ably behaviour closely enough to be trusted as a mock in unit
 ...
 * token / accuracy) do not apply — Ably is a socket / pub-sub primitive,
 * not a token-priced generative call. Latency samples still feed
 * `perf.p95Ms` (the socket round-trip axis) so realtime performance stays
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the real Ably surface — 4
   * methods (joinBoard / moveCursor / rewindHistory / getPresence) are the
   * AC scope of Issue #712.
   */
...
export async function runAdapterMatrix(input: {
  mock: CursorBoardAdapter;
  real: CursorBoardAdapter;
  run: (adapter: CursorBoardAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // the socket round-trip. This mirrors what a user-facing collab client
  // sees end to end (attach + cursor broadcast).
