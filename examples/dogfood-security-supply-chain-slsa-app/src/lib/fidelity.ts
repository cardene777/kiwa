 ...
 *
 * The dogfood app is the source of truth for whether the v1.39-1
 * `@kiwa-test/security` v0.2 supply-chain helpers track the real
 * in-toto + sigstore + cosign behaviour closely enough to be trusted as
 * a mock in unit tests. The report tracks the same 13 ops the adapter
 ...
 * accuracy) do not apply. SLSA / reproducible / attestation /
 * orchestrator latency samples feed `perf.p95Ms` so end-user timings
 * stay visible.
 */
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the SLSA + reproducible +
   * attestation + orchestrator surface — 13 ops (startSlsa /
   * verifySlsaLevel / closeSlsa / startReproducible /
   * matchReproducibleBuild / closeReproducible / startAttestation /
   * signProvenance / verifyAttestation / closeAttestation /
   * startOrchestrator / orchestrateDecision / closeOrchestrator) are
   * the AC scope of Issue CAR-866.
   */
