...
import (
	...
	counter "github.com/cardene777/kiwa/examples/go-gin-poc"
)
...
func setup(t *testing.T) (*counter.Counter, *kiwa_gin.TestServer) {
	t.Helper()
	c := counter.NewCounter()
	engine := counter.NewEngine(c)
	return c, kiwa_gin.NewTestServer(t, engine)
}

// 1) GET /healthz returns 200 "ok" — sanity check for the engine wiring.
...
// 6) Unknown route returns gin's default 404.
