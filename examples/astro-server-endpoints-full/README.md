...
`@kiwa-test/astro` v1.0.x の `invokeEndpoint` helper を **実 Astro v5 SSR project** に統合した参考実装。
...
pnpm -F examples-astro-server-endpoints-full build  # 依存 @kiwa-test/astro + @kiwa-test/core + Astro build
```
...
helper の API 詳細は [`@kiwa-test/astro` の README](../../packages/astro/README.md) を参照。
