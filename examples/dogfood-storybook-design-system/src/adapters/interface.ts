 ...
 * (backed by `@kiwa-test/component` `createStoryRegistry`). Both satisfy the
 * same contract so behavioural fidelity between real vs mock can be measured
 * side-by-side and fed to `@kiwa-test/quality-metrics` 7-axis release gate.
 *
...
export interface StorybookAdapter {
   ...
   * `@kiwa-test/component` + returns any injected violations.
   */
