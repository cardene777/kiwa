 ...
 * `@kiwa-test/quality-metrics` 7-axis release gate, and emits a JSON +
 * markdown report so the release process can consume it.
 *
 * The dogfood app is the source of truth for whether the kiwa
 * `@kiwa-test/component` `createStoryRegistry` mock tracks real Storybook 8
 * closely enough to be trusted in unit tests — this harness is where that
 * trust is measured.
 *
 * Component dogfoods use the common 7 axes (coverage 3 / fidelity / perf
 * p95 / mutation / behavior test count). The AI-LLM 4 axes (cost / latency /
 * token / accuracy) do not apply — Storybook is a rendering surface, not a
 * token-priced generative surface. Latency samples still feed `perf.p95Ms`
 * (the story mount + play round-trip axis) so component render performance
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock ops the app exercised vs the real Storybook adapter
   * surface — 6 ops (registerAll / listStories / resolveArgs / mount /
...
/**
 * Drive the 4 story flows against both adapters, capturing traces + rolled
 * up latency samples. The real adapter may be skipped — divergences are
...
export async function runAdapterMatrix(input: {
  mock: StorybookAdapter;
  real: StorybookAdapter;
  run: (adapter: StorybookAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Real path is allowed to throw when STORYBOOK_URL is unset (SkippedError)
  ...
  try {
    ...
    // Trace already reflects the failure (STORYBOOK_REAL_ENV_MISSING /
    // STORYBOOK_LIVE_NOT_IMPLEMENTED). Downstream divergences count is
