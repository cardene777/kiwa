 ...
 * `@kiwa-test/ai-llm` v0.5 multi-agent-orchestration + agent-swarm
 * helpers track the real Vercel AI SDK + Anthropic Messages behaviour
 * closely enough to be trusted as a mock in unit tests. The report
 * tracks the same 13 ops the adapter contract exposes so any
 ...
 * deterministic state-machine + majority-vote code paths (not a real LLM
 * call), so the 4 sub axes are populated with orchestration-specific
 * proxies —
 *  - cost = $0 per request (no upstream Anthropic call)
 *  - latency = same distribution as perf (in-process planner)
 *  - token = task + delegation length divided by 4 (Claude tokenizer proxy)
 *  - accuracy = 1.0 (deterministic delegation + majority-vote, mock=real by design)
 * Once the real Vercel AI SDK + Anthropic driver lands (follow-up
 * milestone), each proxy is replaced by the corresponding real
 * measurement.
 */
...
import type { LlmMaoSwarmAdapter, TraceEvent } from '../adapters/interface.js';
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the mao + swarm + pipeline
   * surface — 13 ops (startMao / assembleCrew / delegateBySupervisor /
   * transitionGraph / completeRound / closeMao / startSwarm / assignRoles
   * / allocateTasks / reachConsensus / tolerateByzantine / closeSwarm /
   * runPipeline) are the AC scope of Sub-Issue CAR-889.
   */
...
export function runFidelityHarness(input: FidelityRunInput): FidelityRunOutput {
  ...
  // Deterministic planner — cost is $0 per request because the delegation
  // rotation + majority vote + Byzantine gate are in-process. Populate 1
  // sample per op so cost.perRequestUsd is defined.
  ...
  // Anthropic driver lands, actual mock-vs-real trace similarity
  ...
  const report = assembleReport({
    ...
    accuracy: accuracyFromSamples({
      ...
      method: 'deterministic-delegation-parity',
    }),
...
export async function runAdapterMatrix(input: {
  mock: LlmMaoSwarmAdapter;
  real: LlmMaoSwarmAdapter;
  run: (adapter: LlmMaoSwarmAdapter) => Promise<number[]>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
