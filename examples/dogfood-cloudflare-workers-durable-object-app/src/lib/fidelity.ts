 ...
 * `@kiwa-test/edge` v0.2 durable-object + websocket-edge axes track a real
 ...
 *  - durable-object: driveRoomJoin, driveRoomBroadcast, driveStorageTx,
 *    driveAlarmPurge
 *  - websocket-edge: driveWsUpgrade, driveWsSend, driveWsClose,
 *    driveWsHibernation
 *
 ...
 * apply — Durable Object mutations and WebSocket frames are not
 * token-priced generative surfaces. Latency samples feed `perf.p95Ms` so
 * room-join + storage-write + WebSocket send performance stays visible.
 */
...
export async function runAdapterMatrix(input: {
  mock: CloudflareDurableObjectAdapter;
  real: CloudflareDurableObjectAdapter;
  run: (adapter: CloudflareDurableObjectAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Real path is allowed to throw when KIWA_MODE=real + WRANGLER_KEY=1
