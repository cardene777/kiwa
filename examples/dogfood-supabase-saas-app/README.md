...
Dogfood app 1 (v1.11-2) — a SaaS-shaped Next.js-style app that exercises Supabase Auth core + advanced across five features (email/password + magic link + OAuth PKCE + MFA TOTP + SSO SAML + Web3 SIWE + RLS-protected doc list) so we can measure how faithfully `@kiwa-lab/auth` mocks track the real Supabase behaviour.

## Modes

- `KIWA_MODE=mock` (default) — driven by `makeMockAdapter()` (`@kiwa-lab/auth` core + advanced envs)
