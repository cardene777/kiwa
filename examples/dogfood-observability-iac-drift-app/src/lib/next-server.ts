/**
 * Tiny HTTP server that mounts the plan / drift / policy handlers
 * directly. The Playwright e2e specs spawn this server so Chromium can
 * drive the full IaC observability ceremony from a real browser origin,
...
import {
  handleDriftRequest,
  validateDriftRequest,
} from '../app/drift/route.js';
import {
  handlePolicyRequest,
  validatePolicyRequest,
} from '../app/policy/route.js';
import type { IacAdapter } from '../adapters/interface.js';
...
async function dispatch(
  ...
  if (route === 'drift') {
    const parsed = validateDriftRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handleDriftRequest(adapter, parsed.value);
  }
  const parsed = validatePolicyRequest(body);
  if (!parsed.ok) {
    return { ok: false, errorKind: parsed.errorKind };
  }
  return handlePolicyRequest(adapter, parsed.value);
}
