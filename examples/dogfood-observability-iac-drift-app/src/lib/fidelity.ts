 ...
 * `@kiwa-test/observability` v2.2 IaC helpers track the real Terraform +
 ...
 * AI-LLM 4 axes (cost / latency / token / accuracy) do not apply. Plan /
 * drift / policy latency samples feed `perf.p95Ms` so operator timings
 * stay visible.
 */
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the plan + drift + policy
   * surface — 10 ops (startPlan / capturePlan / closePlan / startDrift /
   * detectDrift / closeDrift / startPolicy / evaluatePolicy /
   * attributeCost / closePolicy) are the AC scope of Issue CAR-1047.
   */
   ...
   * regress before the app declares an axe-config baseline. The IaC
...
export async function runAdapterMatrix(input: {
  mock: IacAdapter;
  real: IacAdapter;
  run: (adapter: IacAdapter) => Promise<number[]>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
