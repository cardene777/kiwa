/**
 * Adapter resolver — reads `KIWA_MODE` + `CHAOS_AIOPS_STACK_READY`
 * and returns the correct chaos + AIOps adapter for the current
 * environment.
 *
 * The resolver is the single place that inspects env keys — every other
 * module accepts an already-constructed adapter so tests can inject
 * either flavor without polluting process.env.
 */
...
export function resolveAdapter(opts: ResolveAdapterOptions = {}): ChaosAiopsAdapter {
