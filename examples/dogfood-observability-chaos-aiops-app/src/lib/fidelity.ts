 ...
 * `@kiwa-test/observability` v2.2 chaos + aiops helpers track the real
 ...
 * perf p95 / mutation / behavior test count) + optional a11y axis. The
 * AI-LLM 4 axes (cost / latency / token / accuracy) do not apply.
 * Chaos / remediation / rca latency samples feed `perf.p95Ms` so
 * operator timings stay visible.
 */
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the chaos + remediation
   * + rca surface — 12 ops (startChaos / injectFault / triggerRollback
   * / closeChaos / startRemediation / detectAnomaly /
   * executeRemediation / closeRemediation / startRca /
   * analyzeRootCause / correlateAlerts / closeRca) are the AC scope of
   * Issue CAR-1049.
   */
   ...
   * Absent = keep the legacy 7-axis behaviour so this test suite does
   * not regress before the app declares an axe-config baseline. The
   * chaos-aiops dogfood maps to the SaaS tier (strict 0/0/0) by default
...
export async function runAdapterMatrix(input: {
  mock: ChaosAiopsAdapter;
  real: ChaosAiopsAdapter;
  run: (adapter: ChaosAiopsAdapter) => Promise<number[]>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
