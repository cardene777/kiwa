/**
 * Playwright e2e — testcontainers probe flow (v1.31-4).
 *
 * Drives the /testcontainers-probe route end-to-end against the ad-hoc HTTP
 * server. Assertions cover the AC axis "NATS 2.10+ testcontainers probe".
 */
...
test.describe('testcontainers probe — real driver env-gate', () => {
  ...
  test('testcontainers probe reports mock endpoints + reachable=true', async () => {
    ...
    expect(probe.result.natsUrl).toContain('nats-mock');
    expect(probe.result.natsImage).toContain('nats:');
