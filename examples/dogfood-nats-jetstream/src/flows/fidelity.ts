 ...
 *
 * v1.31-4 upgrades the harness to the 13-axis release gate (7 common +
 ...
 * pipeline + v1.31-3 redpanda-schema-registry shape.
 */
...
export async function runAdapterMatrix(input: {
  mock: NatsJetStreamAdapter;
  real: NatsJetStreamAdapter;
  run: (adapter: NatsJetStreamAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  perfSamplesMs: number[];
