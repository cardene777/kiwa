//! End-to-end PoC for the kiwa-test-rs v0.2 actix-web adapter.
//!
//! Exercises the Counter API defined in `src/lib.rs` through
//! `kiwa::actix::test_app` — no real port bind, no manual runtime, no leaked
//! state across cases (TestApp Drop tears down the per-test actix-rt
//! runtime).
...
use rust_actix_web_poc::{configure, AddRequest, CountResponse, CounterState};

// PoC 1 — `GET /count` returns 0 from a fresh CounterState.
#[test]
fn get_count_starts_at_zero() {
    let state = CounterState::default();
    let test = test_app(move || App::new().app_data(Data::new(state.clone())).configure(configure));
...
fn increment_bumps_counter() {
    ...
    assert_eq!(*state_inspect.0.lock().expect("counter snapshot"), 1);
}
...
fn reset_clears_counter() {
    ...
    assert_eq!(*state_inspect.0.lock().expect("primed"), 3);
    ...
    let body: CountResponse = serde_json::from_slice(resp.body()).expect("parse CountResponse");
    assert_eq!(body, CountResponse { value: 0 });
    assert_eq!(*state_inspect.0.lock().expect("after reset"), 0);
}

// PoC 6 — unknown route surfaces actix-web's default 404 response through the
