...
describe('fidelity harness — 3 platform × 3 stage grid', () => {
  ...
  it('schema-bump stage row has correct op sequence', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    const row = report.rows.find((r) => r.scenario.stage === 'schema-bump');
  ...
  it('data-migrate stage row has correct op sequence', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    const row = report.rows.find((r) => r.scenario.stage === 'data-migrate');
    expect(row?.mockTrace.map((s) => s.op)).toEqual([
      'startDataMigrate',
      'migrateOneInstance',
      'migrateBatch',
  ...
  it('rollout stage row has correct op sequence', async () => {
    const report = await runFidelity(makeMockAdapter(), makeRealAdapter());
    const row = report.rows.find((r) => r.scenario.stage === 'rollout');
    expect(row?.mockTrace.map((s) => s.op)).toEqual([
      'startRollout',
      'completeRolloutOp',
      'rollbackRollout',
