...
describe('dogfood-supabase-realtime-chat — fidelity harness', () => {
  it('T-DFS-FID-001 mock adapter covers all 4 ops (joinRoom / sendMessage / getPresence / sendTyping)', async () => {
    ...
    const matrix = await runAdapterMatrix({
      ...
      real,
      run: async (a) => {
        await joinAndSayHi(a).catch(() => undefined);
        await burstTyping(a).catch(() => undefined);
        await leaveRoomFlow(a).catch(() => undefined);
      },
    });
    ...
    expect(output.report.provider).toBe('@kiwa-lab/realtime/supabase-realtime-chat');
  ...
  it('T-DFS-FID-002 divergence is flagged when real mode is skipped', async () => {
    ...
    const output = runFidelityHarness({
      ...
      mockTraces: matrix.mockTraces,
      realTraces: matrix.realTraces,
      opsUnderTest: ['joinRoom', 'sendMessage', 'getPresence'],
      mockLatencySamplesMs: matrix.mockLatencySamplesMs,
      coverageSummary: {
        lines: { pct: 100 },
        branches: { pct: 100 },
  ...
  it('T-DFS-FID-003 harness emits markdown + json outputs and evaluates 7 axes for realtime provider', async () => {
    ...
    // because they are AI-LLM-specific and Supabase Realtime is not a
    // token-priced generative surface.
