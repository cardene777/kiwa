...
describe(MODULE, () => {
  it(
    'measures chat adapter 4 ops and emits a perf report',
    async () => {
      const adapter = makeMockAdapter();

      const joinRoom = await measure({
        name: 'joinRoom',
        iterations: 60,
        warmup: 3,
        fn: async () => {
          await adapter.joinRoom({
            ...
            user: { id: `u${Math.random()}`, name: 'test' },
          });
        },
      });

      const sendMessage = await measure({
        name: 'sendMessage',
        iterations: 100,
        warmup: 5,
        fn: async () => {
      ...
      const sendTyping = await measure({
        name: 'sendTyping',
        iterations: 30,
        warmup: 3,
        fn: async () => {
      ...
      const results: BaselineMap = { joinRoom, sendMessage, getPresence, sendTyping };
      ...
      const gates = {
        joinRoom: evaluatePerfGate({ result: joinRoom, thresholds: { p95Ms: 50 } }),
        sendMessage: evaluatePerfGate({ result: sendMessage, thresholds: { p95Ms: 30 } }),
        getPresence: evaluatePerfGate({ result: getPresence, thresholds: { p95Ms: 30 } }),
        sendTyping: evaluatePerfGate({ result: sendTyping, thresholds: { p95Ms: 100 } }),
      };
