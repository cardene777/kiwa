 ...
 *
 * The dogfood app is the source of truth for whether the kiwa Realtime mock
 * tracks real Supabase behaviour closely enough to be trusted as a mock in
 ...
 * token / accuracy) do not apply — Supabase Realtime is a socket / pub-sub
 * primitive, not a token-priced generative call. Latency samples still feed
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the real Supabase surface —
   * 4 methods (joinRoom / sendMessage / getPresence / sendTyping) are the AC
   * scope of Issue #711.
   */
...
export async function runAdapterMatrix(input: {
  mock: ChatRoomAdapter;
  real: ChatRoomAdapter;
  run: (adapter: ChatRoomAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Combine subscribe + publish latency samples so `perf.p95Ms` reflects the
  // socket round-trip. This mirrors what a user-facing chat client sees end
  // to end (subscribe + message send).
