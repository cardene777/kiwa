 ...
 * `@kiwa-lab/edge` v0.2 geo-replicated + edge-kv + cron-trigger axes
 ...
 *  - edge-kv: driveKvWrite, driveKvRangeQuery, driveReadYourWrites
 *  - cron-trigger: driveCronSchedule, driveCronComplete
 *
 ...
 * apply — Deno KV writes and cron ticks are not token-priced generative
 * surfaces. Latency samples feed `perf.p95Ms` so multi-region write +
 * read-your-writes + cron schedule performance stays visible.
 */
...
export async function runAdapterMatrix(input: {
  mock: DenoDeployAdapter;
  real: DenoDeployAdapter;
  run: (adapter: DenoDeployAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Real path is allowed to throw when KIWA_MODE=real + DENO_DEPLOY_KEY=1
