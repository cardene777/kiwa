/**
 * Fidelity harness — runs the same 5-op adapter surface against both mock
 ...
 * `POSTGRES_ENV_MISSING` / `REAL_ADAPTER_NOT_IMPLEMENTED` in real is a
...
export interface FidelityRunInput {
   ...
   * (`@kiwa-test/orm` = `saas` here). When present, `evaluateReleaseGate`
   * adds a `mutation.tier` axis using the SSOT threshold table. Absent =
   * legacy 7-axis behaviour (backward compat).
   */
...
export async function runAdapterMatrix(input: {
  mock: PostgresCdcOutboxAdapter;
  real: PostgresCdcOutboxAdapter;
  run: (adapter: PostgresCdcOutboxAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  perfSamplesMs: number[];
