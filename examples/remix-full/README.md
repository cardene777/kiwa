...
`@kiwa-test/remix` v1.0.x の **2 helper** (`invokeLoader` / `invokeAction` / `invokeResourceRoute`) を **実 Remix v2 project** に統合した参考実装。
...
pnpm -F examples-remix-full build  # 依存 @kiwa-test/remix + @kiwa-test/core を先に build + Remix build
```
...
各 helper の API 詳細は [`@kiwa-test/remix` の README](../../packages/remix/README.md) を参照。
