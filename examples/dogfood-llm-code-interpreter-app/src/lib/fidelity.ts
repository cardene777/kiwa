 ...
 * `@kiwa-test/ai-llm` v0.5 code-interpreter helpers track the real
 * Vercel AI SDK + Anthropic Messages + sandboxed Python REPL behaviour
 * closely enough to be trusted as a mock in unit tests. The report
 * tracks the same 7 ops the adapter contract exposes so any divergence
 ...
 * deterministic sandbox + execution + tool-use + rollback code paths
 * (not a real LLM call), so the 4 sub axes are populated with
 * interpreter-specific proxies —
 *  - cost = $0 per request (no upstream Anthropic call、 no sandbox spin-up)
 *  - latency = same distribution as perf (in-process sandbox state machine)
 *  - token = code + tool arg length divided by 4 (Claude tokenizer proxy)
 *  - accuracy = 1.0 (deterministic sandbox + memory snapshot, mock=real by design)
 * Once the real Vercel AI SDK + Anthropic driver + sandbox binary lands
 * (follow-up milestone), each proxy is replaced by the corresponding
 * real measurement.
 */
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the sandbox + tool +
   * pipeline surface — 7 ops (startCi / startSandbox / executeCode /
   * useTool / rollback / closeCi / runPipeline) are the AC scope of
   * Sub-Issue CAR-890.
   */
...
export function runFidelityHarness(input: FidelityRunInput): FidelityRunOutput {
  ...
  // Deterministic sandbox — cost is $0 per request because the sandbox
  // spin-up + code execution + tool use + rollback are in-process.
  ...
  // Anthropic driver + sandbox binary lands, actual mock-vs-real trace
  // similarity replaces this constant.
  ...
  const report = assembleReport({
    ...
    accuracy: accuracyFromSamples({
      ...
      method: 'deterministic-sandbox-parity',
    }),
...
export async function runAdapterMatrix(input: {
  mock: LlmCodeInterpreterAdapter;
  real: LlmCodeInterpreterAdapter;
  run: (adapter: LlmCodeInterpreterAdapter) => Promise<number[]>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
