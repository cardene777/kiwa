...
export async function runAdapterMatrix(input: {
  mock: SqliteWalFtsAdapter;
  real: SqliteWalFtsAdapter;
  run: (adapter: SqliteWalFtsAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  perfSamplesMs: number[];
...
export async function runFullSurface(adapter: SqliteWalFtsAdapter): Promise<void> {
  await adapter.driveWalFullJourney();
  await adapter.driveFts5FullJourney();
  await adapter.driveEdgeRoundtrip();
