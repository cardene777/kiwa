 ...
 * `@kiwa-test/search` v0.3 vector + semantic axes track the production
...
export interface FidelityRunInput {
  ...
  /**
   * The mock adapter walks 13 methods on the SearchHybridAdapter surface
   * (excluding `reset` + `trace` themselves). `mockCoveredMethods`
   * reports how many of the 13 methods were exercised — target = 13/13.
   */
  ...
  for (const [op, mockEntries] of mockByOp) {
    ...
    if (mockOk !== realOk) {
      divergences.push({
        ...
        target: mockEntries[0]?.target ?? 'meilisearch',
        state: 'divergence',
  ...
  for (const [op, realEntries] of realByOp) {
    if (!mockByOp.has(op)) {
      divergences.push({
        ...
        target: realEntries[0]?.target ?? 'meilisearch',
        state: 'divergence',
        timestampMs: Date.now(),
        ok: false,
...
function renderNotes(
  ...
  if (divergences.length === 0) {
    return `No behavioral divergences observed across ${opsUnderTest.length} ops (SEARCH_HYBRID_HARNESS_OPS).`;
  }
  const divergentOps = Array.from(new Set(divergences.map((d) => d.op)));
  return `Divergent ops (${divergences.length}): ${divergentOps.join(', ')} of ${opsUnderTest.length} SEARCH_HYBRID_HARNESS_OPS.`;
}
...
export async function runAdapterMatrix<T>(input: {
  mock: SearchHybridAdapter;
  real: SearchHybridAdapter;
  run: (adapter: SearchHybridAdapter) => Promise<T>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
