 ...
 * between real (mediasoup + coturn) and mock (@kiwa-lab/realtime v0.2) at
 ...
 *  2. `KIWA_MODE=real` env var (only when the mediasoup env-detect passes)
...
/**
 * Pick and construct a {@link VideoCallAdapter} for the current request or
 * test. The chosen mode is returned separately so callers can add it to their
 * telemetry / trace metadata without introspecting `adapter.mode`.
 */
export function resolveVideoCallAdapter(
  ...
  // The real adapter refuses to run unless the mediasoup env-detect passes.
