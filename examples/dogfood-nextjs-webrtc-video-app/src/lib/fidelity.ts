 ...
 * WebRTC mock tracks real mediasoup + coturn behaviour closely enough to be
 ...
 * strict 0/0/0 because WebRTC is a transport primitive with no DOM). The
 * AI-LLM 4 axes (cost / latency / token / accuracy) do not apply. Join /
 * publish / iceRestart latency samples feed `perf.p95Ms` so realtime
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the mediasoup + coturn
   * surface — 8 methods (joinRoom / leaveRoom / publishTrack /
   * unpublishTrack / muteTrack / unmuteTrack / selectLayer / iceRestart)
   * are the AC scope of Sub-Issue #972.
   */
   ...
   * Issue #995). Absent = keep legacy 7-axis behaviour so this test suite
   * does not regress before the app declares an axe-config baseline. The
   * transport-primitive apps map to the SaaS tier (strict 0/0/0) by default
   * because they emit no DOM.
   */
  a11y?: {
    totals: {
      critical?: number;
...
export function runFidelityHarness(input: FidelityRunInput): FidelityRunOutput {
  ...
  // for baseline-declaring apps. Absent = keep the legacy 7-axis behaviour
  // (`evaluateReleaseGate` is backward compatible when the context field is
  // undefined). Default tier for these transport-primitive dogfoods is
  // SaaS (strict 0/0/0) because they emit no DOM.
...
export async function runAdapterMatrix(input: {
  mock: VideoCallAdapter;
  real: VideoCallAdapter;
  run: (adapter: VideoCallAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Combine join + publish + iceRestart samples so `perf.p95Ms` reflects the
  // full user-facing call setup + reconnect round-trip.
  const samples = [
    ...m.joinLatencySamplesMs,
    ...m.publishLatencySamplesMs,
    ...m.iceRestartLatencySamplesMs,
  ];
  return {
