...
import { chatWithUploadedImage } from '../../src/flows/chat-flows.js';
...
describe(`${MODULE} — live`, () => {
  it(
    '3-layer LIVE perf: describeImage via real Anthropic vision API (env-skip when ANTHROPIC_API_KEY absent)',
    async () => {
      ...
      const result = await runPerf3LayerLive({
        ...
        ops: [
          {
            name: 'describeImage.live',
            // Live threshold — vision requests are heavier than text-only
            // so we allow ~2500ms serial p95 vs 1500ms for the plain
            // chatbot.
            serialP95CapMs: 2500,
