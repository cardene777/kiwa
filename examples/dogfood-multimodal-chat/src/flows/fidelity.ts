 ...
 * `@kiwa-lab/quality-metrics` 11-axis release gate, and emits a JSON +
 * markdown report so the release process can consume it.
 *
 * The multimodal dogfood is the source of truth for whether the kiwa
 * mock tracks Anthropic vision behaviour closely enough to be trusted
 * as a mock in unit tests — this harness is where that trust is
 * measured.
 */
...
export interface FidelityRunInput {
  ...
  opsUnderTest: string[];
  /**
   * Real vs mock text pairs — accuracy is calculated as the mean
   * Jaccard similarity of the two strings. When the real adapter is
   * skipped we still record a placeholder score so the harness has
  ...
  /**
   * How many mock methods the app exercised vs the real Anthropic
   * vision surface — 3 methods (describeImage / streamDescribeImage /
   * compareImages) are the AC scope of Issue #749.
   */
...
export async function runAdapterMatrix(input: {
  mock: VisionChatAdapter;
  real: VisionChatAdapter;
  run: (adapter: VisionChatAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockCostSamplesUsd: number[];
