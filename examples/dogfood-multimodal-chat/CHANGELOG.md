...
- 454588a: v1.15-4: dogfood-multimodal-chat — Anthropic vision (image upload + streaming + cost tracking + multi-image compare)
  ...
  - `examples/dogfood-multimodal-chat/` を新設。 v1.15-1 で lands した `@kiwa-lab/ai-llm` v0.2 multimodal を dogfood で実測、 real Anthropic Messages API (`image` content blocks + `type: base64` / `type: url`) と kiwa mock adapter の fidelity を 4 flow (describeImage / streamDescribeImage / compareImages 3 ops) で突合する。
