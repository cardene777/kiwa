...
Dogfood app for `@kiwa-lab/auth` v1.21-1a (WebAuthn L3 adapter). A Next.js 15 App Router RP (Relying Party) that exercises the four spec-critical WebAuthn ceremonies — credential creation + attestation, credential assertion + userVerification, and residentKey-based discoverable credentials — under two drivers:
...
- `KIWA_MODE=mock` — `@kiwa-lab/auth` `setupWebAuthnEnv` + `credentialCreation` / `credentialAssertion` deterministic mock. Always runs.
...
src/
  adapters/
    ...
    mock.ts             # makeMockAdapter — @kiwa-lab/auth setupWebAuthnEnv + credentialCreation + credentialAssertion
