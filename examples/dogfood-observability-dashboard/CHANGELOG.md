...
- 4ede2f8: feat: v1.17-2 — new Next.js App Router style Grafana dashboard dogfood app driven behind a provider-neutral `DashboardAdapter`. Exercises the v1.17-1 `@kiwa/observability` `DashboardMock` end-to-end against a real Prometheus HTTP API path.
  ...
  - Fidelity harness — trace-diffing across mock vs real for `runQuery` + `refreshDashboard`. Feeds `@kiwa/quality-metrics` common 7-axis release gate (`@kiwa/observability/` prefix triggers the non-AI-LLM branch).
