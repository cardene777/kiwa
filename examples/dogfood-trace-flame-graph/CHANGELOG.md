...
- 773f9ad: feat: v1.17-4 — new trace flame graph explorer dogfood app driven behind a provider-neutral `FlameExplorerAdapter`. Exercises the v1.17-1 `@kiwa-lab/observability` `buildSpanTree` + `renderFlameGraph` + `LogCorrelationIndex` end-to-end against a live Jaeger HTTP API path.
  ...
  - Fidelity harness — trace-diffing across mock vs real for all 5 ops under measurement (`loadTrace` / `renderFlame` / `drillDown` / `joinLogs` / `filterByName`). Feeds `@kiwa-lab/quality-metrics` common 7-axis release gate (`@kiwa-lab/observability/` prefix triggers the non-AI-LLM branch).
