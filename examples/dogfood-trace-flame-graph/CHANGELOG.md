...
- 773f9ad: feat: v1.17-4 — new trace flame graph explorer dogfood app driven behind a provider-neutral `FlameExplorerAdapter`. Exercises the v1.17-1 `@kiwa-test/observability` `buildSpanTree` + `renderFlameGraph` + `LogCorrelationIndex` end-to-end against a live Jaeger HTTP API path.
