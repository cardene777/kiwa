 ...
 *
 * The dogfood app is the source of truth for whether the v1.34-1
 * `@kiwa-test/component` v0.3 + `@kiwa-test/nextjs` v1.2 helpers track the
 * real Next.js 15.4 + React 19.1 App Router behaviour closely enough to be
 * trusted as a mock in unit tests. The report tracks the same 12 ops the
 * adapter contract exposes so any divergence surfaces the op that broke.
 *
 * Component / framework dogfoods use the common 7 axes (coverage 3 /
 * fidelity / perf p95 / mutation / behavior test count) + optional a11y
 * axis (v1.30-4, SaaS-tier strict 0/0/0). The AI-LLM 4 axes (cost /
 * latency / token / accuracy) do not apply. Article / catalog / transition
 * / form latency samples feed `perf.p95Ms` so end-user timings stay
 * visible.
 */
...
import type { RscStreamingAdapter, TraceEvent } from '../adapters/interface.js';
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the RSC + streaming SSR +
   * view transitions + form action advanced surface — 12 ops
   * (renderArticle / enterSuspense / streamChunk / completeArticle /
   * startCatalog / pendCatalogBoundary / captureCatalogError /
   * hydrateCatalogBoundary / startTransition / finishTransition /
   * assertAnimation / markFormPending / applyOptimistic / enhanceForm /
   * resolveForm) are the AC scope of Sub-Issue CAR-785.
   */
   ...
   * regress before the app declares an axe-config baseline. The RSC
   * streaming dogfood maps to the SaaS tier (strict 0/0/0) by default
   * because the mock adapter emits no DOM.
   */
  a11y?: {
    totals: {
      critical?: number;
...
export async function runAdapterMatrix(input: {
  mock: RscStreamingAdapter;
  real: RscStreamingAdapter;
  run: (adapter: RscStreamingAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  const samples = [
    ...m.articleLatencySamplesMs,
    ...m.catalogLatencySamplesMs,
    ...m.transitionLatencySamplesMs,
    ...m.formLatencySamplesMs,
  ];
  return {
