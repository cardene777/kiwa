/**
 * Tiny HTTP server that mounts the rbac / abac / policy-store handlers
 * directly. The Playwright e2e specs spawn this server so Chromium can
 * drive the full RBAC + ABAC + policy-store ceremony from a real browser
 * origin, without paying the Next.js dev-server startup cost.
 *
...
import {
  handleRbacRequest,
  validateRbacRequest,
} from '../app/rbac/route.js';
import {
  handleAbacRequest,
  validateAbacRequest,
} from '../app/abac/route.js';
import {
  handlePolicyStoreRequest,
  validatePolicyStoreRequest,
} from '../app/policy-store/route.js';
...
async function dispatch(
  ...
  if (route === 'rbac') {
    const parsed = validateRbacRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handleRbacRequest(adapter, parsed.value);
  }
  if (route === 'abac') {
    const parsed = validateAbacRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handleAbacRequest(adapter, parsed.value);
  }
  const parsed = validatePolicyStoreRequest(body);
  if (!parsed.ok) {
    return { ok: false, errorKind: parsed.errorKind };
  }
  return handlePolicyStoreRequest(adapter, parsed.value);
}
