 ...
 * `@kiwa-test/security` v0.1 authorization helpers track the real Casbin
 * policy store behaviour closely enough to be trusted as a mock in unit
 * tests. The report tracks the same 15 ops the adapter contract exposes
 * so any divergence surfaces the op that broke.
 *
 * Security dogfoods use the common 7 axes (coverage 3 / fidelity / perf
 * p95 / mutation / behavior test count) + optional a11y axis (v1.30-4,
 * SaaS-tier strict 0/0/0). The AI-LLM 4 axes (cost / latency / token /
 * accuracy) do not apply. RBAC / ABAC / policy-store latency samples feed
 * `perf.p95Ms` so end-user timings stay visible.
 */
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the RBAC + ABAC +
   * policy-store surface — 15 ops (startRbac / attachRole / expandRoles /
   * checkPermission / closeRbac / startAbac / attachRule / evaluateAbac /
   * evaluateCombined / closeAbac / startPolicyStore / publishPolicy /
   * activatePolicy / rollbackPolicy / closePolicyStore) are the AC scope
   * of Sub-Issue CAR-827.
   */
   ...
   * regress before the app declares an axe-config baseline. The security
...
export async function runAdapterMatrix(input: {
  mock: SecurityAdapter;
  real: SecurityAdapter;
  run: (adapter: SecurityAdapter) => Promise<number[]>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
