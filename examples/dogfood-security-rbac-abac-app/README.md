...
A Casbin-style Authorization service that drives RBAC (role hierarchy + permission inheritance) + ABAC (attribute matching + combining algorithms) + policy engine (combined RBAC/ABAC decision) + policy store (versioning + rollback + activation) across a provider-neutral `SecurityAdapter`. Both mock (`@kiwa-test/security` v0.1 authorization semantics) and real (Casbin-style policy store driver when `AUTHZ_STORE_READY=1` + `KIWA_POLICY_STORE_URL` are set) implementations satisfy the same 15-op contract so the fidelity harness can diff them side by side.
...
The vitest suite writes `quality-report/fidelity-latest.md` + `quality-report/fidelity-latest.json` that `@kiwa-test/quality-metrics` picks up for the 13-axis release gate. The doc counterpart is added alongside the existing security dogfood entry.
