/**
 * Shared fixture for the Playwright e2e specs (v1.31-2) — spins up a small
 ...
 * on the JSON responses; no UI is rendered because the Kafka pipeline is a
 * headless streaming surface.
 */
...
import type { KafkaEventPipelineAdapter } from '../../src/adapters/interface.js';
 ...
 */
type RouteHandler = (adapter: KafkaEventPipelineAdapter, body: unknown) => Promise<unknown>;

const ROUTES: Readonly<Record<string, RouteHandler>> = {
  '/producer': async (adapter, body) => {
    const events = Array.isArray((body as { events?: unknown }).events)
      ? ((body as { events: readonly { orderId: string; region: 'us' | 'eu' | 'apac'; total: number }[] }).events)
      : [];
    return adapter.driveProducer(events);
  },
  '/consumer-group': async (adapter, body) => {
    const topic = String((body as { topic?: string }).topic ?? 'orders');
    return adapter.driveConsumerGroup(topic);
  },
  '/transaction': async (adapter, body) => {
    const b = body as {
      topic?: string;
      commit?: readonly string[];
      abort?: readonly string[];
    };
    return adapter.driveTransaction(String(b.topic ?? 'txn'), {
      commit: b.commit ?? [],
      abort: b.abort ?? [],
    });
  },
  '/dlq': async (adapter, body) => {
    const payloads = ((body as { payloads?: unknown }).payloads ?? []) as readonly {
      orderId: string;
      valid: boolean;
    }[];
    return adapter.driveDlq(payloads);
  },
  '/raw-protocol': async (adapter) => adapter.driveRawProtocol(),
  '/isr-hw': async (adapter, body) => {
    const b = body as { topic?: string; partition?: number; targetOffset?: number };
    return adapter.driveIsrHighWatermark(
      String(b.topic ?? 'orders'),
      Number(b.partition ?? 0),
      Number(b.targetOffset ?? 1),
    );
  },
  '/schema-registry': async (adapter, body) => {
    const b = body as { subject?: string; compatibility?: 'BACKWARD' | 'FORWARD' | 'FULL' };
    return adapter.driveSchemaRegistry({
      subject: String(b.subject ?? 'orders-value'),
      compatibility: b.compatibility ?? 'BACKWARD',
    });
  },
...
export interface AdapterServerHandle {
  ...
  readonly adapter: KafkaEventPipelineAdapter;
}
...
export async function bootAdapterServer(): Promise<AdapterServerHandle> {
  ...
  const server: Server = createServer(async (nodeReq, nodeRes) => {
    // Collect the request body with a hard cap so a malformed client cannot
    // starve the server on memory.
    const chunks: Buffer[] = [];
    let totalBytes = 0;
    ...
    if (!handler || nodeReq.method !== 'POST') {
      // GET / fallback — return an empty HTML page so the browser can load it
      // as a document + run `fetch` against the routes.
      nodeRes.setHeader('content-type', 'text/html');
      nodeRes.end(
        '<!doctype html><html><head><title>kafka-event-pipeline</title></head><body></body></html>',
      );
      return;
    }
    try {
