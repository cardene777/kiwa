...
`@kiwa-lab/nuxt` v1.0.4 の **3 helper 全部** (`invokeEventHandler` / `invokeRouteMiddleware` / `invokeNitroPlugin`) を **実 Nuxt 3 project** に統合した参考実装。
...
pnpm -F examples-nuxt-server-routes-full build  # 依存 @kiwa-lab/nuxt + @kiwa-lab/core を先に build
```
...
各 helper の API 詳細は [`@kiwa-lab/nuxt` の README](../../packages/nuxt/README.md) を参照。
