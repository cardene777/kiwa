 ...
 * into the `@kiwa-test/quality-metrics` 11-axis release gate, and emits a
 ...
 * the real MCP protocol closely enough to be trusted in unit tests — this
 * harness is where that trust is measured.
 */
...
export async function runAdapterMatrix(input: {
  mock: McpAgentAdapter;
  real: McpAgentAdapter;
  run: (adapter: McpAgentAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockCostSamplesUsd: number[];
