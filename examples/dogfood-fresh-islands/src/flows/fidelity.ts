 ...
 * `@kiwa-lab/fresh` mock tracks real Deno Fresh closely enough to be
 ...
 * / token / accuracy) do not apply — Route Handler dispatch and Islands
 * hydration are not token-priced generative surfaces. Latency samples
 * still feed `perf.p95Ms` so handler + hydration performance stays visible
 * in the report.
 */
...
/**
 * Drive the 4 Fresh flows against both adapters, capturing traces + rolled
...
export async function runAdapterMatrix(input: {
  mock: FreshAdapter;
  real: FreshAdapter;
  run: (adapter: FreshAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Real path is allowed to throw when DENO_INSTALLED is unset (SkippedError)
