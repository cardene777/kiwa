 ...
 * `@kiwa-test/security` v0.2 siem-audit + incident-response helpers
 * track the real Splunk + PagerDuty SIEM behaviour closely enough to
 * be trusted as a mock in unit tests. The report tracks the same 16
 * ops the adapter contract exposes so any divergence surfaces the op
 * that broke.
 *
 ...
 * accuracy) do not apply. SIEM / incident / orchestrator latency
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the SIEM + incident-
   * response + orchestrator surface — 16 ops (startSiem / structureEvent
   * / sealEvents / applyRetention / correlate / closeSiem / startIncident
   * / triggerPlaybook / classifySeverity / escalate / captureForensics /
   * recordPostMortem / closeIncident / startOrchestrator /
   * orchestrateDecision / closeOrchestrator) are the AC scope of Issue
   * CAR-865.
   */
