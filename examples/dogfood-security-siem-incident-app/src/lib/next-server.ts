/**
 * Tiny HTTP server that mounts the siem / incident / ir-orchestrator
 * handlers directly. The Playwright e2e specs spawn this server so
 * Chromium can drive the full SIEM + incident-response ceremony from
 * a real browser origin, without paying the Next.js dev-server startup
 * cost.
 *
...
const ROUTE_MAP = {
  '/siem': 'siem',
  '/incident': 'incident',
  '/ir-orchestrator': 'ir-orchestrator',
} as const;
...
async function dispatch(
  ...
  if (route === 'siem') {
    const parsed = validateSiemRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handleSiemRequest(adapter, parsed.value);
  }
  if (route === 'incident') {
    const parsed = validateIncidentRequest(body);
    if (!parsed.ok) {
      return { ok: false, errorKind: parsed.errorKind };
    }
    return handleIncidentRequest(adapter, parsed.value);
  }
  const parsed = validateOrchestratorRequest(body);
  if (!parsed.ok) {
    return { ok: false, errorKind: parsed.errorKind };
  }
  return handleOrchestratorRequest(adapter, parsed.value);
}
