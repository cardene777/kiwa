...
| `/api/reset` | `reset-stream` / `migrate-connection` / `validate` | `resetStream` / `migrateConnection` |
...
| validator が拒む | **400** | `body_not_object` / `unknown_kind` / `missing_session_id` / `missing_stream_id` / `missing_url` / `missing_data` |
| adapter が投げる | **500** | `err.message` の**英文** |

413 だけ **JSON ではなく平文**を返す。 他の 3 段は `{"ok": false, "errorKind": ...}`。

### `errorKind` は 3 種類の由来を混ぜる

`invalid_json` と validator の 6 token は固定だが、adapter が投げた失敗は
...
開いていない session への op も 500 になる
(`makeMockAdapter.openUniStream: session nope is not open`)。

### stream id は種別ごとの連番

実測で uni は `uni-0` → `uni-1`、bi は `bi-0` → `bi-1` → `bi-2` → `bi-3` と進む。
**種別ごとに別の採番**で、session をまたいでも続く。
...
`windowRemaining` の補充は `@kiwa-lab` 側にある。 そのため
**`windowRemaining` は単純な引き算にならない**。
...
書込**後**の `windowRemaining` は補充を挟むため、前の値から引いた数にならない。

### uni-stream は window を持たない
...
### `metrics()` は 11 個の counter と 3 種の latency 標本を持つ
...
- **WebTransport を張らない**。 browser の `fetch` で JSON を投げるだけで、
  QUIC も stream も 1 度も動かない。 「2 つのタブが別々の session を持つ」 ことも
  server 側の `sessionId` で区別しているだけになる
- **HTTP 層が test file にある**。 `bootAdapterServer` は production の Nuxt route ではないため、
  route の並べ方も status の割り当ても実運用では別物になりうる
- **413 だけ平文を返す**。 他の段が JSON なので、`errorKind` を読む client は
  上限超過だけ parse に失敗する
- **`errorKind` が 3 種類の由来を混ぜる**。 validator は固定 token だが adapter は英文
- **`windowRemaining` が単純な引き算にならない**。 補充が `@kiwa-lab` 側にあるため、
  応答の値から残量を計算する consumer は外れる
...
| # | 観点 | 対象 |
|---|---|---|
| 1 | 2 つのタブが別々の session を開く | `open-session` が両方 `ok` |
...
- T-E2E-001 (P0) — 2 つのタブが別々の session を開き、uni の書込 / bi の backpressure /
  接続の移行が同じ handler を通ることを確かめる happy path

**2 context を使うのはこの 1 件だけ**。 session を分けることが主題なので畳めない。
...
| `read-stream` / `send-datagram` / `close-session` / `reset-stream` / `validate` | できる | 投げていない |
| 同じ session を 2 度開いた時の 500 | できる | 1 度ずつしか開いていない |
| validator の 6 token | できる | 妥当な body だけを送っている |
...
- 応答の `errorKind` を安定した token として扱えるかが決まっていない。
  `invalid_json` と validator の 6 token は固定だが、adapter の失敗は
