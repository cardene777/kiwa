 ...
 * `@kiwa-test/solidjs` mock tracks real Solid closely enough to be trusted
 ...
 * latency / token / accuracy) do not apply — Signal reactivity is not a
 * token-priced generative surface. Latency samples still feed
 * `perf.p95Ms` so component render performance stays visible in the
 * report.
 */
...
/**
 * Drive the 4 signal flows against both adapters, capturing traces + rolled
...
export async function runAdapterMatrix(input: {
  mock: SolidAdapter;
  real: SolidAdapter;
  run: (adapter: SolidAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Real path is allowed to throw when SOLID_LIVE is unset (SkippedError)
