 ...
 * `@kiwa-test/payment` v0.5 bnpl helpers track the real Klarna / Affirm /
 * Afterpay broker behaviour closely enough to be trusted as a mock in
 * unit tests. The report tracks the same 14 ops the adapter contract
 ...
 * 4 axes (cost / latency / token / accuracy) do not apply. Plan / risk /
 * collection latency samples feed `perf.p95Ms` so end-user timings stay
 * visible.
 */
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock methods the app exercised vs the plan + risk +
   * collection surface — 14 ops (startPlan / createPlan /
   * scheduleInstallment / closePlan / startRisk / scoreCustomerRisk /
   * checkRiskThreshold / closeRisk / startCollection / chargeLateFee /
   * markPaid / settlePlan / checkCollectionStatus / closeCollection) are
   * the AC scope of Issue CAR-979.
   */
