 ...
 * `@kiwa-test/search` v0.3 faceted-advanced + geo axes track the
 * production Algolia HTTP surface closely enough to be trusted as a
 * mock in unit tests. The report tracks the same 13 ops the adapter
...
import type { FacetedGeoSearchAdapter, TraceEvent } from '../adapters/interface.js';
...
export interface FidelityRunInput {
  ...
  /**
   * The mock adapter walks 13 methods on the FacetedGeoSearchAdapter
   * surface (excluding `reset` + `trace` themselves).
   * `mockCoveredMethods` reports how many of the 13 methods were
   * exercised — target = 13/13.
   */
  ...
  for (const [op, mockEntries] of mockByOp) {
    ...
    if (mockOk !== realOk) {
      divergences.push({
        ...
        target: mockEntries[0]?.target ?? 'algolia',
        state: 'divergence',
  ...
  for (const [op, realEntries] of realByOp) {
    if (!mockByOp.has(op)) {
      divergences.push({
        ...
        target: realEntries[0]?.target ?? 'algolia',
        state: 'divergence',
        timestampMs: Date.now(),
        ok: false,
...
function renderNotes(
  ...
  if (divergences.length === 0) {
    return `No behavioral divergences observed across ${opsUnderTest.length} ops (FACETED_GEO_HARNESS_OPS).`;
  }
  const divergentOps = Array.from(new Set(divergences.map((d) => d.op)));
  return `Divergent ops (${divergences.length}): ${divergentOps.join(', ')} of ${opsUnderTest.length} FACETED_GEO_HARNESS_OPS.`;
}
...
export async function runAdapterMatrix<T>(input: {
  mock: FacetedGeoSearchAdapter;
  real: FacetedGeoSearchAdapter;
  run: (adapter: FacetedGeoSearchAdapter) => Promise<T>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
