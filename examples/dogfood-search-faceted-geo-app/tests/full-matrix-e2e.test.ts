 ...
 * through both mock + real adapters and asserts the trace + op set stay
 * stable. This is the widest lifecycle test the dogfood app runs — any
 * missing op / divergent event on the neutral trace surfaces here.
 */
...
describe('dogfood-search-faceted-geo-app — full matrix e2e', () => {
  it('T-DFSFG-FM-001 driveFullMatrix on mock runs 3 lifecycles (1 backend x 3 fixtures)', async () => {
    const mock = makeMockAdapter();
    const result = await driveFullMatrix(mock);
    // Algolia backend x 3 fixtures = 3 lifecycles.
    expect(result.lifecyclesRun).toBe(3);
  });

  it('T-DFSFG-FM-002 driveFullMatrix on mock emits every op from OPS_UNDER_TEST at least once', async () => {
    const mock = makeMockAdapter();
  ...
  it('T-DFSFG-FM-003 driveFacetedGeoLifecycle emits emitFidelitySignal + queryAlgoliaHealth for restaurants', async () => {
    const mock = makeMockAdapter();
    await driveFacetedGeoLifecycle(mock, {
      backend: 'algolia',
      indexId: 'idx-rest',
    ...
    expect(events).toContain('search.algolia_health_ok');
  });

  it('T-DFSFG-FM-004 events fixture combines facet + geo ops in one lifecycle', async () => {
    const mock = makeMockAdapter();
    await driveFacetedGeoLifecycle(mock, {
      backend: 'algolia',
      indexId: 'idx-events',
    ...
    // Facet ops (from events fixture — distinct + refined only)
  ...
  it('T-DFSFG-FM-005 real adapter (env-missing) emits search.env_missing for every bucket op', async () => {
    const real = makeRealAdapter({ env: {} });
    await driveFullMatrix(real);
    // Every seedFacetedDocuments / seedGeoDocuments call should emit search.env_missing.
  ...
  it('T-DFSFG-FM-006 real adapter with forceEnvPresent walks the same op set as mock', async () => {
    const mock = makeMockAdapter();
    ...
    // Every op the mock walks must also appear on the forced-env real adapter.
    for (const op of mockOps) {
      expect(realOps.has(op)).toBe(true);
    }
  });

  it('T-DFSFG-FM-007 every fixture lifecycle contributes at least one trace entry', async () => {
  ...
  it('T-DFSFG-FM-008 trace timestamps are monotonically non-decreasing per lifecycle', async () => {
    const mock = makeMockAdapter();
    await driveFacetedGeoLifecycle(mock, {
      backend: 'algolia',
      indexId: 'idx-rest',
