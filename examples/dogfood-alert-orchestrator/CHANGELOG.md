...
- bd368a2: feat: v1.17-3 — new Node.js style Prometheus AlertManager alert orchestrator dogfood app driven behind a provider-neutral `AlertOrchestratorAdapter`. Exercises the v1.17-1 `@kiwa/observability` `AlertRouter` end-to-end against a live AlertManager HTTP API path.
  ...
  - `makeMockAdapter` — drives `@kiwa/observability` `AlertRouter` + `TelemetryCollector`; deterministic lifecycle transitions (fire → route → silence → escalate → resolve) so fidelity tests assert on receiver identity + silence suppression + escalation timing without wall-clock coupling.
  ...
  - Fidelity harness — trace-diffing across mock vs real for `evaluateRules` + `routeAlert` + `advanceEscalation` (3 ops under measurement; `emitMetric` excluded since real is noop). Feeds `@kiwa/quality-metrics` common 7-axis release gate (`@kiwa/observability/` prefix triggers the non-AI-LLM branch).
