...
Dogfood app (v1.17-3) — a Node.js style **Prometheus AlertManager** orchestrator driven behind a provider-neutral surface. 10 alert rules (threshold / rate / anomaly) evaluate against a metric window; fires walk a 3-level routing tree (severity → team → channel); silences (literal + regex) suppress by label match; an escalation ladder (L1 30s → L2 5min → L3 30min) walks the state machine so tests can assert receiver X saw alert Y at tier N. The fidelity harness diffs mock vs a live AlertManager HTTP API and feeds `@kiwa-lab/quality-metrics` release gate.

## Modes

- `KIWA_MODE=mock` (default) — driven by `makeMockAdapter()` (`@kiwa-lab/observability` `AlertRouter` + `TelemetryCollector`, deterministic lifecycle transitions).
