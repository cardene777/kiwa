 ...
 *
 * Alert-orchestrator dogfoods use the common 7 axes (coverage 3 /
 * fidelity / perf p95 / mutation / behavior test count). The AI-LLM 4
 * axes (cost / latency / token / accuracy) do not apply — Prometheus
 * AlertManager is an infrastructure primitive, not a token-priced
 * generative call. Evaluation + routing latency samples feed
 * `perf.p95Ms` so orchestrator performance stays visible in the report.
 */
...
export interface FidelityRunInput {
  ...
  /**
   * How many mock ops the app exercised vs the real AlertManager surface.
   * The 3 ops (`evaluateRules` / `routeAlert` / `advanceEscalation`)
   * scope the AC of Issue #780.
   */
 ...
 * up evaluation + routing latency samples. The real adapter may be
 * skipped — divergences are counted from the trace either way.
 */
export async function runAdapterMatrix(input: {
  mock: AlertOrchestratorAdapter;
  real: AlertOrchestratorAdapter;
  run: (adapter: AlertOrchestratorAdapter) => Promise<void>;
}): Promise<{
  mockTraces: TraceEvent[];
  realTraces: TraceEvent[];
  mockLatencySamplesMs: number[];
  ...
  // Combine evaluation + routing latency samples so `perf.p95Ms`
  // reflects the alert lifecycle a Prometheus AlertManager user sees.
  const samples = [...m.evaluationLatencySamplesMs, ...m.routingLatencySamplesMs];
  return {
