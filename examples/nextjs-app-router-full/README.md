...
`@kiwa-test/nextjs` v1.0.x の **3 helper 全部** (`invokeServerAction` / `invokeMiddleware` / `renderServerComponent`) + **Route Handler** を **実 Next.js v15 App Router project** に統合した参考実装。 Issue #525 v1.2 milestone の 5/5 完遂例。
...
pnpm -F examples-nextjs-app-router-full build  # 依存 @kiwa-test/nextjs + @kiwa-test/core + Next.js build
```
...
各 helper の API 詳細は [`@kiwa-test/nextjs` の README](../../packages/nextjs/README.md) を参照。
