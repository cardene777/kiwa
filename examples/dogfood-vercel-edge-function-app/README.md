...
Dogfood app v1.24-3 — a Next.js 15 middleware + edge runtime app on Vercel that exercises the `geo-replicated` + `edge-kv` + `streaming-response` axes of `@kiwa-test/edge` v0.2 end-to-end. Drivable in both `KIWA_MODE=real` (Vercel Edge sandbox + `VERCEL_KEY=1` env-gate) and `KIWA_MODE=mock` (`@kiwa-test/edge` v0.2 8 axis semantics) so behavioural fidelity feeds the release gate 7 axis.
...
- `KIWA_MODE=mock` — `@kiwa-test/edge` v0.2 `createGeoReplicatedSession` / `geoPrimaryWrite` / `markReplicaLagged` / `syncReplica` / `createEdgeKvSession` / `kvRead` / `kvWrite` / `kvRangeQuery` / `openStream` / `sendChunk` / `closeStream` deterministic mocks. Always runs.
