/**
 * Shared fixture for the Playwright e2e specs (v1.31-3) — spins up a small
 ...
 * on the JSON responses; no UI is rendered because the schema-registry
 * dogfood is a headless surface.
 */
...
import type { RedpandaSchemaRegistryAdapter } from '../../src/adapters/interface.js';
...
const ROUTES: Readonly<Record<string, RouteHandler>> = {
  '/register': async (adapter) => adapter.driveRegister(),
  '/evolution': async (adapter) => adapter.driveEvolution(),
  '/compat-modes': async (adapter) => adapter.driveCompatibilityModes(),
  '/publish': async (adapter, body) => {
    const payloads = ((body as { payloads?: unknown }).payloads ?? []) as readonly {
      id: string;
      displayName: string;
      region: string;
    }[];
    const normalized = payloads.map((p) => sampleUserPayload(p));
    return adapter.drivePublish(normalized);
  },
  '/emit-fidelity': async (adapter) => {
    await adapter.emitFidelity();
    return { ok: true };
  },
  '/evolution-transitive': async (adapter) => adapter.driveEvolutionTransitive(),
  '/subject-strategies': async (adapter) => adapter.driveSubjectStrategies(),
  '/console-admin': async (adapter) => adapter.driveConsoleAdmin(),
...
export interface AdapterServerHandle {
  ...
  readonly adapter: RedpandaSchemaRegistryAdapter;
}
...
export async function bootAdapterServer(): Promise<AdapterServerHandle> {
  ...
  const server: Server = createServer(async (nodeReq, nodeRes) => {
    ...
    if (!handler || nodeReq.method !== 'POST') {
      ...
      nodeRes.end(
        '<!doctype html><html><head><title>redpanda-schema-registry</title></head><body></body></html>',
      );
      return;
    }
    try {
