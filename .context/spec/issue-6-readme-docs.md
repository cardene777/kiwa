...
kiwa v0.1.0 publish 前の最後の前提条件として、README.md を quickstart + features + comparison 全面書き直し + docs/ 配下 5 file (RPC / EVENTS / ERRORS / MIGRATION / COMPARISON) の最小実用ガイドを整備する。
...
- AC 1: `README.md` を全面書き直しで、(a) 1 段落でツール定位 (anvil ベース fork chain testing 特化の headless E2E fixture) (b) `pnpm dlx @kiwa/cli init` → `pnpm install` → `pnpm exec playwright test` の 3 step quickstart (c) features 箇条書き 5-8 項目 (d) `docs/COMPARISON.md` への link を含むツール選び navigation (e) MIT license + Issue tracker link を末尾に配置する
...
- AC 5: `docs/MIGRATION.md` (v0.x → v0.y breaking change tracking、v0.0 → v0.1 初版エントリ) + `docs/COMPARISON.md` (Synpress / wallet-mock / kiwa の 3 軸比較表 + 使い分けガイド) を新規作成、両者とも先頭 1 段落で読者ターゲット + 末尾に関連 link を配置する
...
| docs/COMPARISON.md | Synpress / wallet-mock / kiwa 3 軸比較 + 使い分け |
...
- `/Users/cardene/Desktop/projects/kiwa/docs/RPC.md` — 9 RPC + anvilProxy fallback
- `/Users/cardene/Desktop/projects/kiwa/docs/EVENTS.md` — 4 event + triggerEvent API
- `/Users/cardene/Desktop/projects/kiwa/docs/ERRORS.md` — EIP-1193 code + error envelope + page 境界
- `/Users/cardene/Desktop/projects/kiwa/docs/MIGRATION.md` — v0.0 → v0.1 + 将来 entry テンプレ
- `/Users/cardene/Desktop/projects/kiwa/docs/COMPARISON.md` — Synpress / wallet-mock / kiwa 3 軸比較

修正 1 file:

- `/Users/cardene/Desktop/projects/kiwa/README.md` — 全面書き直し (現状 1 行 `# kiwa` placeholder → quickstart 完成版)
...
- Issue #5 で CLI init scaffold が完成済 (PR #12)、README quickstart で `pnpm dlx @kiwa/cli init` 経由の手順を書ける条件が整った
...
- リスク 3: **README quickstart の 3 step (init / install / test) が実環境で動作しない** — `pnpm dlx @kiwa/cli init` は npm publish 後にしか動かないため、本 Issue 時点では smoke test 不可、local `node packages/cli/dist/index.js init` の動作確認に留める (緩和: README quickstart は `pnpm dlx @kiwa/cli init` 表記、AC 6 publish 後の動作確認は別タスク)
