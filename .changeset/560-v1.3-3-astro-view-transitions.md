...
- View Transitions API 未対応 browser の cross-document fallback path を `supportsViewTransitions: false` で再現、 fallback 時は before-preparation / after-preparation を skip して before-swap → after-swap のみ dispatch する (Astro 公式 router の挙動準拠)。
- `before-preparation.preventDefault()` で nav cancellation、 `before-preparation.loader = async () => {...}` で loader override、 `before-swap.swap()` で DOM swap timing 制御を listener に expose。
