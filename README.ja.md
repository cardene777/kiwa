...
| Component (8 framework) | framework ごとに runner / fixture drift | `@kiwa-lab/ui` 1 package で React / Vue / Svelte / Solid / Lit / Qwik / Angular / Chromium |
...
> 既存コードがある場合、 kiwa は **後付け導入主軸** で設計されています。 Layer 2 generator はすべて既存 test から spec を逆算可能。 dApp 例は [tests/docs/retrofit-existing-dapp.ja.md](./tests/docs/retrofit-existing-dapp.ja.md)、 非 dApp stack は [`@kiwa-lab/api`](./packages/api) / [`@kiwa-lab/ui`](./packages/ui) / [`@kiwa-lab/cli-test`](./packages/cli-test) を参照。
...
| パッケージ | 用途 |
|---|---|
| [`@kiwa-lab/dapp`](./packages/dapp) | Playwright fixture: `window.ethereum` inject、 anvil 起動、 sign、 mine、 time-travel、 EIP-6963 multi-wallet、 ERC-4337 smart account、 custom-error helper |
| [`@kiwa-lab/cli`](./packages/cli) | `kiwa init` で `@kiwa-lab/dapp` と連携した Playwright project を scaffold |
| [`@kiwa-lab/core`](./packages/core) | spec markdown parser (9 列の `test-spec-*.md` を `SpecDoc` に変換、 全 adapter 共通) |
| [`@kiwa-lab/api`](./packages/api) | API integration adapter (Vitest + msw + supertest + Playwright `request`) |
| [`@kiwa-lab/ui`](./packages/ui) | React / Vue / Svelte / SolidJS / Lit / Qwik / Angular / real Chromium 対応 component adapter (render / interaction / snapshot 3 mode) |
| [`@kiwa-lab/data`](./packages/data) | queue / cron / batch adapter (in-memory queue + fake clock + idempotency / DLQ) |
| [`@kiwa-lab/cli-test`](./packages/cli-test) | CLI / shell / file IO adapter (isolated tempdir + env override + stdout/stderr snapshot) |
...
> npm 公開済 — `pnpm dlx @kiwa-lab/cli init` で clone 不要で導入できます。
...
cd ~/kiwa
pnpm install
pnpm -F @kiwa-lab/dapp -F @kiwa-lab/cli build
...
pnpm exec kiwa init     # または: node $HOME/kiwa/packages/cli/dist/bin.js init
```

通常利用は Option C (`pnpm dlx @kiwa-lab/cli init`) を推奨、 公開済 0.1.0 を直接取得します。

### CJS / Next.js 14 プロジェクトとの共存

`@kiwa-lab/dapp` は **ESM と CJS の両方を build 出力** (`dist/index.js` + `dist/index.cjs`) しており、 `import` / `require` どちらも解決できます。 以下いずれの project にもそのまま導入可能:

| Project type | そのまま使える形式 |
|---|---|
| Pure ESM (`"type": "module"`) | `import { dappE2eTest } from '@kiwa-lab/dapp'` |
| Pure CJS (`"type": "commonjs"`) | `const { dappE2eTest } = require('@kiwa-lab/dapp')` |
...
`@kiwa-lab/dapp` は **production 現実的、 ただし挙動差を明示** することを設計方針としています。 default 設定での主な挙動差:
...
### Layer 2: dApp E2E fixture (`/kiwa-play` + `@kiwa-lab/dapp`)
...
|---|---|---|
| [`@kiwa-lab/api`](./packages/api) | **96.06 %** | 90 |
| [`@kiwa-lab/a11y`](./packages/a11y) | **93.62 %** | 90 |
| [`@kiwa-lab/ui`](./packages/ui) | **91.76 %** | 80 |
| [`@kiwa-lab/cli-test`](./packages/cli-test) | 89.69 % | 80 |
| [`@kiwa-lab/data`](./packages/data) | 86.93 % | 80 |
| [`@kiwa-lab/core`](./packages/core) | 85.51 % | 80 |
| [`@kiwa-lab/dapp`](./packages/dapp) | 85.09 % | 80 |
...
### dApp E2E reference (`@kiwa-lab/dapp` fixture)
