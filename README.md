...
| [`/kiwa-rust`](./.claude/skills/kiwa-rust/SKILL.md) | **Layer 2** (polyglot Rust) | Layer 1 spec (`rust-unit` / `rust-integration` / `rust-axum` / `rust-actix-web`) → cargo test `tests/*.rs` driven by `kiwa-test-rs` (`setup_env`, `assert_kiwa_eq!`, `mock_server` + `reqwest`, `axum::test_app`, `actix::test_app`), runs `cargo test` and gates on `cargo llvm-cov` (`--mode {axum|actix-web}` flag added in v1.5-6 for web framework spec → test conversion) |
| [`/kiwa-go`](./.claude/skills/kiwa-go/SKILL.md) | **Layer 2** (polyglot Go) | Layer 1 spec (`go-unit` / `go-integration` / `go-gin` / `go-echo`) → `testing.T` `*_test.go` driven by `kiwa-test-go` (`SetupUnitEnv`, `AssertEqual`, `NewMockServer` + `http.Client`, `kiwa_gin.NewTestServer`, `kiwa_echo.NewTestServer`), runs `go test` and gates on `go test -cover` (`--mode {gin|echo}` flag added in v1.5-6 for web framework spec → test conversion) |
