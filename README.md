...
| **v1.8** | 新 layer 追加 — auth (`@kiwa-test/auth` v0.1 NextAuth v5 + Lucia v3 + Better Auth) + job queue (`@kiwa-test/queue` v0.1 BullMQ + Inngest) + cache (`@kiwa-test/cache` v0.1 Redis testcontainers) + Layer 1 spec 3 layer + Layer 2 skill 3 種 (`/kiwa-auth` + `/kiwa-queue` + `/kiwa-cache`) 新規。 v1.5-v1.7 web framework 縦深化から SaaS prod 実 test 需要へ | **0/6 resolved** (親 [#636](https://github.com/cardene777/kiwa/issues/636)) | [v1.8 label](https://github.com/cardene777/kiwa/issues?q=is%3Aissue+label%3Av1.8) |
