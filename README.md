...
| [`/kiwa-auth`](./.claude/skills/kiwa-auth/SKILL.md) | **Layer 2** (auth) | Layer 1 spec → Vitest `test/*.auth.test.ts` driven by `@kiwa-test/auth` (NextAuth v5 / Lucia v3 / Better Auth 3 backend), covers session mock + OAuth provider mock + email/password + magic link + 2FA + passkey + organizations (v1.8) |
| [`/kiwa-queue`](./.claude/skills/kiwa-queue/SKILL.md) | **Layer 2** (job-queue) | Layer 1 spec → Vitest `test/*.queue.test.ts` driven by `@kiwa-test/queue` (BullMQ sandbox / testcontainers + Inngest stub / dev-server), covers job add / process / retry / fail / drain / delay + event send / step function / concurrency (v1.8) |
| [`/kiwa-cache`](./.claude/skills/kiwa-cache/SKILL.md) | **Layer 2** (cache) | Layer 1 spec → Vitest `test/*.cache.test.ts` driven by `@kiwa-test/cache` (Redis in-memory / testcontainers + ioredis / node-redis client), covers get / set / delete / TTL / expiry / Pub/Sub / publish-subscribe / assertPublished (v1.8) |
...
| [`@kiwa-test/auth`](./packages/auth) (v0.1+) | Auth test adapter — NextAuth v5 (Auth.js) session (jwt + database) + Google / GitHub / Email (Magic Link) provider mocks + Prisma / Drizzle-compatible in-memory database adapter + Lucia v3 + Better Auth (v1.8) |
| [`@kiwa-test/queue`](./packages/queue) (v0.1+) | Queue test adapter — BullMQ (Redis-backed) sandbox (in-memory) + testcontainers Redis env + Inngest (event-driven) dev-server + stub env, with job / run assertion helpers (`waitForJob` / `assertProcessed` / `assertFailed` / `assertRetried` / `assertQueueDrained` / `assertFunctionRan` / `assertStepRan`) (v1.8) |
| [`@kiwa-test/cache`](./packages/cache) (v0.1+) | Cache test adapter — Redis (testcontainers) live env + in-memory sandbox env, with TTL / Pub/Sub / expiry assertion helpers (`assertTTL` / `subscribe` / `assertPublished`) (v1.8) |
...
| **v1.8** | 新 layer 追加 — auth (`@kiwa-test/auth` v0.1 NextAuth v5 + Lucia v3 + Better Auth) + job queue (`@kiwa-test/queue` v0.1 BullMQ + Inngest) + cache (`@kiwa-test/cache` v0.1 Redis testcontainers) + Layer 1 spec 3 layer + Layer 2 skill 3 種 (`/kiwa-auth` + `/kiwa-queue` + `/kiwa-cache`) 新規。 v1.5-v1.7 web framework 縦深化から SaaS prod 実 test 需要へ | **1/6 resolved** ([#637](https://github.com/cardene777/kiwa/issues/637)、 親 [#636](https://github.com/cardene777/kiwa/issues/636)) | [v1.8 label](https://github.com/cardene777/kiwa/issues?q=is%3Aissue+label%3Av1.8) |
