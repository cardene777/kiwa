...
| **v1.10** | SaaS + self-host + polyglot 拡張 — auth (`@kiwa-test/auth` v0.3 + Supabase Auth core + advanced) + job queue (`@kiwa-test/queue` v0.3 + RabbitMQ basic + advanced) + Rust contract layer (`kiwa-test-rs` v0.2 + Foundry-rs + alloy.rs)。 v1.9 の provider 対称拡張から異なる 3 軸への並行拡張へシフト、 SaaS teams (Supabase) / self-host teams (RabbitMQ) / dApp Rust teams (Foundry-rs + alloy.rs) の 3 層同時カバー | **0/6 resolved** (親 [#666](https://github.com/cardene777/kiwa/issues/666)) | [v1.10 label](https://github.com/cardene777/kiwa/issues?q=is%3Aissue+label%3Av1.10) |
