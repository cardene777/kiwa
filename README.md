...
| **v1.11** | quality gate 補強 — `@kiwa-test/quality-metrics` harness (5 軸統一 score: coverage / test count / fidelity / perf p95 / mutation kill、 release gate SSOT 化) + dogfood app 3 種 (Supabase SaaS + RabbitMQ worker + Foundry dApp、 real mode vs mock mode の fidelity 実測) + docs 補強 (typedoc TS + cargo doc Rust + forge doc Solidity + tutorial 5 本 + migration guide) + GitHub Pages 公開 (VitePress + local skill `/docs-publish` で CI 全面禁止規約に沿った build)。 v1.10 まで provider 拡張の直交軸だったが「release 品質を数値で判断可能にする」 縦軸に思想シフト | **0/6 resolved** (親 [#680](https://github.com/cardene777/kiwa/issues/680)) | [v1.11 label](https://github.com/cardene777/kiwa/issues?q=is%3Aissue+label%3Av1.11) |
